  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [agentRes, taskRes, byAgentRes] = await Promise.all([
        fetch(`/api/tokens?action=agent-costs&timeframe=${selectedTimeframe}`, { signal: AbortSignal.timeout(8000) }),
        fetch(`/api/tokens?action=task-costs&timeframe=${selectedTimeframe}`, { signal: AbortSignal.timeout(8000) }),
        fetch(`/api/tokens/by-agent?days=${timeframeToDays(selectedTimeframe)}`, { signal: AbortSignal.timeout(8000) }),
      ])
      ...
      setError('Failed to load agent cost data. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }, [selectedTimeframe])
