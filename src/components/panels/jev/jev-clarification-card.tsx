...
export function JevClarificationCard({
  question, value, onAnswer, onBack,
}: {
    ...
    <section aria-labelledby={`jev-question-${question.id}`} className="space-y-4 rounded-xl border border-border bg-card p-5">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {question.options.map((option) => (
          <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onAnswer(option.value)} className={`rounded-lg border p-3 text-left transition ${value === option.value ? 'border-void-cyan bg-void-cyan/10' : 'border-border bg-background hover:border-void-cyan/50'}`}>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <input aria-label="Custom answer" value={custom} onChange={(event) => setCustom(event.target.value)} placeholder="Or write a custom answer…" className="min-w-0 flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-void-cyan" />
        <Button type="button" variant="outline" disabled={!custom.trim()} onClick={() => onAnswer(custom.trim())}>Use custom answer</Button>
      </div>
