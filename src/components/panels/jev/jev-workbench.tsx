...
export function JevWorkbench({
  ...
  useEffect(() => {
    if (selected?.configuration) setRetain(selected.configuration.retainPreview)
  }, [selected])
  ...
  const loadContext = async () => {
    ...
    try {
      const context = await onLoadContext(selectedId)
  ...
  const run = async () => {
    ...
    try {
      ...
      const nextResult = await onRun(selectedId, state, retain)
      if (requestId !== requestRef.current) return
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col" aria-labelledby="jev-workbench-title">
      <div className="flex shrink-0 flex-wrap items-end gap-2 border-b border-[var(--chat-border)] bg-[var(--chat-elevated)] px-3 py-2 md:px-5">
        ...
        <label className="w-36 text-[11px] text-muted-foreground">Context format<select className={`${field} mt-0.5 h-8 py-1`} value={format} onChange={(event) => setFormat(event.target.value as 'text' | 'json')}><option value="text">Plain text</option><option value="json">Structured JSON</option></select></label>
        ...
        <Button size="sm" onClick={() => void run()} disabled={!canRun || running || loadingContext || !stateText.trim() || !selectedId}>{running ? 'Evaluating…' : canRun ? 'Evaluate with Jev' : disabledReason ?? 'Unavailable'}</Button>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-3 md:p-5">
        <div className="mx-auto max-w-7xl space-y-4">
          ...
          <label className="block text-xs text-muted-foreground">Context Jev will evaluate<textarea className={`${field} mt-1 min-h-52 resize-y font-mono text-xs`} value={stateText} onChange={(event) => { setStateText(event.target.value); setContextInfo(null); setResult(null) }} placeholder={format === 'json' ? '{"goal":"What should this change accomplish?","evidence":{"tests":"…"}}' : 'Describe the goal, implementation, tests, risks, and any missing evidence…'} /></label>
          {selected?.configuration ? <p className="text-xs text-muted-foreground">History preview: <strong className="text-foreground">{retain ? 'up to 500 redacted characters' : 'off'}</strong>, as approved in this policy.</p> : <label className="flex items-start gap-2 text-xs text-muted-foreground"><input type="checkbox" className="mt-0.5" checked={retain} onChange={(event) => setRetain(event.target.checked)} /><span>Keep a maximum 500-character redacted preview in history. Off by default.</span></label>}
