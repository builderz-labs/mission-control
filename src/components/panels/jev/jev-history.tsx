      ...
      <table className="w-full min-w-[720px] text-left text-xs">
        ...
        <tbody>{evaluations.map((evaluation) => (
          <tr key={evaluation.id} className="border-b border-border/60 last:border-0">
            ...
            <td className="px-3 py-3"><details><summary className="cursor-pointer text-void-cyan">Inspect</summary><pre className="mt-2 max-w-sm overflow-x-auto whitespace-pre-wrap text-[10px] text-muted-foreground">{evaluation.error_code ?? JSON.stringify(evaluation.answers, null, 2)}</pre></details></td>
          </tr>
        ))}</tbody>
      </table>
    </div>
