...
export function ActivityFeedPanel() {
  ...
  const fetchActivities = useCallback(
    async (since?: number) => {
      try {
        ...
        const response = await fetch(`/api/activities?${params}`, { signal: AbortSignal.timeout(8000) })
  ...
  // ── Merge SSE-delivered activities into local state ──
  // When SSE is connected, polling stops. The SSE handler pushes new
  // activities into store.activities via addActivity. We subscribe here
  // so the panel stays live without polling.
  const { activities: sseActivities, connection } = useMissionControl()
  ...
  useEffect(() => {
    ...
    setActivities((prev) => {
      ...
      return [...filtered, ...prev].slice(0, limit)
    })
    setLastRefresh(Date.now())
  }, [sseActivities, connection.sseConnected, selectedAgent, filter.type, limit])
