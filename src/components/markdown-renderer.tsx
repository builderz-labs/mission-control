...
// Matches the deep links generated by remarkFileLinks (`/memory?path=<p>`).
const MEMORY_LINK_RE = /^\/memory\?path=([^&]+)$/
    ...
    <div className={`prose prose-invert max-w-none ${preview ? 'text-xs' : 'text-sm'}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ href, children }) => {
            const memoryMatch = linkifyFilePaths ? href?.match(MEMORY_LINK_RE) : null
            if (memoryMatch) {
              return (
                <FilePathLink path={decodeURIComponent(memoryMatch[1])} href={href!}>
                  {children}
                </FilePathLink>
              )
            }
