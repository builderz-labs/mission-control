...
interface MessageRow { id: number; from_agent: string; to_agent: string | null; content: string; conversation_id: string | null; created_at: number }
  ...
  if (!typeFilter || typeFilter === 'message') {
    try {
      const messages = db.prepare(`
        ...
        ORDER BY created_at DESC LIMIT ?
      `).all(workspaceId, likeQ, likeQ, limit) as MessageRow[]
      for (const m of messages) {
        results.push({
          type: 'message',
          id: m.id,
          title: `${m.from_agent} → ${m.to_agent || 'all'}`,
          subtitle: m.conversation_id ?? undefined,
  ...
  if (!typeFilter || typeFilter === 'webhook') {
    try {
      const webhooks = db.prepare(`
        ...
        ORDER BY created_at DESC LIMIT ?
      `).all(workspaceId, likeQ, likeQ, limit) as WebhookRow[]
  ...
  if (!typeFilter || typeFilter === 'pipeline') {
    try {
      const pipelines = db.prepare(`
        ...
        ORDER BY created_at DESC LIMIT ?
      `).all(workspaceId, likeQ, likeQ, limit) as PipelineRow[]
