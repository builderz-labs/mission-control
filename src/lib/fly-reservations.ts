...
export type ReservedJob = {
  ...
  outcome_json: string | null; transport: string; observed_cost_usd: number; worker_app: string | null;
}
...
export function reserveFlyJob(db: Database.Database, row: SubmissionRow, input: FlySubmission) {
  return db.transaction(() => {
    ...
    const previous = (db.prepare('SELECT COALESCE(SUM(observed_cost_usd),0) AS spent FROM fly_worker_jobs WHERE submission_id=?')
      .get(row.id) as {spent:number}).spent
    if (!reason && previous + reserve > flyNumber('MC_FLY_PER_JOB_BUDGET_USD')) reason = 'Cumulative per-job compute budget exhausted'
    const daily = flyNumber('MC_FLY_DAILY_BUDGET_USD')
    if (!reason && (flyCommittedSpend(db,day) + reserve > daily || flyCommittedSpend(db,month) + reserve > flyNumber('MC_FLY_MONTHLY_BUDGET_USD'))) reason = 'Global compute budget reserved or exhausted'
    // A project only yields its daily share while another project is actually waiting.
    if (!reason && flyRepositorySpendToday(db,input.repository,day) + reserve > flyRepositoryDailyCap(daily)
      && flyQueueIsContended(db,input.repository)) reason = 'Project daily compute share reached while other projects are queued'
    if (reason) {
    ...
    const name = `mc-${id}`
    const app = process.env.MC_FLY_WORKER_APP || null
    db.prepare(`INSERT INTO fly_worker_jobs (id,task_id,workspace_id,state,worker_class,execution_target,image_kind,
      branch_name,repository,token_hash,estimated_cost_usd,expires_at,submission_id,hourly_rate_usd,launch_name,transport,worker_app)
