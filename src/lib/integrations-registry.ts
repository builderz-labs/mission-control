/**
 * Integration registry — types, data, helpers, and sub-handlers.
 * Extracted from src/app/api/integrations/route.ts for size compliance.
 */
...
// ---------------------------------------------------------------------------
// Sub-handlers (called by HTTP route handlers)
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    switch (integration.id) {
      ...
      case 'github': {
        ...
        result = res.ok
          ? { ok: true, detail: `User: ${(await res.json()).login}` }
...
// ---------------------------------------------------------------------------
// Shared GET helper — build full integration list with status + redacted values
// ---------------------------------------------------------------------------

export async function buildIntegrationList(): Promise<NextResponse | {
  integrations: ReturnType<typeof mapIntegration>[]
  ...
  const ctx = { envMap, opAvailable, xint, ollamaInstalled, ollamaReachable, gwsInstalled, providerSubscriptions, allCategories }
...
type MappedIntegration = {
  ...
  vaultItem: string | null; testable: boolean; recommendation: string | null
}

function mapIntegration(def: IntegrationDef, ctx: {
  ...
  const { envMap, opAvailable, xint, ollamaInstalled, ollamaReachable, gwsInstalled, providerSubscriptions, allCategories } = ctx
  ...
  if ((def.id === 'anthropic' || def.id === 'openai') && !anySet) {
    ...
    if (sub) {
      vars[def.envVars[0]] = { redacted: `${sub.type} (${sub.source})`, set: true }
      allSet = true
      anySet = true
    }
  }
