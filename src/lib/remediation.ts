 ...
 */
export function getRemediationSuggestion(session: ClaudeSessionRow): RemediationStrategy {
  const stability = session.stability_score ?? 100
  const isAnomaly = session.is_anomaly ? true : false
  ...
  if (session.is_active && (session.total_loc_delta ?? 0) === 0 && (session.user_messages ?? 0) > 10) {
    return {
      action: 'FORCE_SYNC',
      priority: 'medium',
