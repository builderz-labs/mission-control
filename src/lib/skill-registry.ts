...
export async function installFromRegistry(req: InstallRequest): Promise<InstallResult> {
  ...
  try {
    ...
    return { ok: false, name, path: skillDir, message: `Fetch failed: ${getErrorMessage(err)}` }
  }
  ...
  try {
    ...
    return { ok: false, name, path: skillDir, message: `Write failed: ${getErrorMessage(err)}` }
  }
