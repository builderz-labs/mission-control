...
import { GitHealth, ProjectHealth, RoadmapPhase, RoadmapTask, Session } from '@/types'
 ...
 */
export function verifyTaskSuccess(session: Session): { success: boolean, confidence: number, reason: string } {
  const stability = session.stability_score ?? 100
  const toolScore = session.tool_success_count / (session.tool_success_count + session.tool_error_count || 1)
  ...
  if (session.total_loc_delta === 0 && session.user_messages > 5) {
