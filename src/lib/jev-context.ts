...
import { allSeedProjects } from '@/lib/fleet-projects'
...
function projectRoot(project: ProjectRow): string | null {
  const catalogPath = allSeedProjects().find((item) => item.slug === project.slug)?.path
  const raw = catalogPath ?? project.description
...
function git(root: string, args: string[]): string {
  try {
    return redactSecrets(execFileSync('git', args, {
...
function redactContext(text: string): string {
  return redactSecrets(text)
    .replace(/((?:api[_-]?key|access[_-]?token|token|password|secret|authorization)\s*[=:]\s*)[^\r\n\s]+/gi, '$1[redacted]')
    .replace(/\b(?:sk|ghp|doppler)[-_][A-Za-z0-9_-]{12,}\b/gi, '[redacted]')
}
...
function referenceContent(root: string): { files: Record<string, string>; included: string[] } {
  ...
  for (const name of REFERENCE_FILES) {
    ...
    try {
      ...
      const content = readFileSync(path, 'utf8').slice(0, Math.min(remaining, MAX_REFERENCE_BYTES))
...
export function buildJevRepositoryContext(
  ...
  const project = db.prepare(`
    SELECT id,name,slug,description,github_repo,github_default_branch
    FROM projects WHERE id=? AND workspace_id=?
  `).get(projectId, workspaceId) as ProjectRow
  const root = projectRoot(project)
  const repository = {
    name: project.name, slug: project.slug, github: project.github_repo,
  ...
  if (!root) return {
    ...
    state: { repository, purpose: project.description || 'No project description is available.' },
  }
