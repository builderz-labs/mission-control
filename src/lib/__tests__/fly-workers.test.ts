...
describe('Fly worker policy', () => {
  it('accepts only immutable digests from the configured worker app', () => {
    ...
    expect(isFlyWorkerImageRef(`registry.fly.io/example-worker-app@sha256:${digest}`, 'example-worker-app')).toBe(true)
    expect(isFlyWorkerImageRef('registry.fly.io/example-worker-app:core-0.153.4-amd64', 'example-worker-app')).toBe(false)
    expect(isFlyWorkerImageRef('registry.fly.io/example-worker-app@sha256:abc', 'example-worker-app')).toBe(false)
    expect(isFlyWorkerImageRef('registry.fly.io/other-app:core', 'example-worker-app')).toBe(false)
  })
