# Graph Report - mission-control  (2026-09-20)

## Corpus Check
- 1304 files · ~999,107 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 16 file(s) not represented in the graph (top: (none) 11, .example 1, .toml 1)

## Summary
- 7399 nodes · 19913 edges · 350 communities (311 shown, 39 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 194 edges (avg confidence: 0.87)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b15a15a9`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- vitest
- widget-primitives.tsx
- mac-cleanup/index.ts
- resolveWithin
- auth.ts
- chat-icons.tsx
- task-board-panel.tsx
- next
- security-audit-panel.tsx
- useMissionControl
- requireRole
- hermes/route.ts
- Changelog
- agent-squad-panel-phase3.tsx
- login/page.tsx
- store/index.ts
- main.mjs
- group-sessions.ts
- getDatabase
- config
- adapters/index.ts
- office-panel.tsx
- integrations/route.ts
- api/status/route.ts
- nav-rail.tsx
- gateway-config/route.ts
- ref_node_assert
- mc-cli.cjs
- chat-session-identity.ts
- @playwright/test
- session-handoff.ts
- utils.ts
- security-scan.ts
- chat-session-pane.tsx
- task-dispatch.ts
- polled.mjs
- engine-logo.tsx
- mc-fly-enroll.py
- build-app.mjs
- jev-setup-session-repository.ts
- cost-tracker-panel.tsx
- chat-composer.tsx
- dashboard.tsx
- chat-usage-tracker.ts
- cron-management-panel.tsx
- apiFetch
- proxy.ts
- webhooks.ts
- db.ts
- widget-grid.tsx
- package.json
- desktop-auth-store.mjs
- Production (Docker)
- skills/route.ts
- websocket.ts
- skill-sync.ts
- memory-utils.ts
- gateway-config-panel.tsx
- Runbook: move the worker pool onto an isolated private network
- dependencies
- scripts
- exec-approvals/route.ts
- injection-guard.ts
- github/route.ts
- FlyMachinesClient
- dashboard-cli-fleets.ts
- exec-approval-panel.tsx
- channels-panel.tsx
- chat-display.ts
- handoff/route.ts
- agent-comms-panel.tsx
- validation.ts
- react
- mcp-audit.ts
- session-terminal-widget.tsx
- openclaw-doctor.ts
- mc-tui.cjs
- claude-fleet-plans.ts
- gnap-sync.ts
- jev-panel.tsx
- session-list-local.ts
- jev-assistant-provider.ts
- scheduler.ts
- workload/route.ts
- github-sync-engine.ts
- layout.tsx
- start-e2e-server.mjs
- gateways/connect/route.ts
- onboarding/route.ts
- dashboard-board.ts
- fly-topology.test.ts
- install.ps1
- mc-provisioner-daemon.js
- src/index.ts
- skill-registry.ts
- graph/route.ts
- fetchWithRetry
- jev-assistant-service.ts
- super-admin.ts
- fly-admission-schema.ts
- desktop/package.json
- agent-runtimes.ts
- Jev Workspace Product Requirements
- grok-sessions.ts
- local-session-list.ts
- Mission Control Style Guide
- schema.sql
- install.sh
- runCommand
- recurring-tasks.ts
- ref_node_os
- gateways/health/route.ts
- api-contract-parity.ts
- cli-inventory.ts
- device-identity.ts
- compilerOptions
- markdown-renderer.tsx
- devDependencies
- framework-templates.ts
- memory-browser-panel.tsx
- agent-templates.ts
- flight-deck/route.ts
- openclaw-doctor-banner.tsx
- runs.ts
- pty-websocket.ts
- jev-cloud.test.ts
- mc-mcp-server.cjs
- skills-panel.tsx
- opencode-sessions.ts
- pty/setup/route.ts
- fly-admission.test.ts
- session-transcript-types.ts
- claude-transcript.ts
- claude-code-sessions.ts
- ref_node_module
- webhook-panel.tsx
- session-transcript-route.ts
- renderDashboard
- session-archive.ts
- chat-desktop-workspace.tsx
- github-pulls-api.ts
- fly-reservations.ts
- pty-manager.ts
- assistant/route.ts
- mc-fly-contract.test.cjs
- agent-filesystem-isolation.test.ts
- tokens/route.ts
- Troubleshooting
- agent-sync.ts
- coordinator-routing.ts
- local-agent-sync.ts
- mc-fly-cli.test.cjs
- notification-daemon.sh
- adapter-compliance.test.ts
- [id]/diagnostics/route.ts
- session-continue-run.ts
- event-bus.ts
- message-bubble.tsx
- chat-fly-jobs.ts
- websocket-utils.ts
- alert-evaluate.ts
- security-scan-card.tsx
- runtime-history.ts
- kimi-sessions.ts
- package-manager-patches/package.json
- jev-context.ts
- fleet-agents.ts
- Orchestration Patterns
- dashboard-board-persist.ts
- hook-profiles.ts
- createTestAgent
- task-dispatch-sandbox.test.ts
- check-api-contract-parity.mjs
- api/health/route.ts
- fix/route.ts
- channels/route.ts
- Everything Included In The v2 Update
- codex-transcript.ts
- unlock-renderer.mjs
- mcp-server.spec.ts
- provision-jobs/[id]/route.ts
- TranscriptMessage
- jev-repository.ts
- session-terminal-cell.tsx
- better-sqlite3
- ref_node_child_process
- createTestTask
- agent-heartbeat.sh
- health-utils.test.ts
- fleet-projects.ts
- attention-detector.ts
- [id]/memory/route.ts
- openclaw-gateway.ts
- claude-sessions.ts
- github.ts
- check-standalone-artifact.mjs
- deploy-standalone.sh
- session-terminal-view.tsx
- Mission Control Hardening
- task-routing.ts
- chat-permission-mode-route.test.ts
- FrameworkAdapter
- handoff-brief.ts
- helpers.ts
- typesafe-probe.ts
- security-audit.sh
- github-activity.ts
- Mission Control
- jev-service.ts
- ClaudeSdkAdapter
- adaptive-context-handoff.ts
- buildGatewayWebSocketUrl
- aggregate/route.ts
- migrations.ts
- generate-env.sh
- mac-safe-reclaim
- start-standalone.sh
- station-doctor.sh
- index/route.ts
- Mission Control Platform Hardening + Full Agent CLI/TUI PRD
- Mission Control Agent Skill
- openclaw-doctor-fix.ts
- minimax.ts
- attribution/route.ts
- jev-validation.ts
- provisioner-client.ts
- process-title.test.sh
- Agent Setup Guide
- Fly worker topology decision — September 6, 2026
- NavRail
- token-utils.test.ts
- next.config.js
- load-env.sh
- reap-controller.test.sh
- gateways/control/route.ts
- permission-mode/route.ts
- docker-entrypoint.sh
- Command groups
- Mission Control
- locale-parity.test.ts
- executeProvisionJob
- load-env.test.sh
- security-audit.test.sh
- take-screenshots.ts
- Findings
- open-mission-control
- build-app.sh script
- check-node-version.mjs
- opencode
- plugins.ts
- openclaw-doctor-route.test.ts
- git-askpass.mjs
- Full Changelog
- 3. Key Hardening Recommendations
- API Endpoints
- cron-occurrences.ts
- Contributor Covenant Code of Conduct
- mock-gateway.mjs
- gen-star-history.py
- dashboard-page-frame.tsx
- Mission Control production audit
- Mission Control Fly command workers
- Quickstart: Your First Agent in 5 Minutes
- keys/route.ts
- verify-jev-live.mjs
- prepare-standalone-artifact.mjs
- auto-credentials.ts
- Deferred / blocked
- test_mc_fly_enroll.py
- split-pane-layout.tsx
- webhook-retry-lease.ts
- claude-tasks.ts
- Spec Files
- Quick Start
- Fly recovery and shared skill hardening — 2026-09-06
- Образцовый пример: команда из 3 провайдеров (Claude + OpenAI + Local) в Mission Control
- Hardening Checklist
- jev-setup-flow.spec.ts
- fly-stale-controller.ts
- Mission Control Fly audit — 2026-09-06
- Screenshot Guide
- 10. Troubleshooting
- release-update-route-security.test.ts
- tasks-route-noop-update.test.ts
- Contributing to Mission Control
- Fly cleanup and capacity audit — September 6, 2026
- Fly scale and cost decision — 2026-09-06
- Jev in Mission Control
- OpenClaw Gateway Hardening
- 4. Агент №2 — Implementor (OpenAI gpt-4o-mini)
- tasks-bulk-put-agent-access.test.ts
- Consolidated Fly implementation audit — 2026-09-05
- Local Mission Control consolidation — 2026-09-05
- Onboarding + Walkthrough hardening plan
- Mission Control 2.2.0
- Mission Control 2.3.0
- 3. Агент №1 — Architect (Claude Opus)
- Release Process
- Mission Control Installer Skill
- [1.3.0] - 2026-03-02
- [2.2.0] - 2026-07-17
- Desktop consolidation
- OpenClaw Config Compatibility Note
- 8. Запустить конвейер
- pull_request_template.md
- check-workflow-action-pins.py
- webhooks.test.ts
- workspaces-tenant-access.test.ts
- job.test.mjs
- LoginPage
- Support
- TODO later: Overview modular board
- Mission Control Wiki
- Mission Control desktop
- activity-timeline-widget.tsx
- GenericAdapter
- 0. Подготовка
- 5. Агент №3 — Linter (Local LMStudio)
- Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?
- Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout
- runtime-types.ts
- MobileBottomSheet
- 2. Project (для тикет-префикса и группировки задач)
- 6. Агент №4 — Aegis (Claude Sonnet, reviewer)
- LangGraphAdapter
- 20260920231000_mission_control_jev_events.sql
- jev-supabase-persistence.md
- doppler-runtime.test.sh
- webhook-response.test.ts
- ServerEventBus
- engineering-bot/identity.md
- engineering-bot/soul.md
- research-bot/identity.md
- research-bot/soul.md
- repository
- @scalar/api-reference-react
- vitest.config.ts
- auth-guards.spec.ts
- delete-body.spec.ts
- adapter-assignment-isolation.test.ts
- createResourceSampler
- limit-caps.spec.ts

## God Nodes (most connected - your core abstractions)
1. `requireRole()` - 451 edges
2. `getDatabase()` - 404 edges
3. `vitest` - 339 edges
4. `next` - 246 edges
5. `logger` - 163 edges
6. `apiFetch()` - 145 edges
7. `react` - 142 edges
8. `denyUnscopedResourceForStrictWorkspace()` - 125 edges
9. `logAuditEvent()` - 121 edges
10. `useMissionControl` - 104 edges

## Surprising Connections (you probably didn't know these)
- `Active-session CLI bridge` --references--> `build()`  [INFERRED]
  docs/fly-agent-workers.md → apps/desktop/scripts/build-app.mjs
- `How It Works` --references--> `status()`  [INFERRED]
  docs/agent-setup.md → apps/desktop/src/desktop-unlock.mjs
- `4. Report Task Progress` --references--> `status()`  [INFERRED]
  SKILL.md → apps/desktop/src/desktop-unlock.mjs
- `What runs where` --references--> `command()`  [INFERRED]
  docs/fly-agent-workers.md → scripts/mc-fly-enroll.py
- `Recommended Wiki Structure` --references--> `Home()`  [INFERRED]
  wiki/Home.md → src/app/[[...panel]]/page.tsx

## Import Cycles
- None detected.

## Communities (350 total, 39 thin omitted)

### Community 0 - "vitest"
Cohesion: 0.04
Nodes (30): ref_node_fs, ref_node_path, vitest, dbRowsByName, partRowsByMessageId, detectBinary(), detectClaude(), detectCodex() (+22 more)

### Community 1 - "widget-primitives.tsx"
Cohesion: 0.09
Nodes (33): DashboardGitHubStats, DashboardSystemStats, ActivityIconMini(), AgentIcon(), CostIcon(), DashboardData, DbStats, formatTokensShort() (+25 more)

### Community 2 - "mac-cleanup/index.ts"
Cohesion: 0.05
Nodes (94): collectProtectedRoots(), leaseRoots(), CATALOG, catalogBinary(), CatalogEntry, main(), capture(), buildFindings() (+86 more)

### Community 3 - "resolveWithin"
Cohesion: 0.24
Nodes (14): ALLOWED_FILES, FILE_ALIASES, GET(), getAgentByIdOrName(), PUT(), resolveAgentWorkspacePath(), GET(), PATCH() (+6 more)

### Community 4 - "auth.ts"
Cohesion: 0.05
Nodes (92): ref_crypto, ensureUniqueUsername(), GET(), makeUsernameFromEmail(), POST(), POST(), POST(), POST() (+84 more)

### Community 5 - "chat-icons.tsx"
Cohesion: 0.09
Nodes (32): IconArtifacts(), IconChevron(), IconClock(), IconCustomize(), IconDispatch(), IconGrip(), IconPin(), IconPlus() (+24 more)

### Community 6 - "task-board-panel.tsx"
Cohesion: 0.08
Nodes (33): Agent, AgentsResponse, COLOR_PALETTE, mutate(), Project, ProjectManagerModal(), ProjectsResponse, Agent (+25 more)

### Community 7 - "next"
Cohesion: 0.05
Nodes (58): next, dynamic, GET(), POST(), GET(), POST(), GET(), GET() (+50 more)

### Community 8 - "security-audit-panel.tsx"
Cohesion: 0.05
Nodes (43): recharts, classLabel(), formatAge(), MacCleanupJobs(), formatValue(), MacCleanupMeters(), tone(), valueClass() (+35 more)

### Community 9 - "useMissionControl"
Cohesion: 0.03
Nodes (104): bootLabelKeys, CapabilitiesResponse, GatewaySummary, Home(), STEP_KEYS, Dashboard(), ChecklistItem, OnboardingChecklistWidget() (+96 more)

### Community 10 - "requireRole"
Cohesion: 0.03
Nodes (122): DELETE(), POST(), GET(), safeParseJson(), ALLOWED_COLORS, dynamic, GET(), loadPrefs() (+114 more)

### Community 11 - "hermes/route.ts"
Cohesion: 0.09
Nodes (33): dataDir, dynamic, extractDeviceAuth(), GET(), HOOK_DIR, POST(), detectHermes(), countSectionEntries() (+25 more)

### Community 12 - "Changelog"
Cohesion: 0.06
Nodes (35): [1.0.0] - 2026-02-15, [1.1.0] - 2026-02-27, [1.2.0] - 2026-03-01, [2.0.0] - 2026-03-11, [2.0.1] - 2026-03-18, [2.1.0] - 2026-07-04, [2.3.0] - 2026-07-25, Added (+27 more)

### Community 13 - "agent-squad-panel-phase3.tsx"
Cohesion: 0.04
Nodes (64): FleetLogosStrip(), ConnectorsTab(), Inventory, ActivityTab(), Agent, AgentCronJob, ChannelAccountInfo, ChannelEntryInfo (+56 more)

### Community 14 - "login/page.tsx"
Cohesion: 0.09
Nodes (21): ConnectionStatus, GATEWAY_URL_PRESETS, GoogleApi, GoogleCredentialResponse, LoginErrorPayload, LoginRequestBody, Window, getInitialProgress() (+13 more)

### Community 15 - "store/index.ts"
Cohesion: 0.04
Nodes (73): ChatInput(), ChatInputProps, ChatPanel(), ChatWorkspace(), ChatWorkspaceProps, asRecord(), COLOR_OPTIONS, ConversationList() (+65 more)

### Community 16 - "main.mjs"
Cohesion: 0.12
Nodes (29): openBackend(), partitionForOrigin(), isLoginPage(), ensureServer(), attachWindow(), createWindow(), failedWindows, locked (+21 more)

### Community 17 - "group-sessions.ts"
Cohesion: 0.11
Nodes (23): ChatFilterPopover(), OpenMenu, IconCheck(), readJson(), useChatDesktopPrefs(), parsePins(), toActivityMs(), isEffortLevel() (+15 more)

### Community 18 - "getDatabase"
Cohesion: 0.05
Nodes (78): GET(), POST(), AlertRule, DELETE(), GET(), POST(), PUT(), DELETE() (+70 more)

### Community 19 - "config"
Cohesion: 0.05
Nodes (71): GET(), GET(), POST(), slugify(), POST(), GET(), POST(), DELETE() (+63 more)

### Community 20 - "adapters/index.ts"
Cohesion: 0.32
Nodes (6): AgentRegistration, Assignment, HeartbeatPayload, queryPendingAssignments(), TaskReport, AutoGenAdapter

### Community 21 - "office-panel.tsx"
Cohesion: 0.05
Nodes (57): buildPath(), buildWalkabilityGrid(), clamp(), easeInOut(), findGridPath(), FlightDeckResponse, formatLastSeen(), getInitials() (+49 more)

### Community 22 - "integrations/route.ts"
Cohesion: 0.11
Nodes (42): ref_os, BLOCKED_PREFIXES, BLOCKED_VARS, BuiltinCategory, CATEGORIES, checkCommandAvailable(), checkOllamaReachable(), checkOpAuthenticated() (+34 more)

### Community 23 - "api/status/route.ts"
Cohesion: 0.16
Nodes (21): GET(), getAvailableModels(), getCapabilities(), getDashboardData(), getDbStats(), getGatewayStatus(), getMemorySnapshot(), getSystemStatus() (+13 more)

### Community 24 - "nav-rail.tsx"
Cohesion: 0.05
Nodes (7): adminOnlyPanels, gatewayOnlyPanels, groupTranslationKeys, NavGroup, navGroups, NavItem, navItemTranslationKeys

### Community 25 - "gateway-config/route.ts"
Cohesion: 0.16
Nodes (18): applyConfig(), computeHash(), gatewayHeaders(), gatewayUrl(), GET(), getConfigPath(), getSchema(), PUT() (+10 more)

### Community 26 - "ref_node_assert"
Cohesion: 0.14
Nodes (21): ensureService(), ensureServices(), healthStatus(), launchctl(), serviceTarget(), startService(), BACKEND, GATEWAY (+13 more)

### Community 27 - "mc-cli.cjs"
Cohesion: 0.16
Nodes (20): bodyFromFlags(), commands, ensureParentDir(), EXIT, fs, handleEventsWatch(), httpRequest(), loadProfile() (+12 more)

### Community 28 - "chat-session-identity.ts"
Cohesion: 0.11
Nodes (36): GitLensSessionRow, conversationsToItems(), gitLensByProject(), GitLensSessionRow, HomeSessionRow, sortNewestFirst(), toGitLensRow(), toHomeSessions() (+28 more)

### Community 29 - "@playwright/test"
Cohesion: 0.07
Nodes (4): @playwright/test, API_KEY_HEADER, stamp, dbPath

### Community 30 - "session-handoff.ts"
Cohesion: 0.13
Nodes (20): POST(), buildHandoffCommand(), buildHandoffPrompt(), claudeCommand(), codexCommand(), CommandInput, errorMessage(), extractCwd() (+12 more)

### Community 31 - "utils.ts"
Cohesion: 0.10
Nodes (13): clsx, tailwind-merge, normalizeModel(), Agent, AgentStatus, ChatMessage, ConnectionState, Conversation (+5 more)

### Community 32 - "security-scan.ts"
Cohesion: 0.08
Nodes (42): AgentScanFixRequest, buildSummary(), FixScope, isFixableInScope(), POST(), getSchedulerStatus(), CheckStatus, darwinAutoUpdatesCheck() (+34 more)

### Community 33 - "chat-session-pane.tsx"
Cohesion: 0.10
Nodes (30): ChatLiveDot(), ChatSessionRow(), ChatSessionPane(), ContextWindowBar(), dismissKey(), HandoffBanner(), looksReal(), transcriptExcerpt() (+22 more)

### Community 34 - "task-dispatch.ts"
Cohesion: 0.08
Nodes (61): Pattern 7: Stale Task Recovery, auditClaudeSessionDispatch(), firstExisting(), FleetCliResult, resolveGrokCliPath(), resolveKimiCliPath(), runCli(), runGrokPrompt() (+53 more)

### Community 35 - "polled.mjs"
Cohesion: 0.17
Nodes (23): packageFailure(), clearGitCredentials(), withGitAuth(), RESULT_RETENTION_SECONDS, retainResult(), workDeadline(), executeJob(), main() (+15 more)

### Community 36 - "engine-logo.tsx"
Cohesion: 0.10
Nodes (35): LLM identity system, messages_en, EngineLogo(), EngineLogoForText(), EngineLogoSet(), LlmLabel(), ModelPicker(), HANDOFF_SEATS (+27 more)

### Community 37 - "mc-fly-enroll.py"
Cohesion: 0.27
Nodes (12): argparse, fcntl, hashlib, os, command(), enroll_key(), github(), main() (+4 more)

### Community 38 - "build-app.mjs"
Cohesion: 0.18
Nodes (22): build(), stage(), validCache(), buildOptions(), resolveElectron(), exists(), installApp(), publishCandidate() (+14 more)

### Community 39 - "jev-setup-session-repository.ts"
Cohesion: 0.14
Nodes (27): boundedInteger(), Context, GET(), canManage(), Context, DELETE(), GET(), PATCH() (+19 more)

### Community 40 - "cost-tracker-panel.tsx"
Cohesion: 0.05
Nodes (35): ErrorBoundary, log, Props, State, AgentsView(), ByAgentEntry, ByAgentModelBreakdown, ByAgentResponse (+27 more)

### Community 41 - "chat-composer.tsx"
Cohesion: 0.10
Nodes (27): ChatComposer(), ComposerChips(), PermissionMenu(), UsageBanner(), IconFolder(), IconSend(), ALIASES, ARGV (+19 more)

### Community 42 - "dashboard.tsx"
Cohesion: 0.16
Nodes (15): buildDashboardView(), DashboardRequest, REQUEST_LABELS, ClaudeStats, getLocalOsStatus(), getMcHealth(), localSessionLogs(), mergeRecentLogs() (+7 more)

### Community 43 - "chat-usage-tracker.ts"
Cohesion: 0.15
Nodes (21): BAR, UsagePopup(), CAPS, extraNeedle(), liveUsageTracker(), modelNeedle(), oldestSince(), sumTokens() (+13 more)

### Community 44 - "cron-management-panel.tsx"
Cohesion: 0.12
Nodes (24): addDays(), AGENT_COLORS, CalendarViewMode, ClaudeCodeTeamsSection(), CronManagementPanel(), DayJobSummary, extractApiErrorMessage(), formatDateLabel() (+16 more)

### Community 45 - "apiFetch"
Cohesion: 0.02
Nodes (117): next-intl, ContentRouter(), ESSENTIAL_PANELS, routedContent(), FeedItem(), formatRelativeTime(), LiveFeed(), ContextSwitcher() (+109 more)

### Community 46 - "proxy.ts"
Cohesion: 0.21
Nodes (16): buildMissionControlCsp(), buildNonceRequestHeaders(), addSecurityHeaders(), config, envFlag(), extractApiKeyFromRequest(), getImplicitAllowedHosts(), getRequestHostCandidates() (+8 more)

### Community 47 - "webhooks.ts"
Cohesion: 0.17
Nodes (15): ref_node_dns, ref_node_https, ref_node_net, BACKOFF_SECONDS, DeliverOpts, deliverWebhook(), DeliveryResult, EVENT_MAP (+7 more)

### Community 48 - "db.ts"
Cohesion: 0.04
Nodes (84): GET(), handleActivitiesRequest(), handleStatsRequest(), GET(), DELETE(), GET(), PUT(), POST() (+76 more)

### Community 49 - "widget-grid.tsx"
Cohesion: 0.10
Nodes (23): SIZE_CLASSES, WIDGET_COMPONENTS, WidgetGrid(), formatBytes(), StatRow(), FleetStatusWidget(), getSessionSparkline(), GithubSignalWidget() (+15 more)

### Community 50 - "package.json"
Cohesion: 0.06
Nodes (34): config, author, description, keywords, license, name, packageManager, version (+26 more)

### Community 51 - "desktop-auth-store.mjs"
Cohesion: 0.16
Nodes (24): configure(), readPin(), createPinRecord(), createPinThrottle(), credentialsFromEnvironment(), derive(), parseEnvironment(), unquote() (+16 more)

### Community 52 - "Production (Docker)"
Cohesion: 0.10
Nodes (16): Automatic backups, Direct API dispatch (gateway-free), Gateway Connectivity from Docker, Host hardening (Ubuntu quick actions), Local Security Scan Expectations (HTTP dev vs HTTPS prod), Mode-aware Make workflow (minimal commands), Persistent Data, Production (Docker) (+8 more)

### Community 53 - "skills/route.ts"
Cohesion: 0.18
Nodes (24): assertRealPathWithin(), auditSkillMutation(), collectSkillsFromDir(), DELETE(), deleteSkill(), dynamic, extractDescription(), GET() (+16 more)

### Community 54 - "websocket.ts"
Cohesion: 0.07
Nodes (29): authTokenRef, clearHandshakeTimers(), connectRef, connectSentRef, formatAge(), GatewayFrame, GatewayMessage, gatewaySupportsPingRef (+21 more)

### Community 55 - "skill-sync.ts"
Cohesion: 0.19
Nodes (14): parseSkillDescription(), getTargetDir(), ALIAS_SKILL_SOURCES, envPath(), listSkillRoots(), LOCAL_SKILL_SOURCES, SkillRoot, skillTargetDir() (+6 more)

### Community 56 - "memory-utils.ts"
Cohesion: 0.05
Nodes (90): GET(), LinkGraph, mergeGraphs(), scanRoots(), withPathPrefix(), POST(), buildFileTree(), DELETE() (+82 more)

### Community 57 - "gateway-config-panel.tsx"
Cohesion: 0.12
Nodes (27): ArrayField(), computeDiff(), deepSet(), defaultValueFor(), DiffEntry, FallbackField(), Feedback, FormMode (+19 more)

### Community 58 - "Runbook: move the worker pool onto an isolated private network"
Cohesion: 0.22
Nodes (7): 1. Create the isolated app, 2. Set the three keys together, 3. Restart on an idle window, 4. Verify, in this order, Rollback, Runbook: move the worker pool onto an isolated private network, observeActive()

### Community 59 - "dependencies"
Cohesion: 0.06
Nodes (32): dependencies, better-sqlite3, class-variance-authority, clsx, eslint, eslint-config-next, next, next-intl (+24 more)

### Community 60 - "scripts"
Cohesion: 0.06
Nodes (32): scripts, api:parity, api:parity:json, artifact:check, artifact:prepare, build, deploy:standalone, desktop:build (+24 more)

### Community 61 - "exec-approvals/route.ts"
Cohesion: 0.20
Nodes (14): allowlistPatternSchema, allowlistUpdateSchema, approvalResponseSchema, computeHash(), execApprovalsPath(), gatewayUrl(), GET(), getAllowlist() (+6 more)

### Community 62 - "injection-guard.ts"
Cohesion: 0.15
Nodes (23): CONFUSABLES, decodeRot13(), detectRot13(), escapeHtml(), generateDecodingVariants(), GuardOptions, InjectionCategory, InjectionMatch (+15 more)

### Community 63 - "github/route.ts"
Cohesion: 0.19
Nodes (17): GET(), handleClose(), handleComment(), handleGitHubStats(), handleInitLabels(), handleStatus(), handleSync(), handleSyncProject() (+9 more)

### Community 64 - "FlyMachinesClient"
Cohesion: 0.14
Nodes (9): Implemented in this change, enabled(), FlyMachineCreateRequest, FlyMachineResponse, FlyMachinesClient, FlyMachinesClientOptions, FlyMachinesError, retryable() (+1 more)

### Community 65 - "dashboard-cli-fleets.ts"
Cohesion: 0.19
Nodes (16): SessionWorkbenchRow(), SessionWorkbenchWidget(), data(), CLI_KIND_META, CLI_SESSION_KINDS, cliKindLabel(), cliKindMeta, CliSessionKind (+8 more)

### Community 66 - "exec-approval-panel.tsx"
Cohesion: 0.21
Nodes (10): AgentAllowlistCard(), AllowlistState, ApprovalCard(), FilterTab, PanelView, RISK_BADGE, RISK_BORDER, timeAgo() (+2 more)

### Community 67 - "channels-panel.tsx"
Cohesion: 0.12
Nodes (24): AccountList(), ActionResult, asRecord(), CardShell(), ChannelAccount, channelIsActive(), ChannelsSnapshot, ChannelStatus (+16 more)

### Community 68 - "chat-display.ts"
Cohesion: 0.14
Nodes (22): asFeed(), ChatHomeList(), feedTime(), HomeSessionRow, KIND_KEY, pillClass(), ChatWelcome(), asActivity() (+14 more)

### Community 69 - "handoff/route.ts"
Cohesion: 0.13
Nodes (26): dynamic, executeHandoff(), HandoffRequest, makeSpec(), parseBody(), parsedHandoff(), readReply(), unlinkQuiet() (+18 more)

### Community 70 - "agent-comms-panel.tsx"
Cohesion: 0.09
Nodes (25): AggregateEvent, ActivitiesData, activitiesToFeed(), ActivityRecord, AGENT_IDENTITY, AgentCommsPanel(), CATEGORY_META, CommsData (+17 more)

### Community 71 - "validation.ts"
Cohesion: 0.03
Nodes (85): DELETE(), GET(), POST(), CachedDoctor, doctorCache, DoctorCacheModule, GET(), getCommandDetail() (+77 more)

### Community 72 - "react"
Cohesion: 0.06
Nodes (48): react, FlySessionStatus(), useChatFlyJobs(), EMPTY, clearLegacyOrder(), readLegacyOrder(), useFolderOrder(), Activity (+40 more)

### Community 73 - "mcp-audit.ts"
Cohesion: 0.14
Nodes (20): GET(), parsePositiveInteger(), logMcpCall(), McpCallInput, McpCallStats, verifyMcpCallReceipt(), verifyMcpCallReceipts(), canonicalize() (+12 more)

### Community 74 - "session-terminal-widget.tsx"
Cohesion: 0.16
Nodes (12): SessionTerminalWidget(), TerminalWidgetData, transcripts, DashboardSession, COLUMN_CLASS, gridTerminalSessions(), TERMINAL_GRID_LIMIT, TERMINAL_GRID_SNAPS (+4 more)

### Community 75 - "openclaw-doctor.ts"
Cohesion: 0.20
Nodes (16): detectCategory(), isDoctorTitleLine(), isInformationalDoctorLine(), stripDoctorGutter(), isDecorativeLine(), isPositiveOrInstructionalLine(), isSessionAgingLine(), isStateDirectoryListLine() (+8 more)

### Community 76 - "mc-tui.cjs"
Cohesion: 0.14
Nodes (27): activityIcon(), ansi, api(), cleanup(), fetchAgentSessions(), fetchDashboardData(), fetchTranscript(), formatNumber() (+19 more)

### Community 77 - "claude-fleet-plans.ts"
Cohesion: 0.31
Nodes (9): CostFleetPlans(), detectClaudeFleetPlans(), CLAUDE_FLEET_PLANS, ClaudeFleetAuthStatus, ClaudeFleetPlanDef, ClaudeFleetPlanIdentity, ClaudeFleetPlanStatus, claudeFleetPlanTotalUsd() (+1 more)

### Community 78 - "gnap-sync.ts"
Cohesion: 0.20
Nodes (21): GET(), POST(), getGnapStatus(), git(), GNAP_TO_MC_STATUS, gnapStatusToMc(), GnapTask, hasChanges() (+13 more)

### Community 79 - "jev-panel.tsx"
Cohesion: 0.04
Nodes (82): Frontend components, @testing-library/react, Answer(), humanize(), JevAnswerCards(), nextAction(), JevClarificationCard(), formatTime() (+74 more)

### Community 80 - "session-list-local.ts"
Cohesion: 0.24
Nodes (19): CLI_SESSION_SCAN_LIMIT, isForeignPrefixedSessionId(), getLocalGrokSessions(), getLocalKimiSessions(), mapEngineRow(), activityFields(), formatAge(), formatTokens() (+11 more)

### Community 81 - "jev-assistant-provider.ts"
Cohesion: 0.09
Nodes (21): ref_node_events, zod, claudeExecutable(), generateJevAssistantDraft(), isJevAssistantAvailable(), JevAssistantProviderError, PROVIDER_ENV_KEYS, providerEnvironment() (+13 more)

### Community 82 - "scheduler.ts"
Cohesion: 0.14
Nodes (28): POST(), syncAgentsFromConfig(), reconcileFlyWorkers(), Result, startFlyReconcileLoop(), Task, syncLocalAgents(), BACKUP_DIR (+20 more)

### Community 83 - "workload/route.ts"
Cohesion: 0.13
Nodes (21): AgentMetrics, buildAgentMetrics(), buildCapacityMetrics(), buildQueueMetrics(), buildThresholds(), CapacityMetrics, computeRecommendation(), escalate() (+13 more)

### Community 84 - "github-sync-engine.ts"
Cohesion: 0.17
Nodes (20): createIssue(), fetchIssue(), GitHubIssue, ALL_MC_LABELS, ALL_PRIORITY_LABEL_NAMES, ALL_STATUS_LABEL_NAMES, FLEET_AGENT_LABELS, LABEL_PRIORITY_MAP (+12 more)

### Community 85 - "layout.tsx"
Cohesion: 0.15
Nodes (16): Fixed in this pass, next-themes, sonner, src_app_globals, inter, jetbrainsMono, metadata, metadataBase (+8 more)

### Community 86 - "start-e2e-server.mjs"
Cohesion: 0.09
Nodes (17): ref_node_process, baseEnv, buildIdPath, children, dataDir, fixtureSource, gatewayPort, mockBinDir (+9 more)

### Community 87 - "gateways/connect/route.ts"
Cohesion: 0.19
Nodes (18): ensureTable(), GatewayEntry, getBrowserHostname(), inferBrowserProtocol(), isNonBrowserReachableHost(), LOCALHOST_HOSTS, POST(), resolveRemoteGatewayUrl() (+10 more)

### Community 88 - "onboarding/route.ts"
Cohesion: 0.27
Nodes (14): GET(), getOnboardingSetting(), ONBOARDING_SETTING_KEYS, OnboardingSettingKey, POST(), readUserOnboardingSetting(), scopedOnboardingKey(), setOnboardingSetting() (+6 more)

### Community 89 - "dashboard-board.ts"
Cohesion: 0.17
Nodes (25): ALLOWED_W, BOARD_COLS, BoardItem, BoardItemKind, BoardState, canPlace(), clampItem(), compactUp() (+17 more)

### Community 90 - "fly-topology.test.ts"
Cohesion: 0.13
Nodes (21): Read this before touching anything, FlySubmission, SubmissionRow, submitFlyLeaf(), FlyDispatchResult, FlyDispatchTask, flyImageApps(), isFlyWorkerImageRef() (+13 more)

### Community 91 - "install.ps1"
Cohesion: 0.24
Nodes (19): Get-RandomHex(), Get-RandomPassword(), Get-Source(), New-EnvFile(), Deploy-Docker(), Deploy-Local(), Main(), Stop-WithError() (+11 more)

### Community 92 - "mc-provisioner-daemon.js"
Cohesion: 0.10
Nodes (25): {
  COMMAND_TIMEOUT_MS,
  IDLE_SOCKET_TIMEOUT_MS,
  MAX_CONNECTIONS,
  MAX_OUTPUT_BYTES,
  MAX_REQUEST_BYTES,
  appendBounded,
}, escapeRegExp(), { execFileSync, spawn }, fs, GATEWAY_SYSTEMD_TEMPLATE, isSafeHomePath(), isSafeUser(), net (+17 more)

### Community 93 - "src/index.ts"
Cohesion: 0.10
Nodes (20): zustand, Activity, Agent, ChatMessage, Comment, ConnectionStatus, Conversation, CronJob (+12 more)

### Community 94 - "skill-registry.ts"
Cohesion: 0.12
Nodes (28): dynamic, PUT(), VALID_SOURCES, VALID_TARGETS, checkSkillSecurity(), fetchAwesomeIndex(), fetchAwesomeOpenclawSkill(), fetchClawdHubSkill() (+20 more)

### Community 95 - "graph/route.ts"
Cohesion: 0.16
Nodes (20): AgentFileInfo, AgentGraphData, GET(), getAgentData(), execDoctor(), execFileAsync, HealthCategory, parseWikiDoctorOutput() (+12 more)

### Community 96 - "fetchWithRetry"
Cohesion: 0.16
Nodes (15): compareSemver(), GET(), headers, compareSemver(), GET(), canRetryMethod(), FetchRetryPolicy, fetchWithRetry() (+7 more)

### Community 97 - "jev-assistant-service.ts"
Cohesion: 0.22
Nodes (15): sanitizeForPrompt(), JevAssistantRequest, buildJevAssistantPrompt(), configurationFromRequest(), containsInjection(), createJevAssistantDraft(), defaults, protectProviderValue() (+7 more)

### Community 98 - "super-admin.ts"
Cohesion: 0.16
Nodes (19): GET(), POST(), ProvisionJob, Tenant, buildBootstrapPlan(), createTenantAndBootstrapJob(), ensurePort(), getTenantHomeRoot() (+11 more)

### Community 99 - "fly-admission-schema.ts"
Cohesion: 0.20
Nodes (12): flyNumber(), flyReadiness(), flySubmissionSchema, flyBudgetPeriods(), flyRepositoryAuth(), registryCredential(), SshCredential, buildFlyTelemetry() (+4 more)

### Community 100 - "desktop/package.json"
Cohesion: 0.10
Nodes (18): description, devDependencies, electron, engines, node, main, name, packageManager (+10 more)

### Community 101 - "agent-runtimes.ts"
Cohesion: 0.07
Nodes (55): GET(), POST(), VALID_MODES, VALID_RUNTIMES, checkToolExists(), discoverOsUsers(), GET(), installToolForUser() (+47 more)

### Community 102 - "Jev Workspace Product Requirements"
Cohesion: 0.17
Nodes (11): Backend components, Deferred batch scope, Information architecture, Jev Workspace Product Requirements, Outcome, Primary flow, Question behavior, Repository context (+3 more)

### Community 103 - "grok-sessions.ts"
Cohesion: 0.39
Nodes (8): asNumber(), asObject(), asString(), clampTimestamp(), GrokSessionStats, listSummaryFiles(), parseSummary(), scanGrokSessions()

### Community 104 - "local-session-list.ts"
Cohesion: 0.30
Nodes (12): flagsFor(), formatAge(), formatTokens(), fromClaudeRow(), fromScanner(), listNormalizedLocalSessions(), formatAge(), formatTokens() (+4 more)

### Community 105 - "Mission Control Style Guide"
Cohesion: 0.07
Nodes (29): 10. Accessibility, 1. Design Philosophy, 2. Color System, 3. Typography, 4. Spacing, 5. Border Radius, 6. Elevation & Glow, 7. Component Patterns (+21 more)

### Community 106 - "schema.sql"
Cohesion: 0.13
Nodes (27): activities, agents, comments, gateway_health_logs, idx_activities_created_at, idx_activities_type, idx_agents_session_key, idx_agents_status (+19 more)

### Community 107 - "install.sh"
Cohesion: 0.33
Nodes (17): check_openclaw(), check_prerequisites(), command_exists(), detect_os(), die(), err(), fetch_source(), info() (+9 more)

### Community 108 - "runCommand"
Cohesion: 0.22
Nodes (13): cpuTotals(), GET(), getCpuSnapshot(), getDiskSnapshot(), getGpuSnapshot(), getMemorySnapshot(), getNetworkSnapshot(), getProcessSnapshot() (+5 more)

### Community 109 - "recurring-tasks.ts"
Cohesion: 0.18
Nodes (13): GET(), formatDateSuffix(), isSubDailyCron(), MONTHS, RecurrenceMetadata, spawnRecurringTasks(), DAY_MAP, isCronDue() (+5 more)

### Community 110 - "ref_node_os"
Cohesion: 0.08
Nodes (17): ref_node_crypto, ref_node_os, report, result, agentInboxDir(), writeAgentInbox(), defaultDataDir, defaultMemoryDir (+9 more)

### Community 111 - "gateways/health/route.ts"
Cohesion: 0.24
Nodes (12): BLOCKED_HOSTNAMES, BLOCKED_PRIVATE_CIDRS, buildGatewayProbeUrl(), ensureGatewaysTable(), GatewayEntry, hasOpenClaw32ToolsProfileRisk(), HealthResult, ipv4InCidr() (+4 more)

### Community 112 - "api-contract-parity.ts"
Cohesion: 0.20
Nodes (17): collectOpenApiOperations(), collectRouteOperations(), compareApiContractParity(), ContractOperation, extractHttpMethods(), HTTP_METHODS, loadOpenApiFile(), normalizeOperation() (+9 more)

### Community 113 - "cli-inventory.ts"
Cohesion: 0.22
Nodes (17): GET(), AgentInventory, inventoryForAgent(), inventoryLooksSafe(), runtimeOf(), jsonKeys(), listClaudeMcp(), listCodexAutomations() (+9 more)

### Community 114 - "device-identity.ts"
Cohesion: 0.20
Nodes (18): cacheDeviceToken(), clearDeviceIdentity(), createNewIdentity(), DeviceIdentity, fromBase64Url(), getCachedDeviceToken(), getOrCreateDeviceIdentity(), idbDeleteKey() (+10 more)

### Community 115 - "compilerOptions"
Cohesion: 0.11
Nodes (18): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+10 more)

### Community 116 - "markdown-renderer.tsx"
Cohesion: 0.31
Nodes (7): react-dom, react-markdown, remark-gfm, getPreviewContent(), MarkdownRenderer(), MarkdownRendererProps, render()

### Community 117 - "devDependencies"
Cohesion: 0.11
Nodes (18): devDependencies, fast-check, jsdom, pino-pretty, @playwright/test, @testing-library/dom, @testing-library/jest-dom, @testing-library/react (+10 more)

### Community 118 - "framework-templates.ts"
Cohesion: 0.24
Nodes (15): dynamic, GET(), listAdapters(), AGENT_TEMPLATES, AgentTemplate, FRAMEWORK_REGISTRY, FrameworkConnectionConfig, FrameworkInfo (+7 more)

### Community 119 - "memory-browser-panel.tsx"
Cohesion: 0.08
Nodes (25): reagraph, countFiles(), fileIcon(), formatFileSize(), HealthCategory, HealthReport, HealthView(), log (+17 more)

### Community 120 - "agent-templates.ts"
Cohesion: 0.12
Nodes (16): AgentIdentityConfig, AgentMemorySearchConfig, AgentModelConfig, AgentSandboxConfig, AgentSubagentsConfig, AgentToolsConfig, COMMON_DENY, getEffectiveToolGroups() (+8 more)

### Community 121 - "flight-deck/route.ts"
Cohesion: 0.40
Nodes (9): DEFAULT_INSTALL_PATHS, dynamic, GET(), getConfiguredFlightDeckPath(), getFlightDeckBaseUrl(), getFlightDeckLaunchUrl(), isInstalled(), POST() (+1 more)

### Community 122 - "openclaw-doctor-banner.tsx"
Cohesion: 0.23
Nodes (8): BannerState, OpenClawDoctorBanner(), OpenClawDoctorFixProgress, OpenClawDoctorStatus, cleanLine(), comparisonKey(), DoctorBannerCopy, prepareDoctorBannerCopy()

### Community 123 - "runs.ts"
Cohesion: 0.09
Nodes (31): GET(), GET(), POST(), PUT(), GET(), GET(), PATCH(), AgentRun (+23 more)

### Community 124 - "pty-websocket.ts"
Cohesion: 0.17
Nodes (16): ref_http, http, path, PORT, server, standaloneDir, createPtySession(), getPtySession() (+8 more)

### Community 125 - "jev-cloud.test.ts"
Cohesion: 0.14
Nodes (22): canonicalJson(), hashJevCloudPayload(), Db, getJevCloudQueueStatus(), JevCloudEvent, jsonFields, prepareJevCloudPayload(), sources (+14 more)

### Community 126 - "mc-mcp-server.cjs"
Cohesion: 0.15
Nodes (17): ref_node_readline, normalizeMissionControlBaseUrl(), api(), CAPABILITIES, fs, handleMessage(), loadConfig(), main() (+9 more)

### Community 127 - "skills-panel.tsx"
Cohesion: 0.08
Nodes (29): deviceAction(), DeviceTokenSummary, InstancesTab(), NodesPanel(), PairedDevice, PairedDevicesSection(), handleRevokeToken(), handleRotateToken() (+21 more)

### Community 128 - "opencode-sessions.ts"
Cohesion: 0.20
Nodes (16): detectOpenCode(), asNumber(), asObject(), asString(), getOpenCodeBinaryCandidates(), getOpenCodeExecutable(), getOpenCodeVersion(), isOpenCodeInstalled() (+8 more)

### Community 129 - "pty/setup/route.ts"
Cohesion: 0.17
Nodes (14): GET(), POST(), EXEC_OPTIONS, GET(), log, POST(), isTmuxAvailable(), listPtySessions() (+6 more)

### Community 130 - "fly-admission.test.ts"
Cohesion: 0.22
Nodes (9): parseResult(), polledResultSchema, readPolledResult(), recordPolledResult(), settleFlyJob(), expireFlyQueue(), releaseQueuedFlySubmission(), pollJob() (+1 more)

### Community 131 - "session-transcript-types.ts"
Cohesion: 0.27
Nodes (14): asObject(), findSessionDir(), isoFromUnknown(), readGrokTranscript(), epochMsToIso(), getOpenCodeDbCandidates(), fallbackParts(), mapRole() (+6 more)

### Community 132 - "claude-transcript.ts"
Cohesion: 0.23
Nodes (14): findClaudeTranscriptFile(), keepLastArtifact(), parseClaudeTranscriptLines(), parseUser(), assistantParts(), attachToolResults(), dedupeConsecutiveText(), rec() (+6 more)

### Community 133 - "claude-code-sessions.ts"
Cohesion: 0.28
Nodes (9): AgentSessionRow, assertSessionOwnedByAgent(), ClaudeBaseSession, ensureClaudeBaseSession(), isValidClaudeSessionId(), markClaudeBaseSessionMaterialized(), readAgentSessionRow(), rotateClaudeBaseSession() (+1 more)

### Community 134 - "ref_node_module"
Cohesion: 0.13
Nodes (9): ref_node_module, {
  MAX_BASE_URL_LENGTH,
  normalizeMissionControlBaseUrl,
}, require, require, config, require, {
  COMMAND_TIMEOUT_MS,
  MAX_OUTPUT_BYTES,
  appendBounded,
}, require (+1 more)

### Community 135 - "webhook-panel.tsx"
Cohesion: 0.17
Nodes (15): AVAILABLE_EVENTS, CreateWebhookForm(), Delivery, isWebhookTransportFailure(), SchedulerResult, SchedulerTask, Webhook, webhookErrorPayload() (+7 more)

### Community 136 - "session-transcript-route.ts"
Cohesion: 0.21
Nodes (14): dynamic, GET(), epochSecondsToISO(), HermesMessageRow, pushToolCalls(), readHermesTranscript(), readHermesTranscriptFromDbPath(), toolName() (+6 more)

### Community 137 - "renderDashboard"
Cohesion: 0.42
Nodes (12): getTermSize(), pad(), priorityColor(), renderAgentDetail(), renderAgentsList(), renderDashboard(), renderTaskDetail(), renderTasksList() (+4 more)

### Community 138 - "session-archive.ts"
Cohesion: 0.18
Nodes (21): projectSlugOf(), archiveListedSessions(), archivePath(), archiveSessionMeta(), archiveSessionTranscript(), ArchivedSessionRow, ensureSessionTranscriptsTable(), indexSessionArchives() (+13 more)

### Community 139 - "chat-desktop-workspace.tsx"
Cohesion: 0.10
Nodes (28): plan(), ChatDesktopWorkspace(), IconClose(), ChatShell(), HandleProps, useRailResize(), SessionPlanPanel(), useChatUsage() (+20 more)

### Community 140 - "github-pulls-api.ts"
Cohesion: 0.21
Nodes (14): ChatGitHubItem, fetchPullRequests(), src_lib_github_getgithubtoken, emptyFeed(), handleGitHubPulls(), projectRepos(), collectPullRequests(), mapPull() (+6 more)

### Community 141 - "fly-reservations.ts"
Cohesion: 0.25
Nodes (11): flyCommittedSpend(), flyFairOrder(), FlyQueuedRow, flyQueueIsContended(), flyRepositoryDailyCap(), flyRepositorySpendToday(), ratio(), repositoryOf() (+3 more)

### Community 142 - "pty-manager.ts"
Cohesion: 0.15
Nodes (10): ref_child_process, node-pty, shutdown(), disposeAllPtySessions(), disposePtySession(), log, PtyClient, ptyPool (+2 more)

### Community 143 - "assistant/route.ts"
Cohesion: 0.22
Nodes (8): POST(), jevAssistantLimiter, error(), exceedsJsonDepth(), validateJevAssistantRequest(), bulk(), mocks, questions

### Community 144 - "mc-fly-contract.test.cjs"
Cohesion: 0.30
Nodes (11): retryRead(), assert, { createFlyTools }, { test }, { validateAdmission, validateStatus, validateCancellation, retryRead }, validateAdmission(), validateCancellation(), validateStatus() (+3 more)

### Community 145 - "agent-filesystem-isolation.test.ts"
Cohesion: 0.16
Nodes (12): assertStatus(), baseUrl, call(), headers, run(), authUser(), denyUnscopedResourceMock, getAgentWorkspaceCandidatesMock (+4 more)

### Community 146 - "tokens/route.ts"
Cohesion: 0.05
Nodes (65): AgentBreakdown, AgentBreakdownRow, GET(), ModelBreakdown, calculateStats(), DbTokenUsageRow, dedupeTokenRecords(), deriveFromSessions() (+57 more)

### Community 147 - "Troubleshooting"
Cohesion: 0.08
Nodes (25): Agent Registration, Architecture, AUTH_PASS with "#" is not working, Database locked errors, Deployment Guide, Docker: gateway unreachable / WebSocket not connecting, Environment Variables, "Gateway error: device identity required" (+17 more)

### Community 148 - "agent-sync.ts"
Cohesion: 0.09
Nodes (36): ref_fs, ref_path, GET(), deepMerge(), enrichAgentConfigFromWorkspace(), getConfigPath(), mapAgentToMC(), normalizeAgentConfigForOpenClaw() (+28 more)

### Community 149 - "coordinator-routing.ts"
Cohesion: 0.35
Nodes (11): CoordinatorAgentRecord, findSessionForAgent(), getConfigIsDefault(), getConfigOpenClawId(), normalizeName(), normalizeOpenClawId(), parseConfig(), resolveConfiguredCoordinatorTarget() (+3 more)

### Community 150 - "local-agent-sync.ts"
Cohesion: 0.21
Nodes (13): AgentFrontmatter, AgentRow, ALL_MARKERS, CONFIG_FILES, DiskAgent, extractRole(), getLocalAgentRoots(), IDENTITY_FILES (+5 more)

### Community 151 - "mc-fly-cli.test.cjs"
Cohesion: 0.17
Nodes (10): assert, fs, os, path, { runFlyCli }, test, fs, { handleMessage } (+2 more)

### Community 152 - "notification-daemon.sh"
Cohesion: 0.40
Nodes (12): check_daemon(), check_mission_control(), cleanup(), deliver_notifications(), get_delivery_stats(), log(), main(), parse_args() (+4 more)

### Community 153 - "adapter-compliance.test.ts"
Cohesion: 0.18
Nodes (10): getAdapter(), mockBroadcast, mockQuery, simulateAdapterAction(), ALL_FRAMEWORKS, mockBroadcast, mockQuery, testAgent (+2 more)

### Community 154 - "[id]/diagnostics/route.ts"
Cohesion: 0.27
Nodes (12): ALLOWED_SECTIONS, buildActivityBreakdown(), buildErrorAnalysis(), buildSummary(), buildTaskMetrics(), buildTokenMetrics(), buildTrendAlerts(), buildTrends() (+4 more)

### Community 155 - "session-continue-run.ts"
Cohesion: 0.29
Nodes (13): permissionArgv(), ContinueBusyError, ContinueKind, HostMode, runClaude(), runCodex(), runSessionContinue(), sessionJsonlMtime() (+5 more)

### Community 156 - "event-bus.ts"
Cohesion: 0.07
Nodes (37): ref_events, dynamic, POST(), VALID_ROLES, dynamic, GET(), runtime, advanceRun() (+29 more)

### Community 157 - "message-bubble.tsx"
Cohesion: 0.25
Nodes (11): AGENT_COLORS, asRecord(), formatTime(), getAgentTheme(), MessageBubble(), MessageBubbleProps, renderContent(), ToolCallBubble() (+3 more)

### Community 158 - "chat-fly-jobs.ts"
Cohesion: 0.36
Nodes (6): ChatSessionFlyJobs(), flyJobsComplete(), jobsForSession(), SessionFlyJob, sessionMatchesFly(), withSessionFlyJobs()

### Community 159 - "websocket-utils.ts"
Cohesion: 0.29
Nodes (12): buildProtocolNegotiation(), calculateBackoff(), calculateReconnectDelay(), ConnectErrorDetailCodes, detectSequenceGap(), GatewayErrorDetail, isNonRetryableErrorCode(), NON_RETRYABLE_ERROR_CODES (+4 more)

### Community 160 - "alert-evaluate.ts"
Cohesion: 0.31
Nodes (10): AlertRule, compareValue(), countThreshold(), evaluateAgentRule(), evaluateAllRules(), evaluateCountRule(), evaluateRule(), SAFE_COLUMNS (+2 more)

### Community 161 - "security-scan-card.tsx"
Cohesion: 0.07
Nodes (30): ClaudeSetup(), CodexSetup(), HermesSetup(), OpenClawSetup(), OpenCodeSetup(), RuntimeSetupModal(), RuntimeSetupModalProps, Category (+22 more)

### Community 162 - "runtime-history.ts"
Cohesion: 0.15
Nodes (20): asNumber(), asObject(), asString(), clampTimestamp(), CodexSessionStats, deriveSessionId(), extractCodexUserText(), listRecentCodexSessionFiles() (+12 more)

### Community 163 - "kimi-sessions.ts"
Cohesion: 0.21
Nodes (19): asObject(), asString(), clampTimestamp(), isKimiClawPath(), KimiSessionStats, parseIndexLine(), parseState(), scanKimiSessions() (+11 more)

### Community 164 - "package-manager-patches/package.json"
Cohesion: 0.17
Nodes (11): brace-expansion, ip-address, tar, dependencies, brace-expansion, ip-address, tar, description (+3 more)

### Community 165 - "jev-context.ts"
Cohesion: 0.31
Nodes (12): buildJevRepositoryContext(), git(), inside(), languageSummary(), projectRoot(), ProjectRow, readBoundedText(), redactContext() (+4 more)

### Community 166 - "fleet-agents.ts"
Cohesion: 0.23
Nodes (14): Recovery archive, 2026-09-07, Restoring from the archive, The two entries that are not simply superseded, What the archive holds, asFleetAgentName(), canonicalFleetAgentName(), FLEET_AGENT_ALIASES, fleetAgentFromWorkspace() (+6 more)

### Community 167 - "Orchestration Patterns"
Cohesion: 0.08
Nodes (24): API, Capacity Control, CLI, Combining Patterns, Event Streaming, Example: Research → Implement → Review, Flow, How Aegis Reviews (+16 more)

### Community 168 - "dashboard-board-persist.ts"
Cohesion: 0.24
Nodes (13): emptyBoard(), BOARD_SETTING_KEY, BOARD_STORAGE_KEY, emptyOr(), isItem(), LEGACY_LAYOUT_KEY, MAX_BOARD_BYTES, parseBoardJson() (+5 more)

### Community 169 - "hook-profiles.ts"
Cohesion: 0.30
Nodes (10): getActiveProfile(), getRateLimitMultiplier(), HookProfile, HookProfileLevel, PROFILES, shouldAuditMcpCalls(), shouldBlockOnSecretDetection(), shouldScanSecrets() (+2 more)

### Community 171 - "task-dispatch-sandbox.test.ts"
Cohesion: 0.16
Nodes (14): clampCliMaxBudgetUsd(), CLAUDE_CLI_ALLOWED_TOOL_NAMES, CLI_MAX_BUDGET_USD_CEILING, filterCliAllowedTools(), resolveCliDispatchCwd(), resolveCliSandboxOptions(), safeParseMetadata(), base (+6 more)

### Community 172 - "check-api-contract-parity.mjs"
Cohesion: 0.35
Nodes (10): extractHttpMethods(), HTTP_METHODS, normalizeOperation(), normalizeSegment(), parseArgs(), parseIgnoreArg(), routeFileToApiPath(), run() (+2 more)

### Community 173 - "api/health/route.ts"
Cohesion: 0.27
Nodes (6): GET(), GET, STANDALONE_BRAND_LOGO, standaloneReleaseIntact(), { getDatabaseMock, standaloneReleaseIntactMock }, dirs

### Community 174 - "fix/route.ts"
Cohesion: 0.11
Nodes (29): fixRequestSchema, FixResult, getFailingChecks(), getRequestHostCandidates(), normalizeHostname(), parseForwardedHost(), POST(), readEnv() (+21 more)

### Community 175 - "channels/route.ts"
Cohesion: 0.20
Nodes (16): commandStdout(), errorMessage(), gatewayHeaders(), GET(), isGatewayReachable(), loadChannelsViaCli(), POST(), asRecord() (+8 more)

### Community 176 - "Everything Included In The v2 Update"
Cohesion: 0.08
Nodes (24): Agent lifecycle and operator UX, Agents, sessions, chat, and comms, Broader agent observability, Core platform and mode handling, Deployment and runtime hardening, Detailed Fix Areas, Detailed Functional Additions, Dual-mode operation (+16 more)

### Community 177 - "codex-transcript.ts"
Cohesion: 0.26
Nodes (11): contentParts(), findCodexTranscriptFile(), parseCodexTranscriptLines(), readCodexTranscript(), rec(), DEFAULT_TAIL_BYTES, findNamedJsonl(), isSessionFile() (+3 more)

### Community 178 - "unlock-renderer.mjs"
Cohesion: 0.22
Nodes (9): accountButton, message, messages, pinForm, pinInput, separator, setBusy(), touchButton (+1 more)

### Community 179 - "mcp-server.spec.ts"
Cohesion: 0.24
Nodes (9): Next steps, ref_node_util, CLI, execFileAsync, mc(), MCP, mcpCall(), mcpRequest() (+1 more)

### Community 180 - "provision-jobs/[id]/route.ts"
Cohesion: 0.33
Nodes (8): GET(), POST(), appendProvisionEvent(), buildDecommissionPlan(), createTenantDecommissionJob(), getProvisionJob(), ProvisionJobAction, transitionProvisionJobStatus()

### Community 181 - "TranscriptMessage"
Cohesion: 0.17
Nodes (13): allowedFile(), dynamic, GET(), SessionArtifactPanel(), useSessionArtifact(), SESSION_ID_RE, extractSessionArtifacts(), latestArtifact() (+5 more)

### Community 182 - "jev-repository.ts"
Cohesion: 0.14
Nodes (32): GET(), POST(), POST(), Context, DELETE(), PATCH(), GET(), POST() (+24 more)

### Community 183 - "session-terminal-cell.tsx"
Cohesion: 0.23
Nodes (17): SessionTerminalCell(), terminalCellTitle(), transcriptSubject(), normalizeCliKind(), isAgentWorking(), orderTerminalSessions(), recency(), TERMINAL_TAB_LIMIT (+9 more)

### Community 184 - "better-sqlite3"
Cohesion: 0.22
Nodes (9): better-sqlite3, FLY_LAUNCH_BATCH_SIZE, FLY_RECONCILE_INTERVAL_MS, flyCapacity(), flyLaunchRegions(), flyWorkerLimit(), launchMachine(), acquireFlySchedulerLease() (+1 more)

### Community 185 - "ref_node_child_process"
Cohesion: 0.26
Nodes (8): ref_node_child_process, detectOpenClaw(), FleetProjectSpec, expandHome(), gitCount(), inspectRepo(), seedRepoHealthTasks(), isPortOpenSync()

### Community 187 - "agent-heartbeat.sh"
Cohesion: 0.50
Nodes (8): check_agent_heartbeat(), check_mission_control(), get_agent_session_key(), get_agents_to_check(), log(), main(), send_wake_notification(), agent-heartbeat.sh script

### Community 188 - "health-utils.test.ts"
Cohesion: 0.28
Nodes (5): BLOCKED_HOSTNAMES, BLOCKED_PRIVATE_CIDRS, ipv4InCidr(), ipv4ToNum(), isBlockedUrl()

### Community 189 - "fleet-projects.ts"
Cohesion: 0.18
Nodes (13): SessionFilterBar(), SessionFilterState, FLEET_AGENT_NAMES, fleetProjectSlug(), matchFleetProject(), PATH_ALIASES, specByName(), tildePath() (+5 more)

### Community 190 - "attention-detector.ts"
Cohesion: 0.22
Nodes (5): ATTENTION_PATTERNS, AttentionLevel, DetectionResult, ERROR_PATTERNS, lastDetection

### Community 191 - "[id]/memory/route.ts"
Cohesion: 0.44
Nodes (10): agentColumnName(), agentColumnValue(), DELETE(), GET(), getAgentByIdOrName(), PUT(), requireAgentSelfAccess(), prepareMock (+2 more)

### Community 192 - "openclaw-gateway.ts"
Cohesion: 0.21
Nodes (10): ws, CallGatewayOptions, formatGatewayError(), GATEWAY_SCOPES, GatewayFrame, isUnknownMethodError(), parseGatewayJsonOutput(), mocks (+2 more)

### Community 193 - "claude-sessions.ts"
Cohesion: 0.15
Nodes (15): ref_readline, claudeConfigDirForAgent(), claudeSessionHomes(), clampTimestamp(), DEFAULT_PRICING, JSONLEntry, lastSyncResult, MAX_SESSION_FILE_BYTES (+7 more)

### Community 194 - "github.ts"
Cohesion: 0.26
Nodes (11): createLabel(), createPullRequest(), createRef(), ensureLabels(), getRef(), githubFetch(), GitHubLabel, GitHubPullRequest (+3 more)

### Community 195 - "check-standalone-artifact.mjs"
Cohesion: 0.18
Nodes (9): Mission Control desktop, allowedRoots, allowedSourceFiles, failures, forbiddenNames, requiredFiles, root, walk() (+1 more)

### Community 196 - "deploy-standalone.sh"
Cohesion: 0.46
Nodes (7): list_listener_pids(), load_env(), migrate_runtime_data_dir(), deploy-standalone.sh script, stop_existing_server(), stop_pid(), use_project_node()

### Community 197 - "session-terminal-view.tsx"
Cohesion: 0.23
Nodes (7): LINE_CLASS, SessionTerminalView(), TerminalLine, TerminalLineKind, Harness(), TailScroll, useTailScroll()

### Community 198 - "Mission Control Hardening"
Cohesion: 0.17
Nodes (12): 10. Data Retention, 11. Runtime Installer Integrity, 1. Credentials, 2. Network Access Control, 3. HTTPS & Cookies, 4. Rate Limiting, 5. Docker Hardening, 6. Security Headers (+4 more)

### Community 199 - "task-routing.ts"
Cohesion: 0.36
Nodes (6): isNonEmptyString(), parseMetadata(), resolveTaskImplementationTarget(), TaskImplementationTarget, TaskLike, TaskMetadata

### Community 200 - "chat-permission-mode-route.test.ts"
Cohesion: 0.25
Nodes (6): auditMock, isolationMock, limiterMock, loadMock, requireRoleMock, saveMock

### Community 202 - "handoff-brief.ts"
Cohesion: 0.38
Nodes (8): buildHandoffBrief(), HANDOFF_BRIEF_MAX, prsOf(), textOf(), toolsOf(), filesTouched(), redactSecrets(), clipExcerpt()

### Community 203 - "helpers.ts"
Cohesion: 0.15
Nodes (11): createTestAlert(), createTestProject(), createTestUser(), createTestWebhook(), createTestWorkflow(), deleteTestAlert(), deleteTestProject(), deleteTestUser() (+3 more)

### Community 204 - "typesafe-probe.ts"
Cohesion: 0.27
Nodes (6): getJevHealth(), JevHealth, mocks, hasProbeAnswer(), probeTypeSafeApiKey(), TypeSafeProbeResult

### Community 205 - "security-audit.sh"
Cohesion: 0.52
Nodes (6): fail(), info(), pass(), security-audit.sh script, trim_env_value(), warn()

### Community 206 - "github-activity.ts"
Cohesion: 0.15
Nodes (20): asEvent(), collectGitHubActivity(), fetchRepoEvents(), firstCommitMessage(), GitHubEventLike, GitHubFeedKind, keepNewer(), loadEvents() (+12 more)

### Community 207 - "Mission Control"
Cohesion: 0.10
Nodes (21): Agents and runtimes, Architecture, CLI, Connect an agent, Develop, Documentation, License, MCP server (+13 more)

### Community 208 - "jev-service.ts"
Cohesion: 0.12
Nodes (25): createClient(), evaluateWithJev(), exactKeys(), JEV_DEFAULT_MODEL, JEV_SDK_VERSION, JevClientError, JevClientOptions, listJevModels() (+17 more)

### Community 209 - "ClaudeSdkAdapter"
Cohesion: 0.18
Nodes (3): ClaudeSdkAdapter, adapters, OpenClawAdapter

### Community 210 - "adaptive-context-handoff.ts"
Cohesion: 0.36
Nodes (9): adaptiveContextCli(), handoffEnv(), ledgerFromSession(), parseAdaptiveHandoff(), pinAdaptiveContext(), resolveWorkspacePolicyPath(), writeLedger(), dirs (+1 more)

### Community 211 - "buildGatewayWebSocketUrl"
Cohesion: 0.44
Nodes (7): buildGatewayPathFallbackUrls(), buildGatewayWebSocketUrl(), formatWebSocketUrl(), isLocalHost(), normalizeGatewayPath(), normalizeProtocol(), preserveTokenQuery()

### Community 212 - "aggregate/route.ts"
Cohesion: 0.19
Nodes (16): dynamic, GET(), partToEvent(), buildDeferredCompletionMarkers(), findAssistantTextAfterTaskPrompt(), getTranscriptText(), normalizeGatewayIdentifier(), recoverDeferredCompletionTextFromTranscript() (+8 more)

### Community 213 - "migrations.ts"
Cohesion: 0.09
Nodes (24): hasTable(), NAME_COLUMNS, renameAgentTable(), renameClaudeFleetAgentRows(), renameLooseColumns(), renameProjectAssignments(), RENAMES, rewriteAgentConfig() (+16 more)

### Community 214 - "generate-env.sh"
Cohesion: 0.53
Nodes (4): generate_hex(), generate_password(), portable_sed(), generate-env.sh script

### Community 215 - "mac-safe-reclaim"
Cohesion: 0.67
Nodes (5): mac-safe-reclaim script, busy(), clear_dir(), is_protected(), protected_from_lsof()

### Community 216 - "start-standalone.sh"
Cohesion: 0.40
Nodes (5): HOSTNAME, MC_PROCESS_NAME, MISSION_CONTROL_DATA_DIR, reap_previous_controller(), start-standalone.sh script

### Community 217 - "station-doctor.sh"
Cohesion: 0.60
Nodes (5): fail(), info(), pass(), station-doctor.sh script, warn()

### Community 218 - "index/route.ts"
Cohesion: 0.33
Nodes (4): Endpoint, endpoints, payload, revalidate

### Community 219 - "Mission Control Platform Hardening + Full Agent CLI/TUI PRD"
Cohesion: 0.11
Nodes (18): 10) Rollout plan, 11) Risks and mitigations, 12) Acceptance criteria, 13) Immediate implementation tasks (next 1-2 PRs), 1) Problem statement, 2) Product objectives, 3) Personas and workflows, 4) Functional requirements (+10 more)

### Community 220 - "Mission Control Agent Skill"
Cohesion: 0.11
Nodes (18): 1. Register, 2. Heartbeat, 3. Fetch Assignments, 4. Report Task Progress, 5. Disconnect, Adapters — `/api/adapters`, Agent Lifecycle, Agents — `/api/agents` (+10 more)

### Community 221 - "openclaw-doctor-fix.ts"
Cohesion: 0.36
Nodes (6): archiveOrphanTranscriptsForStateDir(), collectReferencedTranscriptNames(), formatArchiveTimestamp(), isPrimaryTranscriptFile(), OpenClawDoctorFixResult, tempDirs

### Community 222 - "minimax.ts"
Cohesion: 0.36
Nodes (6): getMiniMaxApiKey(), MINIMAX_REGIONS, MiniMaxEnvironment, MiniMaxProtocol, MiniMaxRegion, resolveMiniMaxEndpoint()

### Community 223 - "attribution/route.ts"
Cohesion: 0.40
Nodes (9): ALLOWED_SECTIONS, buildAuditTrail(), buildCostAttribution(), buildIdentity(), buildMutations(), GET(), parseHours(), parseSections() (+1 more)

### Community 224 - "jev-validation.ts"
Cohesion: 0.09
Nodes (24): fast-check, @typesafe-ai/sdk, contextModeSchema, enforcementSchema, failureModeSchema, jevPolicyConfigurationSchema, rolloutSchema, scopeSchema (+16 more)

### Community 225 - "provisioner-client.ts"
Cohesion: 0.40
Nodes (5): ref_net, callProvisioner(), ProvisionerRequest, ProvisionerResponse, runProvisionerCommand()

### Community 226 - "process-title.test.sh"
Cohesion: 0.60
Nodes (3): fail(), process-title.test.sh script, spills()

### Community 227 - "Agent Setup Guide"
Cohesion: 0.10
Nodes (20): Agent Configuration, Agent Registration, Agent Setup Guide, Agent Sources, Agent Status Values, Agent Templates, Example: Developer Agent, Example: Researcher Agent (+12 more)

### Community 228 - "Fly worker topology decision — September 6, 2026"
Cohesion: 0.25
Nodes (7): Boundaries, Controller split brain (found during validation, now resolved), Fly worker topology decision — September 6, 2026, Measured evidence, Post-redeploy validation (2026-09-06 20:09), Remaining cutover step (not applied), Why not per-project apps

### Community 229 - "NavRail"
Cohesion: 0.25
Nodes (6): NavRail(), filterItems(), flattenItems(), tLabel(), translateItems(), getPluginNavItems()

### Community 230 - "token-utils.test.ts"
Cohesion: 0.70
Nodes (3): applyTimezoneOffset(), detectProvider(), generateCsvContent()

### Community 232 - "load-env.sh"
Cohesion: 0.67
Nodes (3): load_env_file(), load-env.sh script, trim_env_field()

### Community 234 - "gateways/control/route.ts"
Cohesion: 0.11
Nodes (22): dynamic, formatCommandOutput(), GatewayStatus, GatewayType, GET(), getHermesGatewayStatus(), getOpenClawGatewayStatus(), isDockerEnvironment() (+14 more)

### Community 235 - "permission-mode/route.ts"
Cohesion: 0.14
Nodes (20): dynamic, GET(), PUT(), readOrder(), canSetMode(), dynamic, GET(), PUT() (+12 more)

### Community 237 - "Command groups"
Cohesion: 0.12
Nodes (17): agents, API contract parity gate, auth, Command groups, connect, cron, events, Exit code contract (+9 more)

### Community 238 - "Mission Control"
Cohesion: 0.12
Nodes (15): Agent Control Interfaces, CLI, Common Pitfalls, Conventions, Data Directory, Docker, Key Directories, MCP Server (recommended for agents) (+7 more)

### Community 239 - "locale-parity.test.ts"
Cohesion: 0.31
Nodes (6): defaultLocale, Locale, localeNames, locales, flattenKeys(), Messages

### Community 240 - "executeProvisionJob"
Cohesion: 0.40
Nodes (6): ensureProvisionArtifacts(), executeProvisionJob(), getProvisionArtifactDir(), parseJobRequest(), parseJsonField(), runProvisionStep()

### Community 244 - "Findings"
Cohesion: 0.12
Nodes (15): Alternatives and consequences, Concrete consolidation sequence, Decision, Findings, High: incompatible native SQLite binaries prevented web startup, High: the desktop repository is not backed by a Git remote, High: two backend processes can run background jobs against one database, How the two applications connect (+7 more)

### Community 249 - "plugins.ts"
Cohesion: 0.12
Nodes (9): _categories, _integrations, _navItems, _panels, PluginCategory, PluginIntegrationDef, PluginNavItem, PluginToolProvider (+1 more)

### Community 250 - "openclaw-doctor-route.test.ts"
Cohesion: 0.29
Nodes (4): archiveOrphanTranscriptsForStateDir, openClawMaintenanceLimiter, requireRole, runOpenClaw

### Community 254 - "Full Changelog"
Cohesion: 0.14
Nodes (13): Added, Better first-run setup, Broader self-hosted platform support, Changed, Contributors, Fixed, Full Changelog, Highlights (+5 more)

### Community 255 - "3. Key Hardening Recommendations"
Cohesion: 0.14
Nodes (13): 1. Core Security Model & Deployment Considerations, 2. Hardened Baseline Configuration, 3.1. Network Security, 3.2. Authentication and Access Control, 3.3. Isolation and Sandboxing, 3.4. Credential and Secret Management, 3.5. File System Permissions, 3.6. Tool and Skill Security (+5 more)

### Community 256 - "API Endpoints"
Cohesion: 0.14
Nodes (13): Agent Management, API Endpoints, Automated Health Monitoring, Check for Updates, Common Workflows, Database Backup, Diagnostics (Admin Only), Health Check (+5 more)

### Community 257 - "cron-occurrences.ts"
Cohesion: 0.29
Nodes (10): buildDayKey(), CronOccurrence, getCronOccurrences(), matchesDay(), normalizeCronExpression(), parseCron(), ParsedCron, ParsedField (+2 more)

### Community 258 - "Contributor Covenant Code of Conduct"
Cohesion: 0.15
Nodes (12): 1. Correction, 2. Warning, 3. Temporary Ban, 4. Permanent Ban, Attribution, Contributor Covenant Code of Conduct, Enforcement, Enforcement Guidelines (+4 more)

### Community 259 - "mock-gateway.mjs"
Cohesion: 0.33
Nodes (4): ref_node_http, port, server, wss

### Community 260 - "gen-star-history.py"
Cohesion: 0.29
Nodes (9): datetime, json, build_series(), fetch_star_dates(), fmt_k(), main(), nice_step(), Generate self-hosted GitHub star-history charts (light + dark SVG). Replaces… (+1 more)

### Community 261 - "dashboard-page-frame.tsx"
Cohesion: 0.20
Nodes (9): Dashboard layout and LLM branding audit, Overview terminal sessions, Preview verification, Result, DashboardPageFrame(), dashboardPageLayout, panelLabel(), WIDE_PANELS (+1 more)

### Community 262 - "Mission Control production audit"
Cohesion: 0.17
Nodes (11): Apple design criteria, Browser verification, Decision, Findings and changes, Mission Control production audit, Performance observations, Production-readiness decision, Release handoff (+3 more)

### Community 263 - "Mission Control Fly command workers"
Cohesion: 0.17
Nodes (12): Active-session CLI bridge, Backend build and start, Client setup and active sessions, Commissioning status, Doppler presence-only preflight, Mission Control Fly command workers, Operations, observability, rollback, Operator-only image commissioning (+4 more)

### Community 264 - "Quickstart: Your First Agent in 5 Minutes"
Cohesion: 0.17
Nodes (12): Prerequisites, Quickstart: Your First Agent in 5 Minutes, Step 1: Start Mission Control, Step 2: Register an Agent, Step 3: Create a Task, Step 4: Poll the Task Queue, Step 5: Complete the Task, Step 6: Send a Heartbeat (+4 more)

### Community 265 - "keys/route.ts"
Cohesion: 0.29
Nodes (10): AgentKeyRow, AgentRow, ALLOWED_SCOPES, DELETE(), GET(), hashApiKey(), parseExpiry(), parseScopes() (+2 more)

### Community 266 - "verify-jev-live.mjs"
Cohesion: 0.33
Nodes (4): apiKey, baseUrl, contexts, headers

### Community 267 - "prepare-standalone-artifact.mjs"
Cohesion: 0.40
Nodes (4): allowedRoots, root, schemaPath, sourceRoot

### Community 268 - "auto-credentials.ts"
Cohesion: 0.36
Nodes (8): ensureAutoGeneratedCredentials(), generate(), getGeneratedFilePath(), PersistedValues, PLACEHOLDER_API_KEYS, PLACEHOLDER_AUTH_SECRETS, readPersisted(), writePersisted()

### Community 269 - "Deferred / blocked"
Cohesion: 0.18
Nodes (10): Deferred / blocked, Fly ops commissioning, Fly secret blockers (documentation only), Mega-files (debt), Mission Control audit — 2026-09-19 (repo-only), OpenAPI parity TODOs, Overview modular board, Ruflo MCP (`CLAUDE_PLUGIN_ROOT`) (+2 more)

### Community 270 - "test_mc_fly_enroll.py"
Cohesion: 0.18
Nodes (6): importlib_util, EnrollmentTests, subprocess, tempfile, unittest, unittest_mock

### Community 271 - "split-pane-layout.tsx"
Cohesion: 0.24
Nodes (8): getLayoutMode(), LAYOUT_CLASSES, LayoutMode, SplitPane, SplitPaneLayout(), SplitPaneLayoutProps, TerminalToolbar(), TerminalToolbarProps

### Community 272 - "webhook-retry-lease.ts"
Cohesion: 0.33
Nodes (6): claimDueWebhookRetry(), DueWebhookRetry, releaseWebhookRetryClaim(), WEBHOOK_RETRY_BATCH_LIMIT, WEBHOOK_RETRY_LEASE_SECONDS, processWebhookRetries()

### Community 273 - "claude-tasks.ts"
Cohesion: 0.33
Nodes (8): cachedResult, ClaudeCodeScanResult, ClaudeCodeTask, ClaudeCodeTeam, safeParse(), scanClaudeCodeTasks(), scanTasks(), scanTeams()

### Community 274 - "Spec Files"
Cohesion: 0.18
Nodes (10): CRUD Lifecycle, E2E Tests, Features, Infrastructure, OpenClaw Offline Harness, Running, Security & Auth, Shared (+2 more)

### Community 275 - "Quick Start"
Cohesion: 0.20
Nodes (10): 1. Register a connection, 2. Heartbeat loop, 3. Subscribe to events (SSE), 4. Report token usage, 5. Disconnect, API Reference, Connection Lifecycle, Direct CLI Integration (+2 more)

### Community 276 - "Fly recovery and shared skill hardening — 2026-09-06"
Cohesion: 0.22
Nodes (6): Deployed verification, Fly recovery and shared skill hardening — 2026-09-06, Forecast and operating boundaries, Global repository access, Implemented, Verification

### Community 277 - "Образцовый пример: команда из 3 провайдеров (Claude + OpenAI + Local) в Mission Control"
Cohesion: 0.20
Nodes (9): 11. Что менять для своего сценария, 12. Полезные команды, 13. Открытые вопросы (отметь если что-то не сошлось), 1.A. Создание нового workspace (если есть OpenClaw template), 1. Workspace — рекомендация: пропустить, использовать `default`, 7.A. Про "Owner" и колонку "Awaiting Owner", 7. Master-задача, 9. Чек-лист приёмки (+1 more)

### Community 278 - "Hardening Checklist"
Cohesion: 0.20
Nodes (10): Credentials, Docker (if applicable), Hardening Checklist, Monitoring, Network, OpenClaw Gateway, Reporting a Vulnerability, Security Considerations (+2 more)

### Community 279 - "jev-setup-flow.spec.ts"
Cohesion: 0.29
Nodes (3): assistant, policy, projects

### Community 280 - "fly-stale-controller.ts"
Cohesion: 0.39
Nodes (4): defaultProbe, flyControllerArtifactPath(), flyControllerIsStale(), FlyStaleProbe

### Community 281 - "Mission Control Fly audit — 2026-09-06"
Cohesion: 0.22
Nodes (9): Architecture and operating limits, Findings and repairs, Image scan scope, Initial r9 live result, Mission Control Fly audit — 2026-09-06, Recovery, Recovery and global repository release, Session scope (+1 more)

### Community 282 - "Screenshot Guide"
Cohesion: 0.22
Nodes (8): Automation, How to Take New Screenshots, Prerequisites, Screenshot Guide, Screenshots in the README, Steps, Tips, When to Refresh

### Community 283 - "10. Troubleshooting"
Cohesion: 0.22
Nodes (9): 10. Troubleshooting, Aegis не запускается, Architect отвечает обычным текстом, не структурированно, dispatchModel `local/...` падает с timeout, LMStudio не отвечает / `local API 404`, `OPENAI_API_KEY not set` в логах, Кнопки `+ Create Agent` нет, Кнопки `Projects` нет на /tasks (+1 more)

### Community 285 - "tasks-route-noop-update.test.ts"
Cohesion: 0.25
Nodes (7): currentTask, getMock, mutationLimiterMock, normalizeTaskUpdateStatusMock, prepareMock, requireRoleMock, validateBodyMock

### Community 286 - "Contributing to Mission Control"
Cohesion: 0.25
Nodes (8): Code Style, Contributing to Mission Control, Development Workflow, Getting Started, License, Project Structure, Reporting Bugs, Testing

### Community 287 - "Fly cleanup and capacity audit — September 6, 2026"
Cohesion: 0.25
Nodes (7): Apps and resources, Configuration for speed and efficiency, Fly cleanup and capacity audit — September 6, 2026, Remaining boundaries, Removed code, Session activation, Validation

### Community 288 - "Fly scale and cost decision — 2026-09-06"
Cohesion: 0.25
Nodes (8): Anticipated cost — transparent scenarios, Architecture, Cost improvements already implemented, Deliberate limits, Fly scale and cost decision — 2026-09-06, Recommendation, Recovery and skill integration, Sizing and growth

### Community 289 - "Jev in Mission Control"
Cohesion: 0.20
Nodes (9): Agent and CI use, Jev in Mission Control, Model management, Operator workflow, Privacy and audit behavior, Runtime configuration, Setup assistant boundary, Webdev persistence (+1 more)

### Community 290 - "OpenClaw Gateway Hardening"
Cohesion: 0.11
Nodes (18): 1. Network Security, 2. Authentication, 3. Hardened Gateway Configuration, 4. File Permissions, 5. Tool Security, 6. Monitoring, 7. Known CVEs, Agent Auth: Least-Privilege Key Guidance (+10 more)

### Community 291 - "4. Агент №2 — Implementor (OpenAI gpt-4o-mini)"
Cohesion: 0.25
Nodes (8): 4.1. Step 1 — Template, 4.2. Step 2 — Configure, 4.3. Soul, 4.4. ⚠ Важно — задать `dispatchModel = openai/gpt-4o-mini`, 4. Агент №2 — Implementor (OpenAI gpt-4o-mini), Вариант A: через UI (Config tab), Вариант B: через API (если UI падает), Что должно произойти

### Community 292 - "tasks-bulk-put-agent-access.test.ts"
Cohesion: 0.33
Nodes (4): prepareMock, requireRoleMock, transactionMock, validateBodyMock

### Community 293 - "Consolidated Fly implementation audit — 2026-09-05"
Cohesion: 0.29
Nodes (6): Consolidated Fly implementation audit — 2026-09-05, Current verdict, Improvements/deferred gaps, Remaining release gates (ordered), Repairs integrated, Verification in this continuation

### Community 294 - "Local Mission Control consolidation — 2026-09-05"
Cohesion: 0.29
Nodes (6): Local Mission Control consolidation — 2026-09-05, Preservation and integration, Remaining host/runtime findings, Result, Runtime completion, Verification

### Community 295 - "Onboarding + Walkthrough hardening plan"
Cohesion: 0.29
Nodes (6): Deliverables, Goals, Onboarding + Walkthrough hardening plan, Phase 1: audit and test map, Phase 2: implementation hardening, Phase 3: verification

### Community 296 - "Mission Control 2.2.0"
Cohesion: 0.29
Nodes (6): Compatibility, Highlights, Mission Control 2.2.0, Security posture, Upgrade notes, Verification

### Community 297 - "Mission Control 2.3.0"
Cohesion: 0.29
Nodes (6): Compatibility, Highlights, Mission Control 2.3.0, Security posture, Upgrade notes, Verification

### Community 298 - "3. Агент №1 — Architect (Claude Opus)"
Cohesion: 0.29
Nodes (7): 3.1. Открыть форму создания, 3.2. Step 1 — Template (выбор шаблона), 3.3. Step 2 — Configure (точные названия полей и опций), 3.4. Step 3 — Review, 3.5. После создания — задать Soul, 3.6. dispatchModel НЕ нужен для Anthropic, 3. Агент №1 — Architect (Claude Opus)

### Community 299 - "Release Process"
Cohesion: 0.29
Nodes (6): Branch Strategy, Compatibility and Deprecation, Release Checklist, Release Process, Required Gates Before Release, Tagging and Publishing

### Community 300 - "Mission Control Installer Skill"
Cohesion: 0.29
Nodes (6): Environment Configuration, Mission Control Installer Skill, Post-Install, Prerequisites, Usage, What This Skill Does

### Community 301 - "[1.3.0] - 2026-03-02"
Cohesion: 0.40
Nodes (5): [1.3.0] - 2026-03-02, Added, Changed, Contributors, Fixed

### Community 303 - "[2.2.0] - 2026-07-17"
Cohesion: 0.33
Nodes (6): [2.2.0] - 2026-07-17, Added, Changed, Fixed, Security, Upgrade notes

### Community 304 - "Desktop consolidation"
Cohesion: 0.33
Nodes (5): Desktop consolidation, Development, History and local data, Runtime ownership, Validation

### Community 305 - "OpenClaw Config Compatibility Note"
Cohesion: 0.33
Nodes (5): Fix, Incident, OpenClaw Config Compatibility Note, Recovery, Root Cause

### Community 306 - "8. Запустить конвейер"
Cohesion: 0.33
Nodes (6): 8.1. Architect декомпозирует, 8.2. Раскладка подзадач (вручную, ~3 мин), 8.3. Dev пишет код, 8.4. Linter (опционально, показывает работу local LLM), 8.5. Aegis ревью, 8. Запустить конвейер

### Community 307 - "pull_request_template.md"
Cohesion: 0.33
Nodes (5): Contribution Checklist, Evidence, Notes, Risk Level, Summary

### Community 308 - "check-workflow-action-pins.py"
Cohesion: 0.33
Nodes (4): pathlib, re, Fail when a workflow executes a remote action through a mutable ref., sys

### Community 309 - "webhooks.test.ts"
Cohesion: 0.39
Nodes (7): ipv4FromEmbedded(), isBlockedWebhookUrl(), isPrivateAddress(), pinSafeWebhookDestination(), pinWebhookUrl(), selectPublicWebhookAddress(), verifyWebhookSignature()

### Community 310 - "workspaces-tenant-access.test.ts"
Cohesion: 0.38
Nodes (5): AuditEvent, createTestDb(), FakeDb, Project, Workspace

### Community 311 - "job.test.mjs"
Cohesion: 0.39
Nodes (6): CHECKS, loadJob(), RUNTIMES, SETUPS, text(), validateJob()

### Community 312 - "LoginPage"
Cohesion: 0.40
Nodes (3): GoogleAccountsIdApi, LoginPage(), readLoginErrorPayload()

### Community 313 - "Support"
Cohesion: 0.33
Nodes (6): Bugs, Feature proposals, Maintainer scope, Questions and setup help, Security reports, Support

### Community 314 - "TODO later: Overview modular board"
Cohesion: 0.33
Nodes (5): Constraints, Decisions (locked), Done, Resume, TODO later: Overview modular board

### Community 315 - "Mission Control Wiki"
Cohesion: 0.33
Nodes (6): Core Concepts, Mission Control Wiki, Notes For Maintainers, Quick Start (5 Minutes), Recommended Wiki Structure, Start Here

### Community 316 - "Mission Control desktop"
Cohesion: 0.40
Nodes (4): Development, Local configuration and authentication, Mission Control desktop, Packaging and installation

### Community 317 - "activity-timeline-widget.tsx"
Cohesion: 0.53
Nodes (5): LogLike, ActivityTimelineWidget(), getSourceLabel(), getStatusBadge(), timeAgo()

### Community 319 - "0. Подготовка"
Cohesion: 0.40
Nodes (5): 0.1. `.env` в корне проекта, 0.2. LMStudio — поднять на хосте, 0.3. Поднять MC, 0.4. Войти, 0. Подготовка

### Community 320 - "5. Агент №3 — Linter (Local LMStudio)"
Cohesion: 0.40
Nodes (5): 5.1. Step 1 — Template, 5.2. Step 2 — Configure, 5.3. Soul, 5.4. dispatchModel — указать LMStudio модель, 5. Агент №3 — Linter (Local LMStudio)

### Community 321 - "Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?, Source Nodes

### Community 322 - "Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout, Source Nodes

### Community 323 - "runtime-types.ts"
Cohesion: 0.60
Nodes (3): AGENT_RUNTIME_TYPES, AgentRuntimeType, isAgentRuntimeType()

### Community 324 - "MobileBottomSheet"
Cohesion: 1.00
Nodes (3): MobileBottomSheet(), handleClose(), handleKeyDown()

### Community 325 - "2. Project (для тикет-префикса и группировки задач)"
Cohesion: 0.50
Nodes (4): 2.1. Создать проект — top-bar форма, 2.2. После создания — раскрыть и доконфигурить (опционально), 2.3. Закрой модал, 2. Project (для тикет-префикса и группировки задач)

### Community 326 - "6. Агент №4 — Aegis (Claude Sonnet, reviewer)"
Cohesion: 0.50
Nodes (4): 6.1. Создание (если нет), 6.2. Soul (формат строгий — MC парсит ответ), 6.3. dispatchModel — оставь Anthropic (нужен дефолт), 6. Агент №4 — Aegis (Claude Sonnet, reviewer)

### Community 331 - "webhook-response.test.ts"
Cohesion: 0.70
Nodes (3): ref_node_stream, MAX_WEBHOOK_RESPONSE_BYTES, readLimitedHttpBody()

### Community 341 - "repository"
Cohesion: 0.67
Nodes (3): repository, type, url

### Community 347 - "createResourceSampler"
Cohesion: 1.00
Nodes (3): createResourceSampler(), count(), text()

## Knowledge Gaps
- **2122 isolated node(s):** `name`, `version`, `private`, `description`, `type` (+2117 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 2571 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **39 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Work-memory lessons

**Preferred sources** — corroborated by past sessions; start here.
- `runs.ts` (2× useful, score=1.994201285)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `next` connect `next` to `resolveWithin`, `auth.ts`, `task-board-panel.tsx`, `useMissionControl`, `requireRole`, `hermes/route.ts`, `agent-squad-panel-phase3.tsx`, `login/page.tsx`, `store/index.ts`, `getDatabase`, `config`, `office-panel.tsx`, `integrations/route.ts`, `api/status/route.ts`, `nav-rail.tsx`, `gateway-config/route.ts`, `session-handoff.ts`, `security-scan.ts`, `engine-logo.tsx`, `jev-setup-session-repository.ts`, `apiFetch`, `proxy.ts`, `db.ts`, `package.json`, `skills/route.ts`, `memory-utils.ts`, `exec-approvals/route.ts`, `github/route.ts`, `handoff/route.ts`, `validation.ts`, `react`, `mcp-audit.ts`, `gnap-sync.ts`, `workload/route.ts`, `layout.tsx`, `gateways/connect/route.ts`, `onboarding/route.ts`, `skill-registry.ts`, `graph/route.ts`, `fetchWithRetry`, `super-admin.ts`, `agent-runtimes.ts`, `runCommand`, `recurring-tasks.ts`, `gateways/health/route.ts`, `cli-inventory.ts`, `framework-templates.ts`, `flight-deck/route.ts`, `runs.ts`, `pty/setup/route.ts`, `session-transcript-route.ts`, `github-pulls-api.ts`, `assistant/route.ts`, `agent-filesystem-isolation.test.ts`, `tokens/route.ts`, `agent-sync.ts`, `[id]/diagnostics/route.ts`, `event-bus.ts`, `message-bubble.tsx`, `api/health/route.ts`, `fix/route.ts`, `channels/route.ts`, `provision-jobs/[id]/route.ts`, `TranscriptMessage`, `jev-repository.ts`, `[id]/memory/route.ts`, `chat-permission-mode-route.test.ts`, `aggregate/route.ts`, `migrations.ts`, `index/route.ts`, `attribution/route.ts`, `gateways/control/route.ts`, `permission-mode/route.ts`, `locale-parity.test.ts`, `keys/route.ts`, `release-update-route-security.test.ts`, `tasks-route-noop-update.test.ts`, `tasks-bulk-put-agent-access.test.ts`?**
  _High betweenness centrality (0.199) - this node is a cross-community bridge._
- **Why does `vitest` connect `vitest` to `mac-cleanup/index.ts`, `resolveWithin`, `auth.ts`, `chat-icons.tsx`, `task-board-panel.tsx`, `next`, `useMissionControl`, `requireRole`, `hermes/route.ts`, `agent-squad-panel-phase3.tsx`, `login/page.tsx`, `store/index.ts`, `group-sessions.ts`, `getDatabase`, `config`, `api/status/route.ts`, `gateway-config/route.ts`, `chat-session-identity.ts`, `session-handoff.ts`, `security-scan.ts`, `chat-session-pane.tsx`, `task-dispatch.ts`, `engine-logo.tsx`, `jev-setup-session-repository.ts`, `chat-composer.tsx`, `dashboard.tsx`, `chat-usage-tracker.ts`, `cron-management-panel.tsx`, `apiFetch`, `proxy.ts`, `db.ts`, `package.json`, `Production (Docker)`, `skills/route.ts`, `skill-sync.ts`, `memory-utils.ts`, `gateway-config-panel.tsx`, `exec-approvals/route.ts`, `injection-guard.ts`, `github/route.ts`, `dashboard-cli-fleets.ts`, `exec-approval-panel.tsx`, `chat-display.ts`, `handoff/route.ts`, `validation.ts`, `react`, `mcp-audit.ts`, `session-terminal-widget.tsx`, `openclaw-doctor.ts`, `claude-fleet-plans.ts`, `gnap-sync.ts`, `jev-panel.tsx`, `session-list-local.ts`, `jev-assistant-provider.ts`, `workload/route.ts`, `github-sync-engine.ts`, `layout.tsx`, `gateways/connect/route.ts`, `onboarding/route.ts`, `dashboard-board.ts`, `fly-topology.test.ts`, `skill-registry.ts`, `graph/route.ts`, `fetchWithRetry`, `jev-assistant-service.ts`, `fly-admission-schema.ts`, `agent-runtimes.ts`, `runCommand`, `recurring-tasks.ts`, `ref_node_os`, `api-contract-parity.ts`, `cli-inventory.ts`, `device-identity.ts`, `markdown-renderer.tsx`, `framework-templates.ts`, `openclaw-doctor-banner.tsx`, `jev-cloud.test.ts`, `pty/setup/route.ts`, `fly-admission.test.ts`, `claude-transcript.ts`, `claude-code-sessions.ts`, `ref_node_module`, `session-archive.ts`, `chat-desktop-workspace.tsx`, `github-pulls-api.ts`, `fly-reservations.ts`, `assistant/route.ts`, `agent-filesystem-isolation.test.ts`, `tokens/route.ts`, `agent-sync.ts`, `coordinator-routing.ts`, `adapter-compliance.test.ts`, `session-continue-run.ts`, `event-bus.ts`, `message-bubble.tsx`, `chat-fly-jobs.ts`, `websocket-utils.ts`, `alert-evaluate.ts`, `runtime-history.ts`, `kimi-sessions.ts`, `fleet-agents.ts`, `dashboard-board-persist.ts`, `hook-profiles.ts`, `task-dispatch-sandbox.test.ts`, `api/health/route.ts`, `fix/route.ts`, `channels/route.ts`, `codex-transcript.ts`, `TranscriptMessage`, `jev-repository.ts`, `session-terminal-cell.tsx`, `better-sqlite3`, `ref_node_child_process`, `health-utils.test.ts`, `fleet-projects.ts`, `[id]/memory/route.ts`, `openclaw-gateway.ts`, `claude-sessions.ts`, `github.ts`, `session-terminal-view.tsx`, `task-routing.ts`, `chat-permission-mode-route.test.ts`, `handoff-brief.ts`, `typesafe-probe.ts`, `github-activity.ts`, `jev-service.ts`, `adaptive-context-handoff.ts`, `buildGatewayWebSocketUrl`, `migrations.ts`, `openclaw-doctor-fix.ts`, `minimax.ts`, `jev-validation.ts`, `token-utils.test.ts`, `gateways/control/route.ts`, `permission-mode/route.ts`, `locale-parity.test.ts`, `openclaw-doctor-route.test.ts`, `cron-occurrences.ts`, `dashboard-page-frame.tsx`, `webhook-retry-lease.ts`, `fly-stale-controller.ts`, `release-update-route-security.test.ts`, `tasks-route-noop-update.test.ts`, `tasks-bulk-put-agent-access.test.ts`, `agent-model-trim.test.ts`, `webhooks.test.ts`, `workspaces-tenant-access.test.ts`, `runtime-types.ts`, `webhook-response.test.ts`, `vitest.config.ts`, `adapter-assignment-isolation.test.ts`?**
  _High betweenness centrality (0.164) - this node is a cross-community bridge._
- **Why does `react` connect `react` to `widget-primitives.tsx`, `chat-icons.tsx`, `dashboard-page-frame.tsx`, `task-board-panel.tsx`, `security-audit-panel.tsx`, `useMissionControl`, `webhook-panel.tsx`, `chat-desktop-workspace.tsx`, `agent-squad-panel-phase3.tsx`, `login/page.tsx`, `store/index.ts`, `split-pane-layout.tsx`, `group-sessions.ts`, `office-panel.tsx`, `nav-rail.tsx`, `message-bubble.tsx`, `chat-fly-jobs.ts`, `chat-session-pane.tsx`, `security-scan-card.tsx`, `engine-logo.tsx`, `cost-tracker-panel.tsx`, `chat-composer.tsx`, `dashboard.tsx`, `chat-usage-tracker.ts`, `cron-management-panel.tsx`, `apiFetch`, `widget-grid.tsx`, `package.json`, `TranscriptMessage`, `websocket.ts`, `session-terminal-cell.tsx`, `gateway-config-panel.tsx`, `dashboard-cli-fleets.ts`, `exec-approval-panel.tsx`, `channels-panel.tsx`, `chat-display.ts`, `session-terminal-view.tsx`, `agent-comms-panel.tsx`, `session-terminal-widget.tsx`, `jev-panel.tsx`, `layout.tsx`, `markdown-renderer.tsx`, `memory-browser-panel.tsx`, `plugins.ts`, `openclaw-doctor-banner.tsx`, `skills-panel.tsx`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **What connects `name`, `version`, `private` to the rest of the system?**
  _2122 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `vitest` be split into smaller, more focused modules?**
  _Cohesion score 0.03886572143452877 - nodes in this community are weakly interconnected._
- **Should `widget-primitives.tsx` be split into smaller, more focused modules?**
  _Cohesion score 0.08787878787878788 - nodes in this community are weakly interconnected._
- **Should `mac-cleanup/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.0545996477442081 - nodes in this community are weakly interconnected._