# Graph Report - mission-control  (2026-09-20)

## Corpus Check
- 1300 files · ~995,567 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 16 file(s) not represented in the graph (top: (none) 11, .example 1, .toml 1)

## Summary
- 7381 nodes · 19861 edges · 344 communities (308 shown, 36 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 194 edges (avg confidence: 0.87)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ffe20156`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- vitest
- widget-primitives.tsx
- mac-cleanup/index.ts
- lib/config.ts
- auth.ts
- chat-icons.tsx
- assistant/route.ts
- jev-setup-session-repository.ts
- security-audit-panel.tsx
- useMissionControl
- denyUnscopedResourceForStrictWorkspace
- hermes/route.ts
- Changelog
- agent-squad-panel-phase3.tsx
- login/page.tsx
- store/index.ts
- main.mjs
- group-sessions.ts
- header-bar.tsx
- api/tasks/route.ts
- eventBus
- office-panel.tsx
- integrations/route.ts
- api/status/route.ts
- nav-rail.tsx
- jev-context.ts
- memory-utils.ts
- getGitHubToken
- chat-session-identity.ts
- @playwright/test
- workspaces.ts
- utils.ts
- security-scan.ts
- chat-session-metrics.ts
- task-dispatch.ts
- polled.mjs
- engine-logo.tsx
- mc-fly-enroll.py
- build-app.mjs
- fixtures.mjs
- cost-tracker-panel.tsx
- chat-composer.tsx
- keys/route.ts
- task-board-panel.tsx
- cron-management-panel.tsx
- apiFetch
- claude-fleet-rename.ts
- webhooks.ts
- next
- widget-grid.tsx
- package.json
- desktop-auth-store.mjs
- agent-avatar.tsx
- chat-usage-tracker.ts
- websocket.ts
- claude-sessions.ts
- api/memory/route.ts
- gateway-config-panel.tsx
- Runbook: move the worker pool onto an isolated private network
- dependencies
- scripts
- ref_fs
- injection-guard.ts
- github/route.ts
- fly-topology.test.ts
- dashboard-cli-fleets.ts
- exec-approval-panel.tsx
- channels-panel.tsx
- chat-display.ts
- handoff/route.ts
- agent-comms-panel.tsx
- requireRole
- useSmartPoll
- mcp-audit.ts
- session-terminal-widget.tsx
- doctor/route.ts
- mc-tui.cjs
- claude-fleet-plans.ts
- gnap-sync.ts
- jev-panel.tsx
- session-list-local.ts
- skills-panel.tsx
- scheduler.ts
- workload/route.ts
- github-sync-engine.ts
- layout.tsx
- start-e2e-server.mjs
- gateways/connect/route.ts
- onboarding/route.ts
- dashboard-board.ts
- fly/status/route.ts
- install.ps1
- mc-provisioner-daemon.js
- models.ts
- skills/route.ts
- graph/route.ts
- fetchWithRetry
- evals/route.ts
- super-admin.ts
- proxy.ts
- desktop/package.json
- agent-runtimes.ts
- Jev Workspace Product Requirements
- runtime-history.ts
- local-session-list.ts
- Mission Control Style Guide
- schema.sql
- install.sh
- runCommand
- recurring-tasks.ts
- tokens/route.ts
- gateways/health/route.ts
- api-contract-parity.ts
- cli-inventory.ts
- device-identity.ts
- compilerOptions
- chat-session-pane.tsx
- devDependencies
- framework-templates.ts
- memory-browser-panel.tsx
- agent-templates.ts
- flight-deck/route.ts
- react
- runs.ts
- pty-websocket.ts
- jev-cloud.test.ts
- conversation-list.tsx
- nodes-panel.tsx
- opencode-sessions.ts
- artifact/route.ts
- fly-reconciler.ts
- claude-transcript.ts
- session-transcript-types.ts
- claude-code-sessions.ts
- ref_node_module
- codex-transcript.ts
- ref_node_assert
- session-transcript-route.ts
- session-archive.ts
- rail-width.ts
- webhook-panel.tsx
- ref_node_crypto
- session-continue-run.ts
- websocket-utils.ts
- mc-fly-contract.test.cjs
- agent-filesystem-isolation.test.ts
- provider-subscriptions.ts
- Troubleshooting
- agent-sync.ts
- coordinator-routing.ts
- local-agent-sync.ts
- mc-fly-cli.test.cjs
- notification-daemon.sh
- adapter-compliance.test.ts
- [id]/diagnostics/route.ts
- pipeline-tab.tsx
- rate-limit.ts
- message-bubble.tsx
- chat-fly-jobs.ts
- openclaw-doctor-banner.tsx
- alert-evaluate.ts
- security-scan-card.tsx
- codex-sessions.ts
- kimi-sessions.ts
- package-manager-patches/package.json
- dashboard-cli-fleets.test.ts
- fleet-agents.ts
- Orchestration Patterns
- handoff-brief.ts
- hook-profiles.ts
- createTestAgent
- fleetMemoryRoots
- check-api-contract-parity.mjs
- api/health/route.ts
- gateway-runtime.ts
- channels/route.ts
- Everything Included In The v2 Update
- PtySession
- unlock-renderer.mjs
- mcp-server.spec.ts
- dashboard-board-persist.ts
- terminal-view.tsx
- readLimiter
- session-terminal-cell.tsx
- memory/health/route.ts
- pty-manager.ts
- createTestTask
- agent-heartbeat.sh
- health-utils.test.ts
- fleet-projects.ts
- attention-detector.ts
- fly-stale-controller.ts
- permission-mode/route.ts
- openclaw-gateway.ts
- task-costs.ts
- check-standalone-artifact.mjs
- deploy-standalone.sh
- github.ts
- Mission Control Hardening
- task-routing.ts
- chat-permission-mode-route.test.ts
- chat/messages/route.ts
- tasks-route-noop-update.test.ts
- helpers.ts
- typesafe-probe.ts
- security-audit.sh
- github-activity.ts
- Mission Control
- jev-client.ts
- hermes-sessions.ts
- optimize/route.ts
- releases/update/route.ts
- aggregate/route.ts
- migrations.ts
- generate-env.sh
- mac-safe-reclaim
- start-standalone.sh
- station-doctor.sh
- index/route.ts
- Mission Control Platform Hardening + Full Agent CLI/TUI PRD
- Mission Control Agent Skill
- os-users/route.ts
- minimax.ts
- attribution/route.ts
- jev-validation.ts
- docs-knowledge.ts
- process-title.test.sh
- Agent Setup Guide
- locale-parity.test.ts
- NavRail
- token-utils.test.ts
- next.config.js
- load-env.sh
- reap-controller.test.sh
- gateways/control/route.ts
- chat-folder-order-route.test.ts
- docker-entrypoint.sh
- Command groups
- Mission Control
- memory-search.ts
- repo-health-tasks.ts
- load-env.test.sh
- security-audit.test.sh
- take-screenshots.ts
- Findings
- open-mission-control
- build-app.sh script
- check-node-version.mjs
- opencode
- plugins.ts
- token-runtime-isolation.test.ts
- git-askpass.mjs
- Full Changelog
- 3. Key Hardening Recommendations
- API Endpoints
- task-dispatch-sandbox.test.ts
- Contributor Covenant Code of Conduct
- mock-gateway.mjs
- gen-star-history.py
- dashboard-page-frame.tsx
- Mission Control production audit
- Mission Control Fly command workers
- Quickstart: Your First Agent in 5 Minutes
- github-pulls-api.ts
- verify-jev-live.mjs
- workspace-sql-regressions.test.ts
- task-branch-route-agent-access.test.ts
- Deferred / blocked
- test_mc_fly_enroll.py
- session-plan.ts
- ErrorBoundary
- opencode-transcript.ts
- Spec Files
- Quick Start
- Fly recovery and shared skill hardening — 2026-09-06
- Образцовый пример: команда из 3 провайдеров (Claude + OpenAI + Local) в Mission Control
- Hardening Checklist
- jev-setup-flow.spec.ts
- dashboard-widgets.ts
- Mission Control Fly audit — 2026-09-06
- Screenshot Guide
- 10. Troubleshooting
- release-update-route-security.test.ts
- eslint.config.mjs
- Contributing to Mission Control
- Fly cleanup and capacity audit — September 6, 2026
- Fly scale and cost decision — 2026-09-06
- Jev in Mission Control
- OpenClaw Gateway Hardening
- 4. Агент №2 — Implementor (OpenAI gpt-4o-mini)
- tasks-bulk-put-agent-access.test.ts
- Consolidated Fly implementation audit — 2026-09-05
- Local Mission Control consolidation — 2026-09-05
- Onboarding + Walkthrough hardening plan
- Mission Control 2.2.0
- Mission Control 2.3.0
- 3. Агент №1 — Architect (Claude Opus)
- Release Process
- Mission Control Installer Skill
- [1.3.0] - 2026-03-02
- Fly worker topology decision — September 6, 2026
- [2.2.0] - 2026-07-17
- Desktop consolidation
- OpenClaw Config Compatibility Note
- 8. Запустить конвейер
- pull_request_template.md
- check-workflow-action-pins.py
- @testing-library/jest-dom
- better-sqlite3
- claude-tasks.ts
- repository
- Support
- TODO later: Overview modular board
- Mission Control Wiki
- Mission Control desktop
- vitest.config.ts
- activity-timeline-widget.tsx
- 0. Подготовка
- 5. Агент №3 — Linter (Local LMStudio)
- Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?
- Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout
- runtime-types.ts
- MobileBottomSheet
- 2. Project (для тикет-префикса и группировки задач)
- 6. Агент №4 — Aegis (Claude Sonnet, reviewer)
- event-bus.ts
- 20260920231000_mission_control_jev_events.sql
- jev-supabase-persistence.md
- doppler-runtime.test.sh
- engineering-bot/identity.md
- engineering-bot/soul.md
- research-bot/identity.md
- research-bot/soul.md
- auth-guards.spec.ts
- delete-body.spec.ts
- @scalar/api-reference-react
- limit-caps.spec.ts

## God Nodes (most connected - your core abstractions)
1. `requireRole()` - 451 edges
2. `getDatabase()` - 404 edges
3. `vitest` - 337 edges
4. `next` - 245 edges
5. `logger` - 163 edges
6. `apiFetch()` - 145 edges
7. `react` - 142 edges
8. `denyUnscopedResourceForStrictWorkspace()` - 125 edges
9. `logAuditEvent()` - 121 edges
10. `useMissionControl` - 104 edges

## Surprising Connections (you probably didn't know these)
- `Active-session CLI bridge` --references--> `build()`  [INFERRED]
  docs/fly-agent-workers.md → apps/desktop/scripts/build-app.mjs
- `How It Works` --references--> `status()`  [INFERRED]
  docs/agent-setup.md → apps/desktop/src/desktop-unlock.mjs
- `What runs where` --references--> `command()`  [INFERRED]
  docs/fly-agent-workers.md → scripts/mc-fly-enroll.py
- `Recommended Wiki Structure` --references--> `Home()`  [INFERRED]
  wiki/Home.md → src/app/[[...panel]]/page.tsx
- `Notes` --references--> `sessionId()`  [INFERRED]
  docs/cli-integration.md → src/app/api/jev/sessions/[id]/route.ts

## Import Cycles
- None detected.

## Communities (344 total, 36 thin omitted)

### Community 0 - "vitest"
Cohesion: 0.03
Nodes (36): ref_node_fs, ref_node_os, ref_node_path, vitest, allowedRoots, root, schemaPath, sourceRoot (+28 more)

### Community 1 - "widget-primitives.tsx"
Cohesion: 0.11
Nodes (24): DashboardGitHubStats, DashboardSystemStats, ActivityIconMini(), AgentIcon(), ClaudeStats, CostIcon(), DbStats, formatTokensShort() (+16 more)

### Community 2 - "mac-cleanup/index.ts"
Cohesion: 0.05
Nodes (94): collectProtectedRoots(), leaseRoots(), CATALOG, catalogBinary(), CatalogEntry, main(), capture(), buildFindings() (+86 more)

### Community 3 - "lib/config.ts"
Cohesion: 0.10
Nodes (40): ALLOWED_FILES, FILE_ALIASES, GET(), getAgentByIdOrName(), PUT(), resolveAgentWorkspacePath(), agentColumnName(), agentColumnValue() (+32 more)

### Community 4 - "auth.ts"
Cohesion: 0.06
Nodes (72): ref_crypto, ensureUniqueUsername(), GET(), makeUsernameFromEmail(), POST(), POST(), POST(), POST() (+64 more)

### Community 5 - "chat-icons.tsx"
Cohesion: 0.10
Nodes (28): IconArtifacts(), IconChevron(), IconClock(), IconCustomize(), IconDispatch(), IconGrip(), IconPin(), IconPlus() (+20 more)

### Community 6 - "assistant/route.ts"
Cohesion: 0.08
Nodes (35): zod, POST(), sanitizeForPrompt(), claudeExecutable(), generateJevAssistantDraft(), isJevAssistantAvailable(), JevAssistantProviderError, PROVIDER_ENV_KEYS (+27 more)

### Community 7 - "jev-setup-session-repository.ts"
Cohesion: 0.11
Nodes (33): boundedInteger(), Context, GET(), canManage(), Context, DELETE(), GET(), PATCH() (+25 more)

### Community 8 - "security-audit-panel.tsx"
Cohesion: 0.05
Nodes (43): recharts, classLabel(), formatAge(), MacCleanupJobs(), formatValue(), MacCleanupMeters(), tone(), valueClass() (+35 more)

### Community 9 - "useMissionControl"
Cohesion: 0.03
Nodes (95): bootLabelKeys, CapabilitiesResponse, GatewaySummary, Home(), STEP_KEYS, Dashboard(), DashboardRequest, REQUEST_LABELS (+87 more)

### Community 10 - "denyUnscopedResourceForStrictWorkspace"
Cohesion: 0.04
Nodes (83): GET(), POST(), ALLOWED_COLORS, dynamic, GET(), loadPrefs(), PATCH(), savePrefs() (+75 more)

### Community 11 - "hermes/route.ts"
Cohesion: 0.13
Nodes (21): GET(), dataDir, dynamic, extractDeviceAuth(), GET(), HOOK_DIR, POST(), GET() (+13 more)

### Community 12 - "Changelog"
Cohesion: 0.06
Nodes (35): [1.0.0] - 2026-02-15, [1.1.0] - 2026-02-27, [1.2.0] - 2026-03-01, [2.0.0] - 2026-03-11, [2.0.1] - 2026-03-18, [2.1.0] - 2026-07-04, [2.3.0] - 2026-07-25, Added (+27 more)

### Community 13 - "agent-squad-panel-phase3.tsx"
Cohesion: 0.05
Nodes (46): ConnectorsTab(), ActivityTab(), Agent, AgentCronJob, ChannelAccountInfo, ChannelEntryInfo, ChannelsTab(), ConfigTab() (+38 more)

### Community 14 - "login/page.tsx"
Cohesion: 0.08
Nodes (24): ConnectionStatus, GATEWAY_URL_PRESETS, GoogleAccountsIdApi, GoogleApi, GoogleCredentialResponse, LoginErrorPayload, LoginPage(), LoginRequestBody (+16 more)

### Community 15 - "store/index.ts"
Cohesion: 0.04
Nodes (60): messages_en, ChatInput(), ChatInputProps, ChatPanel(), ChatWorkspace(), ChatWorkspaceProps, ChatDesktopWorkspace(), IconSparkle() (+52 more)

### Community 16 - "main.mjs"
Cohesion: 0.11
Nodes (34): openBackend(), partitionForOrigin(), ensureServer(), ensureService(), ensureServices(), healthStatus(), launchctl(), serviceTarget() (+26 more)

### Community 17 - "group-sessions.ts"
Cohesion: 0.11
Nodes (22): ChatFilterPopover(), OpenMenu, applyFolderOrder(), mergeFolderOrder(), moveFolder(), parseFolderOrder(), uniqueStrings(), buildSidebarRows() (+14 more)

### Community 18 - "header-bar.tsx"
Cohesion: 0.09
Nodes (20): ModeBadge(), NavigationLatencyStat(), QUICK_NAV_COMMANDS, SearchResult, AgentSquadPanelPhase3(), DigitalClock(), ThemeSelector(), buildTaskStatParts() (+12 more)

### Community 19 - "api/tasks/route.ts"
Cohesion: 0.06
Nodes (57): GET(), POST(), GET(), GET(), GET(), POST(), DELETE(), formatTicketRef() (+49 more)

### Community 20 - "eventBus"
Cohesion: 0.09
Nodes (16): AgentRegistration, Assignment, FrameworkAdapter, HeartbeatPayload, queryPendingAssignments(), TaskReport, AutoGenAdapter, ClaudeSdkAdapter (+8 more)

### Community 21 - "office-panel.tsx"
Cohesion: 0.05
Nodes (57): buildPath(), buildWalkabilityGrid(), clamp(), easeInOut(), findGridPath(), FlightDeckResponse, formatLastSeen(), getInitials() (+49 more)

### Community 22 - "integrations/route.ts"
Cohesion: 0.11
Nodes (36): BLOCKED_PREFIXES, BLOCKED_VARS, BuiltinCategory, CATEGORIES, checkCommandAvailable(), checkOllamaReachable(), checkOpAuthenticated(), checkOpAvailable() (+28 more)

### Community 23 - "api/status/route.ts"
Cohesion: 0.13
Nodes (25): GET(), getAvailableModels(), getCapabilities(), getDashboardData(), getDbStats(), getGatewayStatus(), getMemorySnapshot(), getSystemStatus() (+17 more)

### Community 24 - "nav-rail.tsx"
Cohesion: 0.05
Nodes (7): adminOnlyPanels, gatewayOnlyPanels, groupTranslationKeys, NavGroup, navGroups, NavItem, navItemTranslationKeys

### Community 25 - "jev-context.ts"
Cohesion: 0.11
Nodes (25): formatHermesCommandOutput(), hermesMutationSchema, parseHermesSetupCommand(), buildJevRepositoryContext(), git(), inside(), languageSummary(), projectRoot() (+17 more)

### Community 26 - "memory-utils.ts"
Cohesion: 0.11
Nodes (23): GET(), mergeContextPayloads(), ConsolidationItem, ConsolidationReport, ContextPayload, extractFrontmatter(), extractSchema(), GapItem (+15 more)

### Community 27 - "getGitHubToken"
Cohesion: 0.33
Nodes (7): getGitHubToken(), readGhAuthToken(), getEffectiveEnvValue(), parseEnvLine(), readOpenClawEnvFile(), getEffectiveEnvValue, spawnSync

### Community 28 - "chat-session-identity.ts"
Cohesion: 0.12
Nodes (35): conversationsToItems(), gitLensByProject(), GitLensSessionRow, HomeSessionRow, sortNewestFirst(), toGitLensRow(), toHomeSessions(), toSessionItem() (+27 more)

### Community 29 - "@playwright/test"
Cohesion: 0.07
Nodes (4): @playwright/test, API_KEY_HEADER, stamp, dbPath

### Community 30 - "workspaces.ts"
Cohesion: 0.15
Nodes (23): DELETE(), GET(), POST(), toProjectId(), DELETE(), GET(), normalizePrefix(), PATCH() (+15 more)

### Community 31 - "utils.ts"
Cohesion: 0.12
Nodes (10): Agent, AgentStatus, ChatMessage, ConnectionState, Conversation, DashboardStats, FlowEdge, FlowNode (+2 more)

### Community 32 - "security-scan.ts"
Cohesion: 0.07
Nodes (56): AgentScanFixRequest, buildSummary(), FixScope, isFixableInScope(), POST(), fixRequestSchema, FixResult, getFailingChecks() (+48 more)

### Community 33 - "chat-session-metrics.ts"
Cohesion: 0.18
Nodes (20): ChatSessionPane(), ContextWindowBar(), transcriptExcerpt(), contextPercent(), DEFAULT_CONTEXT_WINDOW, formatDuration(), formatTokenLabel(), isInOutPair() (+12 more)

### Community 34 - "task-dispatch.ts"
Cohesion: 0.07
Nodes (63): ref_node_child_process, auditClaudeSessionDispatch(), firstExisting(), FleetCliResult, resolveGrokCliPath(), resolveKimiCliPath(), runCli(), runGrokPrompt() (+55 more)

### Community 35 - "polled.mjs"
Cohesion: 0.16
Nodes (24): ref_node_url, packageFailure(), clearGitCredentials(), withGitAuth(), RESULT_RETENTION_SECONDS, retainResult(), workDeadline(), executeJob() (+16 more)

### Community 36 - "engine-logo.tsx"
Cohesion: 0.15
Nodes (23): LLM identity system, EngineLogo(), EngineLogoSet(), ModelPicker(), IconCheck(), HANDOFF_SEATS, HandoffPicker(), modelsFor() (+15 more)

### Community 37 - "mc-fly-enroll.py"
Cohesion: 0.27
Nodes (12): argparse, fcntl, hashlib, os, command(), enroll_key(), github(), main() (+4 more)

### Community 38 - "build-app.mjs"
Cohesion: 0.24
Nodes (17): build(), stage(), validCache(), buildOptions(), resolveElectron(), exists(), installApp(), publishCandidate() (+9 more)

### Community 39 - "fixtures.mjs"
Cohesion: 0.17
Nodes (13): canonicalRoot(), checkoutRoot(), discoverRoot(), PACKAGE_ROOT, NetworkError, request(), fixture(), noWait() (+5 more)

### Community 40 - "cost-tracker-panel.tsx"
Cohesion: 0.14
Nodes (21): AgentsView(), ByAgentEntry, ByAgentModelBreakdown, ByAgentResponse, COLORS, CostTrackerPanel(), formatCost(), formatNumber() (+13 more)

### Community 41 - "chat-composer.tsx"
Cohesion: 0.10
Nodes (28): fast-check, ChatComposer(), ComposerChips(), PermissionMenu(), UsageBanner(), IconFolder(), IconSend(), ALIASES (+20 more)

### Community 42 - "keys/route.ts"
Cohesion: 0.29
Nodes (10): AgentKeyRow, AgentRow, ALLOWED_SCOPES, DELETE(), GET(), hashApiKey(), parseExpiry(), parseScopes() (+2 more)

### Community 43 - "task-board-panel.tsx"
Cohesion: 0.08
Nodes (33): Agent, AgentsResponse, COLOR_PALETTE, mutate(), Project, ProjectManagerModal(), ProjectsResponse, Agent (+25 more)

### Community 44 - "cron-management-panel.tsx"
Cohesion: 0.09
Nodes (34): addDays(), AGENT_COLORS, CalendarViewMode, ClaudeCodeTeamsSection(), CronManagementPanel(), DayJobSummary, extractApiErrorMessage(), formatDateLabel() (+26 more)

### Community 45 - "apiFetch"
Cohesion: 0.03
Nodes (81): LlmLabel(), ContextSwitcher(), ClaudeSetup(), CodexSetup(), HermesSetup(), OpenClawSetup(), OpenCodeSetup(), RuntimeSetupModal() (+73 more)

### Community 46 - "claude-fleet-rename.ts"
Cohesion: 0.38
Nodes (8): hasTable(), NAME_COLUMNS, renameAgentTable(), renameClaudeFleetAgentRows(), renameLooseColumns(), renameProjectAssignments(), RENAMES, rewriteAgentConfig()

### Community 47 - "webhooks.ts"
Cohesion: 0.10
Nodes (30): delay(), ref_node_dns, ref_node_https, ref_node_stream, MAX_WEBHOOK_RESPONSE_BYTES, readLimitedHttpBody(), claimDueWebhookRetry(), DueWebhookRetry (+22 more)

### Community 48 - "next"
Cohesion: 0.04
Nodes (69): next, GET(), handleActivitiesRequest(), handleStatsRequest(), DELETE(), POST(), BACKUP_DIR, GET() (+61 more)

### Community 49 - "widget-grid.tsx"
Cohesion: 0.11
Nodes (22): SIZE_CLASSES, WIDGET_COMPONENTS, DashboardData, formatBytes(), formatUptime(), HealthRow(), LogRow(), StatRow() (+14 more)

### Community 50 - "package.json"
Cohesion: 0.06
Nodes (33): author, description, keywords, license, name, packageManager, version, class-variance-authority (+25 more)

### Community 51 - "desktop-auth-store.mjs"
Cohesion: 0.16
Nodes (23): configure(), readPin(), createPinRecord(), createPinThrottle(), credentialsFromEnvironment(), derive(), parseEnvironment(), unquote() (+15 more)

### Community 52 - "agent-avatar.tsx"
Cohesion: 0.24
Nodes (11): AgentAvatar(), AgentAvatarProps, getAvatarColors(), getInitials(), hashString(), logoSize, sizeClasses, AgentBrand (+3 more)

### Community 53 - "chat-usage-tracker.ts"
Cohesion: 0.15
Nodes (21): BAR, UsagePopup(), CAPS, extraNeedle(), liveUsageTracker(), modelNeedle(), oldestSince(), sumTokens() (+13 more)

### Community 54 - "websocket.ts"
Cohesion: 0.07
Nodes (29): authTokenRef, clearHandshakeTimers(), connectRef, connectSentRef, formatAge(), GatewayFrame, GatewayMessage, gatewaySupportsPingRef (+21 more)

### Community 55 - "claude-sessions.ts"
Cohesion: 0.14
Nodes (16): ref_readline, looksLikeNoisePrompt(), claudeConfigDirForAgent(), claudeSessionHomes(), clampTimestamp(), DEFAULT_PRICING, JSONLEntry, lastSyncResult (+8 more)

### Community 56 - "api/memory/route.ts"
Cohesion: 0.18
Nodes (23): GET(), LinkGraph, mergeGraphs(), scanRoots(), withPathPrefix(), buildFileTree(), DELETE(), GET() (+15 more)

### Community 57 - "gateway-config-panel.tsx"
Cohesion: 0.12
Nodes (27): ArrayField(), computeDiff(), deepSet(), defaultValueFor(), DiffEntry, FallbackField(), Feedback, FormMode (+19 more)

### Community 58 - "Runbook: move the worker pool onto an isolated private network"
Cohesion: 0.25
Nodes (6): 1. Create the isolated app, 2. Set the three keys together, 3. Restart on an idle window, 4. Verify, in this order, Rollback, Runbook: move the worker pool onto an isolated private network

### Community 59 - "dependencies"
Cohesion: 0.06
Nodes (32): dependencies, better-sqlite3, class-variance-authority, clsx, eslint, eslint-config-next, next, next-intl (+24 more)

### Community 60 - "scripts"
Cohesion: 0.06
Nodes (32): scripts, api:parity, api:parity:json, artifact:check, artifact:prepare, build, deploy:standalone, desktop:build (+24 more)

### Community 61 - "ref_fs"
Cohesion: 0.12
Nodes (19): ref_fs, ref_path, allowlistPatternSchema, allowlistUpdateSchema, approvalResponseSchema, computeHash(), execApprovalsPath(), gatewayUrl() (+11 more)

### Community 62 - "injection-guard.ts"
Cohesion: 0.15
Nodes (23): CONFUSABLES, decodeRot13(), detectRot13(), escapeHtml(), generateDecodingVariants(), GuardOptions, InjectionCategory, InjectionMatch (+15 more)

### Community 63 - "github/route.ts"
Cohesion: 0.24
Nodes (15): GET(), handleClose(), handleComment(), handleGitHubStats(), handleInitLabels(), handleStatus(), handleSync(), handleSyncProject() (+7 more)

### Community 64 - "fly-topology.test.ts"
Cohesion: 0.10
Nodes (15): Read this before touching anything, Implemented in this change, sleep(), enabled(), FlyMachineCreateRequest, FlyMachineResponse, FlyMachinesClient, FlyMachinesClientOptions (+7 more)

### Community 65 - "dashboard-cli-fleets.ts"
Cohesion: 0.19
Nodes (16): EngineLogoForText(), SessionWorkbenchRow(), SessionWorkbenchWidget(), data(), CLI_KIND_META, cliKindLabel(), cliKindMeta, CliSessionKind (+8 more)

### Community 66 - "exec-approval-panel.tsx"
Cohesion: 0.11
Nodes (22): emit(), getSnapshot(), listeners, Snapshot, subscribe(), usePermissionMode(), ExecApprovalOverlay(), formatRemaining() (+14 more)

### Community 67 - "channels-panel.tsx"
Cohesion: 0.12
Nodes (24): AccountList(), ActionResult, asRecord(), CardShell(), ChannelAccount, channelIsActive(), ChannelsSnapshot, ChannelStatus (+16 more)

### Community 68 - "chat-display.ts"
Cohesion: 0.20
Nodes (15): asFeed(), ChatHomeList(), feedTime(), HomeSessionRow, KIND_KEY, pillClass(), readJson(), useChatDesktopPrefs() (+7 more)

### Community 69 - "handoff/route.ts"
Cohesion: 0.08
Nodes (50): dynamic, executeHandoff(), HandoffRequest, makeSpec(), parseBody(), parsedHandoff(), POST(), readReply() (+42 more)

### Community 70 - "agent-comms-panel.tsx"
Cohesion: 0.09
Nodes (25): AggregateEvent, ActivitiesData, activitiesToFeed(), ActivityRecord, AGENT_IDENTITY, AgentCommsPanel(), CATEGORY_META, CommsData (+17 more)

### Community 71 - "requireRole"
Cohesion: 0.03
Nodes (155): GET(), DELETE(), POST(), GET(), PUT(), POST(), GET(), POST() (+147 more)

### Community 72 - "useSmartPoll"
Cohesion: 0.09
Nodes (30): @testing-library/react, FlySessionStatus(), useChatFlyJobs(), clearLegacyOrder(), readLegacyOrder(), useFolderOrder(), AuditTrailPanel(), FlyActivityTable() (+22 more)

### Community 73 - "mcp-audit.ts"
Cohesion: 0.14
Nodes (20): GET(), parsePositiveInteger(), logMcpCall(), McpCallInput, McpCallStats, verifyMcpCallReceipt(), verifyMcpCallReceipts(), canonicalize() (+12 more)

### Community 74 - "session-terminal-widget.tsx"
Cohesion: 0.16
Nodes (12): SessionTerminalWidget(), TerminalWidgetData, transcripts, DashboardSession, COLUMN_CLASS, gridTerminalSessions(), TERMINAL_GRID_LIMIT, TERMINAL_GRID_SNAPS (+4 more)

### Community 75 - "doctor/route.ts"
Cohesion: 0.08
Nodes (36): CachedDoctor, doctorCache, DoctorCacheModule, GET(), getCommandDetail(), invalidateDoctorCache(), isMissingOpenClaw(), POST() (+28 more)

### Community 76 - "mc-tui.cjs"
Cohesion: 0.05
Nodes (78): Mission Control desktop, ref_node_readline, normalizeMissionControlBaseUrl(), bodyFromFlags(), commands, ensureParentDir(), EXIT, fs (+70 more)

### Community 77 - "claude-fleet-plans.ts"
Cohesion: 0.31
Nodes (9): CostFleetPlans(), detectClaudeFleetPlans(), CLAUDE_FLEET_PLANS, ClaudeFleetAuthStatus, ClaudeFleetPlanDef, ClaudeFleetPlanIdentity, ClaudeFleetPlanStatus, claudeFleetPlanTotalUsd() (+1 more)

### Community 78 - "gnap-sync.ts"
Cohesion: 0.20
Nodes (21): GET(), POST(), getGnapStatus(), git(), GNAP_TO_MC_STATUS, gnapStatusToMc(), GnapTask, hasChanges() (+13 more)

### Community 79 - "jev-panel.tsx"
Cohesion: 0.06
Nodes (60): Frontend components, Answer(), humanize(), JevAnswerCards(), nextAction(), formatTime(), JevHistory(), JevPanel() (+52 more)

### Community 80 - "session-list-local.ts"
Cohesion: 0.24
Nodes (19): CLI_SESSION_SCAN_LIMIT, isForeignPrefixedSessionId(), getLocalGrokSessions(), getLocalKimiSessions(), mapEngineRow(), activityFields(), formatAge(), formatTokens() (+11 more)

### Community 81 - "skills-panel.tsx"
Cohesion: 0.15
Nodes (15): react-dom, getSourceLabel(), PanelTab, RegistryInstallResponse, RegistryResponse, RegistrySkill, skillApiMessage(), skillApiPayload() (+7 more)

### Community 82 - "scheduler.ts"
Cohesion: 0.15
Nodes (29): syncAgentsFromConfig(), syncClaudeSessions(), FLY_RECONCILE_INTERVAL_MS, Result, startFlyReconcileLoop(), Task, syncLocalAgents(), BACKUP_DIR (+21 more)

### Community 83 - "workload/route.ts"
Cohesion: 0.10
Nodes (27): AgentMetrics, buildAgentMetrics(), buildCapacityMetrics(), buildQueueMetrics(), buildThresholds(), CapacityMetrics, computeRecommendation(), escalate() (+19 more)

### Community 84 - "github-sync-engine.ts"
Cohesion: 0.16
Nodes (19): ALL_MC_LABELS, ALL_PRIORITY_LABEL_NAMES, ALL_STATUS_LABEL_NAMES, FLEET_AGENT_LABELS, LABEL_PRIORITY_MAP, LABEL_STATUS_MAP, LabelDef, labelToPriority() (+11 more)

### Community 85 - "layout.tsx"
Cohesion: 0.15
Nodes (16): Fixed in this pass, next-themes, sonner, src_app_globals, inter, jetbrainsMono, metadata, metadataBase (+8 more)

### Community 86 - "start-e2e-server.mjs"
Cohesion: 0.09
Nodes (18): ref_node_net, ref_node_process, baseEnv, buildIdPath, children, dataDir, fixtureSource, gatewayPort (+10 more)

### Community 87 - "gateways/connect/route.ts"
Cohesion: 0.19
Nodes (18): ensureTable(), GatewayEntry, getBrowserHostname(), inferBrowserProtocol(), isNonBrowserReachableHost(), LOCALHOST_HOSTS, POST(), resolveRemoteGatewayUrl() (+10 more)

### Community 88 - "onboarding/route.ts"
Cohesion: 0.19
Nodes (20): GET(), getOnboardingSetting(), ONBOARDING_SETTING_KEYS, OnboardingSettingKey, POST(), readUserOnboardingSetting(), scopedOnboardingKey(), setOnboardingSetting() (+12 more)

### Community 89 - "dashboard-board.ts"
Cohesion: 0.20
Nodes (22): ALLOWED_W, BoardItemKind, BoardState, canPlace(), clampItem(), compactUp(), firstFit(), MAX_H (+14 more)

### Community 90 - "fly/status/route.ts"
Cohesion: 0.14
Nodes (23): dynamic, GET(), buildFlyActivity(), flySubmissionStatus(), flyReadiness(), FlySubmission, submitFlyLeaf(), pricedFlyJob() (+15 more)

### Community 91 - "install.ps1"
Cohesion: 0.24
Nodes (19): Get-RandomHex(), Get-RandomPassword(), Get-Source(), New-EnvFile(), Deploy-Docker(), Deploy-Local(), Main(), Stop-WithError() (+11 more)

### Community 92 - "mc-provisioner-daemon.js"
Cohesion: 0.11
Nodes (24): {
  COMMAND_TIMEOUT_MS,
  IDLE_SOCKET_TIMEOUT_MS,
  MAX_CONNECTIONS,
  MAX_OUTPUT_BYTES,
  MAX_REQUEST_BYTES,
  appendBounded,
}, escapeRegExp(), { execFileSync, spawn }, fs, GATEWAY_SYSTEMD_TEMPLATE, isSafeHomePath(), isSafeUser(), net (+16 more)

### Community 93 - "models.ts"
Cohesion: 0.08
Nodes (30): zustand, Activity, Agent, ChatMessage, Comment, ConnectionStatus, Conversation, CronJob (+22 more)

### Community 94 - "skills/route.ts"
Cohesion: 0.06
Nodes (66): dynamic, PUT(), VALID_SOURCES, VALID_TARGETS, assertRealPathWithin(), auditSkillMutation(), collectSkillsFromDir(), DELETE() (+58 more)

### Community 95 - "graph/route.ts"
Cohesion: 0.23
Nodes (15): AgentFileInfo, AgentGraphData, GET(), getAgentData(), CatalogRow, GraphAgentData, GraphFileInfo, historyFiles() (+7 more)

### Community 96 - "fetchWithRetry"
Cohesion: 0.16
Nodes (15): compareSemver(), GET(), headers, compareSemver(), GET(), canRetryMethod(), FetchRetryPolicy, fetchWithRetry() (+7 more)

### Community 97 - "evals/route.ts"
Cohesion: 0.20
Nodes (18): GET(), POST(), checkDrift(), convergenceScore(), DriftResult, evalCorrectnessScore(), EvalLayer, evalReasoningCoherence() (+10 more)

### Community 98 - "super-admin.ts"
Cohesion: 0.08
Nodes (43): ref_net, GET(), POST(), POST(), GET(), POST(), POST(), GET() (+35 more)

### Community 99 - "proxy.ts"
Cohesion: 0.21
Nodes (16): buildMissionControlCsp(), buildNonceRequestHeaders(), addSecurityHeaders(), config, envFlag(), extractApiKeyFromRequest(), getImplicitAllowedHosts(), getRequestHostCandidates() (+8 more)

### Community 100 - "desktop/package.json"
Cohesion: 0.10
Nodes (18): description, devDependencies, electron, engines, node, main, name, packageManager (+10 more)

### Community 101 - "agent-runtimes.ts"
Cohesion: 0.08
Nodes (39): GET(), POST(), VALID_MODES, VALID_RUNTIMES, DeploymentMode, detectAllRuntimes(), detectBinary(), detectCodex() (+31 more)

### Community 102 - "Jev Workspace Product Requirements"
Cohesion: 0.17
Nodes (11): Backend components, Deferred batch scope, Information architecture, Jev Workspace Product Requirements, Outcome, Primary flow, Question behavior, Repository context (+3 more)

### Community 103 - "runtime-history.ts"
Cohesion: 0.17
Nodes (16): asNumber(), asObject(), asString(), clampTimestamp(), GrokSessionStats, listSummaryFiles(), parseSummary(), scanGrokSessions() (+8 more)

### Community 104 - "local-session-list.ts"
Cohesion: 0.29
Nodes (14): flagsFor(), formatAge(), formatTokens(), fromClaudeRow(), fromScanner(), listNormalizedLocalSessions(), formatAge(), formatTokens() (+6 more)

### Community 105 - "Mission Control Style Guide"
Cohesion: 0.07
Nodes (29): 10. Accessibility, 1. Design Philosophy, 2. Color System, 3. Typography, 4. Spacing, 5. Border Radius, 6. Elevation & Glow, 7. Component Patterns (+21 more)

### Community 106 - "schema.sql"
Cohesion: 0.13
Nodes (27): activities, agents, comments, gateway_health_logs, idx_activities_created_at, idx_activities_type, idx_agents_session_key, idx_agents_status (+19 more)

### Community 107 - "install.sh"
Cohesion: 0.33
Nodes (17): check_openclaw(), check_prerequisites(), command_exists(), detect_os(), die(), err(), fetch_source(), info() (+9 more)

### Community 108 - "runCommand"
Cohesion: 0.16
Nodes (16): ref_node_events, cpuTotals(), GET(), getCpuSnapshot(), getDiskSnapshot(), getGpuSnapshot(), getMemorySnapshot(), getNetworkSnapshot() (+8 more)

### Community 109 - "recurring-tasks.ts"
Cohesion: 0.18
Nodes (13): GET(), formatDateSuffix(), isSubDailyCron(), MONTHS, RecurrenceMetadata, spawnRecurringTasks(), DAY_MAP, isCronDue() (+5 more)

### Community 110 - "tokens/route.ts"
Cohesion: 0.15
Nodes (24): AgentBreakdown, AgentBreakdownRow, GET(), ModelBreakdown, calculateStats(), DbTokenUsageRow, dedupeTokenRecords(), deriveFromSessions() (+16 more)

### Community 111 - "gateways/health/route.ts"
Cohesion: 0.24
Nodes (12): BLOCKED_HOSTNAMES, BLOCKED_PRIVATE_CIDRS, buildGatewayProbeUrl(), ensureGatewaysTable(), GatewayEntry, hasOpenClaw32ToolsProfileRisk(), HealthResult, ipv4InCidr() (+4 more)

### Community 112 - "api-contract-parity.ts"
Cohesion: 0.20
Nodes (17): collectOpenApiOperations(), collectRouteOperations(), compareApiContractParity(), ContractOperation, extractHttpMethods(), HTTP_METHODS, loadOpenApiFile(), normalizeOperation() (+9 more)

### Community 113 - "cli-inventory.ts"
Cohesion: 0.27
Nodes (14): GET(), AgentInventory, inventoryForAgent(), inventoryLooksSafe(), runtimeOf(), jsonKeys(), listClaudeMcp(), listCodexAutomations() (+6 more)

### Community 114 - "device-identity.ts"
Cohesion: 0.20
Nodes (18): cacheDeviceToken(), clearDeviceIdentity(), createNewIdentity(), DeviceIdentity, fromBase64Url(), getCachedDeviceToken(), getOrCreateDeviceIdentity(), idbDeleteKey() (+10 more)

### Community 115 - "compilerOptions"
Cohesion: 0.11
Nodes (18): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+10 more)

### Community 116 - "chat-session-pane.tsx"
Cohesion: 0.07
Nodes (35): react-markdown, remark-gfm, IconClose(), OverlaySessionView(), dismissKey(), HandoffBanner(), looksReal(), getMeta() (+27 more)

### Community 117 - "devDependencies"
Cohesion: 0.11
Nodes (18): devDependencies, fast-check, jsdom, pino-pretty, @playwright/test, @testing-library/dom, @testing-library/jest-dom, @testing-library/react (+10 more)

### Community 118 - "framework-templates.ts"
Cohesion: 0.26
Nodes (14): dynamic, GET(), listAdapters(), AGENT_TEMPLATES, FRAMEWORK_REGISTRY, FrameworkConnectionConfig, FrameworkInfo, getFrameworkInfo() (+6 more)

### Community 119 - "memory-browser-panel.tsx"
Cohesion: 0.08
Nodes (25): reagraph, countFiles(), fileIcon(), formatFileSize(), HealthCategory, HealthReport, HealthView(), log (+17 more)

### Community 120 - "agent-templates.ts"
Cohesion: 0.12
Nodes (17): AgentIdentityConfig, AgentMemorySearchConfig, AgentModelConfig, AgentSandboxConfig, AgentSubagentsConfig, AgentTemplate, AgentToolsConfig, COMMON_DENY (+9 more)

### Community 121 - "flight-deck/route.ts"
Cohesion: 0.40
Nodes (9): DEFAULT_INSTALL_PATHS, dynamic, GET(), getConfiguredFlightDeckPath(), getFlightDeckBaseUrl(), getFlightDeckLaunchUrl(), isInstalled(), POST() (+1 more)

### Community 122 - "react"
Cohesion: 0.04
Nodes (65): next-intl, react, log, Props, State, FeedItem(), formatRelativeTime(), UpdateState (+57 more)

### Community 123 - "runs.ts"
Cohesion: 0.09
Nodes (31): GET(), GET(), POST(), PUT(), GET(), GET(), PATCH(), AgentRun (+23 more)

### Community 124 - "pty-websocket.ts"
Cohesion: 0.13
Nodes (18): ref_http, http, path, PORT, server, standaloneDir, getPtySession(), handlePtyUpgrade() (+10 more)

### Community 125 - "jev-cloud.test.ts"
Cohesion: 0.15
Nodes (19): Db, getJevCloudQueueStatus(), JevCloudEvent, jsonFields, prepareJevCloudPayload(), sources, Db, running (+11 more)

### Community 126 - "conversation-list.tsx"
Cohesion: 0.16
Nodes (16): asRecord(), COLOR_OPTIONS, ConversationList(), renderConversationItem(), ConversationListProps, log, readNumber(), readSessionPrefs() (+8 more)

### Community 127 - "nodes-panel.tsx"
Cohesion: 0.17
Nodes (15): deviceAction(), DeviceTokenSummary, InstancesTab(), NodesPanel(), PairedDevice, PairedDevicesSection(), handleRevokeToken(), handleRotateToken() (+7 more)

### Community 128 - "opencode-sessions.ts"
Cohesion: 0.21
Nodes (15): detectOpenCode(), asNumber(), asObject(), asString(), getOpenCodeBinaryCandidates(), getOpenCodeVersion(), isOpenCodeInstalled(), OpenCodeMessageRow (+7 more)

### Community 129 - "artifact/route.ts"
Cohesion: 0.27
Nodes (8): allowedFile(), dynamic, GET(), SessionArtifactPanel(), SESSION_ID_RE, extractSessionArtifacts(), latestArtifact(), SessionArtifact

### Community 130 - "fly-reconciler.ts"
Cohesion: 0.15
Nodes (19): flySubmissionSchema, SubmissionRow, FLY_LAUNCH_BATCH_SIZE, flyCapacity(), flyLaunchRegions(), flyWorkerLimit(), parseResult(), polledResultSchema (+11 more)

### Community 131 - "claude-transcript.ts"
Cohesion: 0.24
Nodes (13): findClaudeTranscriptFile(), keepLastArtifact(), parseClaudeTranscriptLines(), parseUser(), assistantParts(), attachToolResults(), dedupeConsecutiveText(), rec() (+5 more)

### Community 132 - "session-transcript-types.ts"
Cohesion: 0.22
Nodes (17): SessionMessageProps, asObject(), findSessionDir(), isoFromUnknown(), readGrokTranscript(), asObject(), findSessionDir(), isoFromTime() (+9 more)

### Community 133 - "claude-code-sessions.ts"
Cohesion: 0.28
Nodes (9): AgentSessionRow, assertSessionOwnedByAgent(), ClaudeBaseSession, ensureClaudeBaseSession(), isValidClaudeSessionId(), markClaudeBaseSessionMaterialized(), readAgentSessionRow(), rotateClaudeBaseSession() (+1 more)

### Community 134 - "ref_node_module"
Cohesion: 0.13
Nodes (9): ref_node_module, {
  MAX_BASE_URL_LENGTH,
  normalizeMissionControlBaseUrl,
}, require, require, config, require, {
  COMMAND_TIMEOUT_MS,
  MAX_OUTPUT_BYTES,
  appendBounded,
}, require (+1 more)

### Community 135 - "codex-transcript.ts"
Cohesion: 0.26
Nodes (11): contentParts(), findCodexTranscriptFile(), parseCodexTranscriptLines(), readCodexTranscript(), rec(), DEFAULT_TAIL_BYTES, findNamedJsonl(), isSessionFile() (+3 more)

### Community 136 - "ref_node_assert"
Cohesion: 0.14
Nodes (15): isLoginPage(), configureRequestCookies(), isBackendRequest(), withoutHeader(), ref_node_assert, ref_node_test, CHECKS, loadJob() (+7 more)

### Community 137 - "session-transcript-route.ts"
Cohesion: 0.21
Nodes (14): dynamic, GET(), epochSecondsToISO(), HermesMessageRow, pushToolCalls(), readHermesTranscript(), readHermesTranscriptFromDbPath(), toolName() (+6 more)

### Community 138 - "session-archive.ts"
Cohesion: 0.18
Nodes (21): projectSlugOf(), archiveListedSessions(), archivePath(), archiveSessionMeta(), archiveSessionTranscript(), ArchivedSessionRow, ensureSessionTranscriptsTable(), indexSessionArchives() (+13 more)

### Community 139 - "rail-width.ts"
Cohesion: 0.29
Nodes (11): ChatShell(), HandleProps, useRailResize(), clampRailWidth(), RAIL_DEFAULT, RAIL_MAIN_MIN, RAIL_MAX, RAIL_MIN (+3 more)

### Community 140 - "webhook-panel.tsx"
Cohesion: 0.17
Nodes (15): AVAILABLE_EVENTS, CreateWebhookForm(), Delivery, isWebhookTransportFailure(), SchedulerResult, SchedulerTask, Webhook, webhookErrorPayload() (+7 more)

### Community 141 - "ref_node_crypto"
Cohesion: 0.21
Nodes (13): ref_node_crypto, flyBudgetPeriods(), flyCommittedSpend(), flyFairOrder(), FlyQueuedRow, flyQueueIsContended(), flyRepositoryDailyCap(), flyRepositorySpendToday() (+5 more)

### Community 142 - "session-continue-run.ts"
Cohesion: 0.14
Nodes (25): dynamic, POST(), mocks, getOpenCodeExecutable(), parsePermissionMode(), permissionArgv(), continueEffort(), continueModelId() (+17 more)

### Community 143 - "websocket-utils.ts"
Cohesion: 0.29
Nodes (12): buildProtocolNegotiation(), calculateBackoff(), calculateReconnectDelay(), ConnectErrorDetailCodes, detectSequenceGap(), GatewayErrorDetail, isNonRetryableErrorCode(), NON_RETRYABLE_ERROR_CODES (+4 more)

### Community 144 - "mc-fly-contract.test.cjs"
Cohesion: 0.30
Nodes (11): retryRead(), assert, { createFlyTools }, { test }, { validateAdmission, validateStatus, validateCancellation, retryRead }, validateAdmission(), validateCancellation(), validateStatus() (+3 more)

### Community 145 - "agent-filesystem-isolation.test.ts"
Cohesion: 0.16
Nodes (12): assertStatus(), baseUrl, call(), headers, run(), authUser(), denyUnscopedResourceMock, getAgentWorkspaceCandidatesMock (+4 more)

### Community 146 - "provider-subscriptions.ts"
Cohesion: 0.14
Nodes (23): detectModelAccess(), envSet(), detectAnthropicFromFile(), detectFromEnv(), detectOpenAIFromFile(), detectProviderSubscriptions(), findNestedString(), getPrimarySubscription() (+15 more)

### Community 147 - "Troubleshooting"
Cohesion: 0.05
Nodes (38): Agent Registration, Architecture, AUTH_PASS with "#" is not working, Automatic backups, Database locked errors, Deployment Guide, Direct API dispatch (gateway-free), Docker: gateway unreachable / WebSocket not connecting (+30 more)

### Community 148 - "agent-sync.ts"
Cohesion: 0.07
Nodes (48): applyConfig(), computeHash(), gatewayHeaders(), gatewayUrl(), GET(), getConfigPath(), getSchema(), PUT() (+40 more)

### Community 149 - "coordinator-routing.ts"
Cohesion: 0.35
Nodes (11): CoordinatorAgentRecord, findSessionForAgent(), getConfigIsDefault(), getConfigOpenClawId(), normalizeName(), normalizeOpenClawId(), parseConfig(), resolveConfiguredCoordinatorTarget() (+3 more)

### Community 150 - "local-agent-sync.ts"
Cohesion: 0.21
Nodes (13): AgentFrontmatter, AgentRow, ALL_MARKERS, CONFIG_FILES, DiskAgent, extractRole(), getLocalAgentRoots(), IDENTITY_FILES (+5 more)

### Community 151 - "mc-fly-cli.test.cjs"
Cohesion: 0.17
Nodes (10): assert, fs, os, path, { runFlyCli }, test, fs, { handleMessage } (+2 more)

### Community 152 - "notification-daemon.sh"
Cohesion: 0.40
Nodes (12): check_daemon(), check_mission_control(), cleanup(), deliver_notifications(), get_delivery_stats(), log(), main(), parse_args() (+4 more)

### Community 153 - "adapter-compliance.test.ts"
Cohesion: 0.18
Nodes (10): getAdapter(), mockBroadcast, mockQuery, simulateAdapterAction(), ALL_FRAMEWORKS, mockBroadcast, mockQuery, testAgent (+2 more)

### Community 154 - "[id]/diagnostics/route.ts"
Cohesion: 0.27
Nodes (12): ALLOWED_SECTIONS, buildActivityBreakdown(), buildErrorAnalysis(), buildSummary(), buildTaskMetrics(), buildTokenMetrics(), buildTrendAlerts(), buildTrends() (+4 more)

### Community 155 - "pipeline-tab.tsx"
Cohesion: 0.11
Nodes (16): Agent, emptyForm, isOrchestrationNetworkFailure(), OrchestrationBar(), orchestrationError(), TemplateFormData, WorkflowTemplate, isPipelineNetworkFailure() (+8 more)

### Community 156 - "rate-limit.ts"
Cohesion: 0.04
Nodes (66): dynamic, GET(), POST(), dynamic, POST(), VALID_ROLES, POST(), POST() (+58 more)

### Community 157 - "message-bubble.tsx"
Cohesion: 0.25
Nodes (11): AGENT_COLORS, asRecord(), formatTime(), getAgentTheme(), MessageBubble(), MessageBubbleProps, renderContent(), ToolCallBubble() (+3 more)

### Community 158 - "chat-fly-jobs.ts"
Cohesion: 0.36
Nodes (6): ChatSessionFlyJobs(), flyJobsComplete(), jobsForSession(), SessionFlyJob, sessionMatchesFly(), withSessionFlyJobs()

### Community 159 - "openclaw-doctor-banner.tsx"
Cohesion: 0.23
Nodes (8): BannerState, OpenClawDoctorBanner(), OpenClawDoctorFixProgress, OpenClawDoctorStatus, cleanLine(), comparisonKey(), DoctorBannerCopy, prepareDoctorBannerCopy()

### Community 160 - "alert-evaluate.ts"
Cohesion: 0.31
Nodes (10): AlertRule, compareValue(), countThreshold(), evaluateAgentRule(), evaluateAllRules(), evaluateCountRule(), evaluateRule(), SAFE_COLUMNS (+2 more)

### Community 161 - "security-scan-card.tsx"
Cohesion: 0.13
Nodes (15): Category, CATEGORY_LABELS, Check, CheckSeverity, FIX_SAFETY, FIXABLE_IDS, FixResponse, FixSafety (+7 more)

### Community 162 - "codex-sessions.ts"
Cohesion: 0.29
Nodes (12): asNumber(), asObject(), asString(), clampTimestamp(), CodexSessionStats, deriveSessionId(), extractCodexUserText(), listRecentCodexSessionFiles() (+4 more)

### Community 163 - "kimi-sessions.ts"
Cohesion: 0.26
Nodes (14): asObject(), asString(), clampTimestamp(), isKimiClawPath(), KimiSessionStats, parseIndexLine(), parseState(), scanKimiSessions() (+6 more)

### Community 164 - "package-manager-patches/package.json"
Cohesion: 0.17
Nodes (11): brace-expansion, ip-address, tar, dependencies, brace-expansion, ip-address, tar, description (+3 more)

### Community 165 - "dashboard-cli-fleets.test.ts"
Cohesion: 0.15
Nodes (16): GET(), buildDashboardView(), getLocalOsStatus(), getMcHealth(), CLI_SESSION_KINDS, localSessionLogs(), mergeRecentLogs(), SessionLogLike (+8 more)

### Community 166 - "fleet-agents.ts"
Cohesion: 0.23
Nodes (12): Recovery archive, 2026-09-07, Restoring from the archive, The two entries that are not simply superseded, What the archive holds, SessionFilterBar(), SessionFilterState, asFleetAgentName(), canonicalFleetAgentName() (+4 more)

### Community 167 - "Orchestration Patterns"
Cohesion: 0.08
Nodes (25): API, Capacity Control, CLI, Combining Patterns, Event Streaming, Example: Research → Implement → Review, Flow, How Aegis Reviews (+17 more)

### Community 168 - "handoff-brief.ts"
Cohesion: 0.38
Nodes (8): buildHandoffBrief(), HANDOFF_BRIEF_MAX, prsOf(), textOf(), toolsOf(), filesTouched(), redactSecrets(), clipExcerpt()

### Community 169 - "hook-profiles.ts"
Cohesion: 0.30
Nodes (10): getActiveProfile(), getRateLimitMultiplier(), HookProfile, HookProfileLevel, PROFILES, shouldAuditMcpCalls(), shouldBlockOnSecretDetection(), shouldScanSecrets() (+2 more)

### Community 171 - "fleetMemoryRoots"
Cohesion: 0.27
Nodes (16): POST(), fileStem(), consolidateFleet(), gapDetectFleet(), fleetMemoryRoots(), collectCatalogStems(), collectFleetWikiStems(), HUB_STEMS (+8 more)

### Community 172 - "check-api-contract-parity.mjs"
Cohesion: 0.35
Nodes (10): extractHttpMethods(), HTTP_METHODS, normalizeOperation(), normalizeSegment(), parseArgs(), parseIgnoreArg(), routeFileToApiPath(), run() (+2 more)

### Community 173 - "api/health/route.ts"
Cohesion: 0.27
Nodes (6): GET(), GET, STANDALONE_BRAND_LOGO, standaloneReleaseIntact(), { getDatabaseMock, standaloneReleaseIntactMock }, dirs

### Community 174 - "gateway-runtime.ts"
Cohesion: 0.31
Nodes (11): dashboardOriginAliases(), getDetectedGatewayToken(), OpenClawGatewayConfig, readOpenClawConfig(), registerMcAsDashboard(), cachedGatewayToken(), isUsableGatewayToken(), readSecretRefId() (+3 more)

### Community 175 - "channels/route.ts"
Cohesion: 0.20
Nodes (16): commandStdout(), errorMessage(), gatewayHeaders(), GET(), isGatewayReachable(), loadChannelsViaCli(), POST(), asRecord() (+8 more)

### Community 176 - "Everything Included In The v2 Update"
Cohesion: 0.08
Nodes (24): Agent lifecycle and operator UX, Agents, sessions, chat, and comms, Broader agent observability, Core platform and mode handling, Deployment and runtime hardening, Detailed Fix Areas, Detailed Functional Additions, Dual-mode operation (+16 more)

### Community 178 - "unlock-renderer.mjs"
Cohesion: 0.22
Nodes (9): accountButton, message, messages, pinForm, pinInput, separator, setBusy(), touchButton (+1 more)

### Community 179 - "mcp-server.spec.ts"
Cohesion: 0.24
Nodes (9): Next steps, ref_node_util, CLI, execFileAsync, mc(), MCP, mcpCall(), mcpRequest() (+1 more)

### Community 180 - "dashboard-board-persist.ts"
Cohesion: 0.19
Nodes (16): BOARD_COLS, BoardItem, emptyBoard(), BOARD_SETTING_KEY, BOARD_STORAGE_KEY, emptyOr(), isItem(), LEGACY_LAYOUT_KEY (+8 more)

### Community 181 - "terminal-view.tsx"
Cohesion: 0.13
Nodes (11): getLayoutMode(), LAYOUT_CLASSES, LayoutMode, SplitPane, SplitPaneLayout(), SplitPaneLayoutProps, TerminalToolbar(), TerminalToolbarProps (+3 more)

### Community 182 - "readLimiter"
Cohesion: 0.13
Nodes (35): GET(), GET(), POST(), POST(), Context, DELETE(), PATCH(), GET() (+27 more)

### Community 183 - "session-terminal-cell.tsx"
Cohesion: 0.17
Nodes (22): SessionTerminalCell(), terminalCellTitle(), transcriptSubject(), LINE_CLASS, SessionTerminalView(), workingDirLeaf(), normalizeCliKind(), isAgentWorking() (+14 more)

### Community 184 - "memory/health/route.ts"
Cohesion: 0.21
Nodes (13): GET(), mergeReports(), classifyMemoryPath(), extractFrontmatterBlock(), hasDiscoverabilityField(), isHealthScored(), MemoryFileClass, runHealthDiagnostics() (+5 more)

### Community 185 - "pty-manager.ts"
Cohesion: 0.18
Nodes (16): ref_child_process, node-pty, shutdown(), GET(), POST(), createPtySession(), disposeAllPtySessions(), disposePtySession() (+8 more)

### Community 187 - "agent-heartbeat.sh"
Cohesion: 0.50
Nodes (8): check_agent_heartbeat(), check_mission_control(), get_agent_session_key(), get_agents_to_check(), log(), main(), send_wake_notification(), agent-heartbeat.sh script

### Community 188 - "health-utils.test.ts"
Cohesion: 0.28
Nodes (5): BLOCKED_HOSTNAMES, BLOCKED_PRIVATE_CIDRS, ipv4InCidr(), ipv4ToNum(), isBlockedUrl()

### Community 189 - "fleet-projects.ts"
Cohesion: 0.24
Nodes (11): FLEET_AGENT_NAMES, fleetProjectSlug(), matchFleetProject(), PATH_ALIASES, specByName(), tildePath(), allSeedProjects(), DISCOVERED_PROJECTS (+3 more)

### Community 190 - "attention-detector.ts"
Cohesion: 0.22
Nodes (5): ATTENTION_PATTERNS, AttentionLevel, DetectionResult, ERROR_PATTERNS, lastDetection

### Community 191 - "fly-stale-controller.ts"
Cohesion: 0.39
Nodes (4): defaultProbe, flyControllerArtifactPath(), flyControllerIsStale(), FlyStaleProbe

### Community 192 - "permission-mode/route.ts"
Cohesion: 0.25
Nodes (13): dynamic, GET(), PUT(), readOrder(), canSetMode(), dynamic, GET(), PUT() (+5 more)

### Community 193 - "openclaw-gateway.ts"
Cohesion: 0.15
Nodes (17): ws, buildGatewayPathFallbackUrls(), buildGatewayWebSocketUrl(), formatWebSocketUrl(), isLocalHost(), normalizeGatewayPath(), normalizeProtocol(), preserveTokenQuery() (+9 more)

### Community 194 - "task-costs.ts"
Cohesion: 0.22
Nodes (13): TaskMetadataRow, AgentTaskCostEntry, buildTaskCostReport(), buildTimeline(), calculateStats(), formatTicketRef(), groupByModel(), ProjectTaskCostEntry (+5 more)

### Community 195 - "check-standalone-artifact.mjs"
Cohesion: 0.25
Nodes (7): allowedRoots, allowedSourceFiles, failures, forbiddenNames, requiredFiles, root, walk()

### Community 196 - "deploy-standalone.sh"
Cohesion: 0.46
Nodes (7): list_listener_pids(), load_env(), migrate_runtime_data_dir(), deploy-standalone.sh script, stop_existing_server(), stop_pid(), use_project_node()

### Community 197 - "github.ts"
Cohesion: 0.19
Nodes (17): POST(), slugify(), createIssue(), createLabel(), createPullRequest(), createRef(), ensureLabels(), fetchIssue() (+9 more)

### Community 198 - "Mission Control Hardening"
Cohesion: 0.17
Nodes (12): 10. Data Retention, 11. Runtime Installer Integrity, 1. Credentials, 2. Network Access Control, 3. HTTPS & Cookies, 4. Rate Limiting, 5. Docker Hardening, 6. Security Headers (+4 more)

### Community 199 - "task-routing.ts"
Cohesion: 0.36
Nodes (6): isNonEmptyString(), parseMetadata(), resolveTaskImplementationTarget(), TaskImplementationTarget, TaskLike, TaskMetadata

### Community 200 - "chat-permission-mode-route.test.ts"
Cohesion: 0.25
Nodes (6): auditMock, isolationMock, limiterMock, loadMock, requireRoleMock, saveMock

### Community 201 - "chat/messages/route.ts"
Cohesion: 0.27
Nodes (12): ChatAttachmentInput, createChatReply(), extractReplyText(), extractToolEvents(), ForwardInfo, GET(), normalizeToolEvent(), parseGatewayJson() (+4 more)

### Community 202 - "tasks-route-noop-update.test.ts"
Cohesion: 0.25
Nodes (7): currentTask, getMock, mutationLimiterMock, normalizeTaskUpdateStatusMock, prepareMock, requireRoleMock, validateBodyMock

### Community 203 - "helpers.ts"
Cohesion: 0.15
Nodes (11): createTestAlert(), createTestProject(), createTestUser(), createTestWebhook(), createTestWorkflow(), deleteTestAlert(), deleteTestProject(), deleteTestUser() (+3 more)

### Community 204 - "typesafe-probe.ts"
Cohesion: 0.27
Nodes (6): getJevHealth(), JevHealth, mocks, hasProbeAnswer(), probeTypeSafeApiKey(), TypeSafeProbeResult

### Community 205 - "security-audit.sh"
Cohesion: 0.52
Nodes (6): fail(), info(), pass(), security-audit.sh script, trim_env_value(), warn()

### Community 206 - "github-activity.ts"
Cohesion: 0.15
Nodes (20): asEvent(), collectGitHubActivity(), fetchRepoEvents(), firstCommitMessage(), GitHubEventLike, GitHubFeedKind, keepNewer(), loadEvents() (+12 more)

### Community 207 - "Mission Control"
Cohesion: 0.10
Nodes (21): Agents and runtimes, Architecture, CLI, Connect an agent, Develop, Documentation, License, MCP server (+13 more)

### Community 208 - "jev-client.ts"
Cohesion: 0.13
Nodes (23): createClient(), evaluateWithJev(), exactKeys(), JEV_DEFAULT_MODEL, JEV_SDK_VERSION, JevClientError, JevClientOptions, listJevModels() (+15 more)

### Community 209 - "hermes-sessions.ts"
Cohesion: 0.23
Nodes (13): detectHermes(), installHermesLocal(), clearHermesDetectionCache(), epochSecondsToISO(), getHermesDbPath(), getHermesPidPath(), hasHermesCliBinary(), HermesSessionRow (+5 more)

### Community 210 - "optimize/route.ts"
Cohesion: 0.33
Nodes (9): GET(), analyzeTokenEfficiency(), analyzeToolPatterns(), FleetBenchmark, generateRecommendations(), getFleetBenchmarks(), Recommendation, TokenEfficiency (+1 more)

### Community 211 - "releases/update/route.ts"
Cohesion: 0.29
Nodes (8): EXEC_OPTS, git(), log, pnpm(), POST(), releaseUpdateLimiter, normalizeReleaseTag(), releaseUpdateSchema

### Community 212 - "aggregate/route.ts"
Cohesion: 0.20
Nodes (14): dynamic, GET(), partToEvent(), GET(), extractAgentName(), GATEWAY_KEY_RE, isSilentReplyText(), MessageContentPart (+6 more)

### Community 213 - "migrations.ts"
Cohesion: 0.14
Nodes (14): desktopBrowserLoginMigration, flyAdmissionMigration, flyRepairMigration, flyTopologyMigration, jevAssistantMigration, entities, jevCloudMigration, jevMigration (+6 more)

### Community 214 - "generate-env.sh"
Cohesion: 0.53
Nodes (4): generate_hex(), generate_password(), portable_sed(), generate-env.sh script

### Community 215 - "mac-safe-reclaim"
Cohesion: 0.67
Nodes (5): mac-safe-reclaim script, busy(), clear_dir(), is_protected(), protected_from_lsof()

### Community 216 - "start-standalone.sh"
Cohesion: 0.40
Nodes (5): HOSTNAME, MC_PROCESS_NAME, MISSION_CONTROL_DATA_DIR, reap_previous_controller(), start-standalone.sh script

### Community 217 - "station-doctor.sh"
Cohesion: 0.60
Nodes (5): fail(), info(), pass(), station-doctor.sh script, warn()

### Community 218 - "index/route.ts"
Cohesion: 0.33
Nodes (4): Endpoint, endpoints, payload, revalidate

### Community 219 - "Mission Control Platform Hardening + Full Agent CLI/TUI PRD"
Cohesion: 0.11
Nodes (18): 10) Rollout plan, 11) Risks and mitigations, 12) Acceptance criteria, 13) Immediate implementation tasks (next 1-2 PRs), 1) Problem statement, 2) Product objectives, 3) Personas and workflows, 4) Functional requirements (+10 more)

### Community 220 - "Mission Control Agent Skill"
Cohesion: 0.11
Nodes (19): status(), 1. Register, 2. Heartbeat, 3. Fetch Assignments, 4. Report Task Progress, 5. Disconnect, Adapters — `/api/adapters`, Agent Lifecycle (+11 more)

### Community 221 - "os-users/route.ts"
Cohesion: 0.15
Nodes (18): ref_os, checkToolExists(), discoverOsUsers(), GET(), installToolForUser(), OsUser, POST(), SERVICE_ACCOUNTS (+10 more)

### Community 222 - "minimax.ts"
Cohesion: 0.33
Nodes (7): getMiniMaxApiKey(), MINIMAX_REGIONS, MiniMaxEnvironment, MiniMaxProtocol, MiniMaxRegion, resolveMiniMaxEndpoint(), callMiniMaxDirectly()

### Community 223 - "attribution/route.ts"
Cohesion: 0.40
Nodes (9): ALLOWED_SECTIONS, buildAuditTrail(), buildCostAttribution(), buildIdentity(), buildMutations(), GET(), parseHours(), parseSections() (+1 more)

### Community 224 - "jev-validation.ts"
Cohesion: 0.10
Nodes (24): @typesafe-ai/sdk, contextModeSchema, enforcementSchema, failureModeSchema, jevPolicyConfigurationSchema, rolloutSchema, scopeSchema, triggerSchema (+16 more)

### Community 225 - "docs-knowledge.ts"
Cohesion: 0.28
Nodes (14): GET(), allowedRoots(), buildTreeFrom(), DOC_ROOT_CANDIDATES, DocsTreeNode, getDocsTree(), isDocsPathAllowed(), isSearchable() (+6 more)

### Community 226 - "process-title.test.sh"
Cohesion: 0.60
Nodes (3): fail(), process-title.test.sh script, spills()

### Community 227 - "Agent Setup Guide"
Cohesion: 0.10
Nodes (20): Agent Configuration, Agent Registration, Agent Setup Guide, Agent Sources, Agent Status Values, Agent Templates, Example: Developer Agent, Example: Researcher Agent (+12 more)

### Community 228 - "locale-parity.test.ts"
Cohesion: 0.31
Nodes (6): defaultLocale, Locale, localeNames, locales, flattenKeys(), Messages

### Community 229 - "NavRail"
Cohesion: 0.25
Nodes (6): NavRail(), filterItems(), flattenItems(), tLabel(), translateItems(), getPluginNavItems()

### Community 230 - "token-utils.test.ts"
Cohesion: 0.70
Nodes (3): applyTimezoneOffset(), detectProvider(), generateCsvContent()

### Community 232 - "load-env.sh"
Cohesion: 0.67
Nodes (3): load_env_file(), load-env.sh script, trim_env_field()

### Community 234 - "gateways/control/route.ts"
Cohesion: 0.18
Nodes (15): dynamic, formatCommandOutput(), GatewayStatus, GatewayType, GET(), getHermesGatewayStatus(), getOpenClawGatewayStatus(), isDockerEnvironment() (+7 more)

### Community 235 - "chat-folder-order-route.test.ts"
Cohesion: 0.25
Nodes (6): isolationMock, limiterMock, loadMock, operator, requireRoleMock, saveMock

### Community 237 - "Command groups"
Cohesion: 0.12
Nodes (17): agents, API contract parity gate, auth, Command groups, connect, cron, events, Exit code contract (+9 more)

### Community 238 - "Mission Control"
Cohesion: 0.12
Nodes (15): Agent Control Interfaces, CLI, Common Pitfalls, Conventions, Data Directory, Docker, Key Directories, MCP Server (recommended for agents) (+7 more)

### Community 239 - "memory-search.ts"
Cohesion: 0.18
Nodes (19): GET(), POST(), rebuildFleetMemoryIndex(), searchFleetMemory(), ensureFtsTable(), ensureIndex(), extractTitle(), indexFile() (+11 more)

### Community 240 - "repo-health-tasks.ts"
Cohesion: 0.48
Nodes (5): FleetProjectSpec, expandHome(), gitCount(), inspectRepo(), seedRepoHealthTasks()

### Community 244 - "Findings"
Cohesion: 0.12
Nodes (15): Alternatives and consequences, Concrete consolidation sequence, Decision, Findings, High: incompatible native SQLite binaries prevented web startup, High: the desktop repository is not backed by a Git remote, High: two backend processes can run background jobs against one database, How the two applications connect (+7 more)

### Community 249 - "plugins.ts"
Cohesion: 0.12
Nodes (9): _categories, _integrations, _navItems, _panels, PluginCategory, PluginIntegrationDef, PluginNavItem, PluginToolProvider (+1 more)

### Community 250 - "token-runtime-isolation.test.ts"
Cohesion: 0.29
Nodes (6): getAllGatewaySessionsMock, getWorkspaceIsolationMock, prepareMock, requireRoleMock, state, strictUser

### Community 254 - "Full Changelog"
Cohesion: 0.14
Nodes (13): Added, Better first-run setup, Broader self-hosted platform support, Changed, Contributors, Fixed, Full Changelog, Highlights (+5 more)

### Community 255 - "3. Key Hardening Recommendations"
Cohesion: 0.14
Nodes (13): 1. Core Security Model & Deployment Considerations, 2. Hardened Baseline Configuration, 3.1. Network Security, 3.2. Authentication and Access Control, 3.3. Isolation and Sandboxing, 3.4. Credential and Secret Management, 3.5. File System Permissions, 3.6. Tool and Skill Security (+5 more)

### Community 256 - "API Endpoints"
Cohesion: 0.14
Nodes (13): Agent Management, API Endpoints, Automated Health Monitoring, Check for Updates, Common Workflows, Database Backup, Diagnostics (Admin Only), Health Check (+5 more)

### Community 257 - "task-dispatch-sandbox.test.ts"
Cohesion: 0.16
Nodes (14): clampCliMaxBudgetUsd(), CLAUDE_CLI_ALLOWED_TOOL_NAMES, CLI_MAX_BUDGET_USD_CEILING, filterCliAllowedTools(), resolveCliDispatchCwd(), resolveCliSandboxOptions(), safeParseMetadata(), base (+6 more)

### Community 258 - "Contributor Covenant Code of Conduct"
Cohesion: 0.15
Nodes (12): 1. Correction, 2. Warning, 3. Temporary Ban, 4. Permanent Ban, Attribution, Contributor Covenant Code of Conduct, Enforcement, Enforcement Guidelines (+4 more)

### Community 259 - "mock-gateway.mjs"
Cohesion: 0.33
Nodes (4): ref_node_http, port, server, wss

### Community 260 - "gen-star-history.py"
Cohesion: 0.29
Nodes (9): datetime, json, build_series(), fetch_star_dates(), fmt_k(), main(), nice_step(), Generate self-hosted GitHub star-history charts (light + dark SVG). Replaces… (+1 more)

### Community 261 - "dashboard-page-frame.tsx"
Cohesion: 0.20
Nodes (9): Dashboard layout and LLM branding audit, Overview terminal sessions, Preview verification, Result, DashboardPageFrame(), dashboardPageLayout, panelLabel(), WIDE_PANELS (+1 more)

### Community 262 - "Mission Control production audit"
Cohesion: 0.17
Nodes (11): Apple design criteria, Browser verification, Decision, Findings and changes, Mission Control production audit, Performance observations, Production-readiness decision, Release handoff (+3 more)

### Community 263 - "Mission Control Fly command workers"
Cohesion: 0.17
Nodes (12): Active-session CLI bridge, Backend build and start, Client setup and active sessions, Commissioning status, Doppler presence-only preflight, Mission Control Fly command workers, Operations, observability, rollback, Operator-only image commissioning (+4 more)

### Community 264 - "Quickstart: Your First Agent in 5 Minutes"
Cohesion: 0.17
Nodes (12): Prerequisites, Quickstart: Your First Agent in 5 Minutes, Step 1: Start Mission Control, Step 2: Register an Agent, Step 3: Create a Task, Step 4: Poll the Task Queue, Step 5: Complete the Task, Step 6: Send a Heartbeat (+4 more)

### Community 265 - "github-pulls-api.ts"
Cohesion: 0.18
Nodes (16): GithubPayload, ChatGitHubItem, fetchPullRequests(), src_lib_github_getgithubtoken, emptyFeed(), handleGitHubPulls(), projectRepos(), ChatPullRequest (+8 more)

### Community 266 - "verify-jev-live.mjs"
Cohesion: 0.33
Nodes (4): apiKey, baseUrl, contexts, headers

### Community 267 - "workspace-sql-regressions.test.ts"
Cohesion: 0.40
Nodes (3): preparedTemplateSql(), source(), dispatch()

### Community 268 - "task-branch-route-agent-access.test.ts"
Cohesion: 0.33
Nodes (5): createPullRequestMock, fetchPullRequestsMock, foreignTask, prepareMock, requireRoleMock

### Community 269 - "Deferred / blocked"
Cohesion: 0.18
Nodes (10): Deferred / blocked, Fly ops commissioning, Fly secret blockers (documentation only), Mega-files (debt), Mission Control audit — 2026-09-19 (repo-only), OpenAPI parity TODOs, Overview modular board, Ruflo MCP (`CLAUDE_PLUGIN_ROOT`) (+2 more)

### Community 270 - "test_mc_fly_enroll.py"
Cohesion: 0.18
Nodes (6): importlib_util, EnrollmentTests, subprocess, tempfile, unittest, unittest_mock

### Community 271 - "session-plan.ts"
Cohesion: 0.50
Nodes (3): plan(), extractPlanMarkdown(), PlanPart

### Community 273 - "opencode-transcript.ts"
Cohesion: 0.50
Nodes (8): epochMsToIso(), getOpenCodeDbCandidates(), fallbackParts(), mapRole(), pushParsedParts(), readOpenCodeTranscript(), rec(), textPart()

### Community 274 - "Spec Files"
Cohesion: 0.18
Nodes (10): CRUD Lifecycle, E2E Tests, Features, Infrastructure, OpenClaw Offline Harness, Running, Security & Auth, Shared (+2 more)

### Community 275 - "Quick Start"
Cohesion: 0.20
Nodes (10): 1. Register a connection, 2. Heartbeat loop, 3. Subscribe to events (SSE), 4. Report token usage, 5. Disconnect, API Reference, Connection Lifecycle, Direct CLI Integration (+2 more)

### Community 276 - "Fly recovery and shared skill hardening — 2026-09-06"
Cohesion: 0.22
Nodes (6): Deployed verification, Fly recovery and shared skill hardening — 2026-09-06, Forecast and operating boundaries, Global repository access, Implemented, Verification

### Community 277 - "Образцовый пример: команда из 3 провайдеров (Claude + OpenAI + Local) в Mission Control"
Cohesion: 0.20
Nodes (9): 11. Что менять для своего сценария, 12. Полезные команды, 13. Открытые вопросы (отметь если что-то не сошлось), 1.A. Создание нового workspace (если есть OpenClaw template), 1. Workspace — рекомендация: пропустить, использовать `default`, 7.A. Про "Owner" и колонку "Awaiting Owner", 7. Master-задача, 9. Чек-лист приёмки (+1 more)

### Community 278 - "Hardening Checklist"
Cohesion: 0.20
Nodes (10): Credentials, Docker (if applicable), Hardening Checklist, Monitoring, Network, OpenClaw Gateway, Reporting a Vulnerability, Security Considerations (+2 more)

### Community 279 - "jev-setup-flow.spec.ts"
Cohesion: 0.29
Nodes (3): assistant, policy, projects

### Community 280 - "dashboard-widgets.ts"
Cohesion: 0.26
Nodes (10): WidgetGrid(), DashboardWidget, WIDGET_CATALOG, GATEWAY_DEFAULT_LAYOUT, getAvailableWidgets(), getDefaultLayout(), getWidgetById(), isSupersededDefault() (+2 more)

### Community 281 - "Mission Control Fly audit — 2026-09-06"
Cohesion: 0.22
Nodes (9): Architecture and operating limits, Findings and repairs, Image scan scope, Initial r9 live result, Mission Control Fly audit — 2026-09-06, Recovery, Recovery and global repository release, Session scope (+1 more)

### Community 282 - "Screenshot Guide"
Cohesion: 0.22
Nodes (8): Automation, How to Take New Screenshots, Prerequisites, Screenshot Guide, Screenshots in the README, Steps, Tips, When to Refresh

### Community 283 - "10. Troubleshooting"
Cohesion: 0.22
Nodes (9): 10. Troubleshooting, Aegis не запускается, Architect отвечает обычным текстом, не структурированно, dispatchModel `local/...` падает с timeout, LMStudio не отвечает / `local API 404`, `OPENAI_API_KEY not set` в логах, Кнопки `+ Create Agent` нет, Кнопки `Projects` нет на /tasks (+1 more)

### Community 284 - "release-update-route-security.test.ts"
Cohesion: 0.10
Nodes (6): expectGuardBefore(), handlerBody(), expectGuardBefore(), handlerBody(), admin, {
  execFileSyncMock,
  logAuditEventMock,
  readFileSyncMock,
  releaseUpdateLimiterMock,
  requireRoleMock,
}

### Community 286 - "Contributing to Mission Control"
Cohesion: 0.25
Nodes (8): Code Style, Contributing to Mission Control, Development Workflow, Getting Started, License, Project Structure, Reporting Bugs, Testing

### Community 287 - "Fly cleanup and capacity audit — September 6, 2026"
Cohesion: 0.25
Nodes (7): Apps and resources, Configuration for speed and efficiency, Fly cleanup and capacity audit — September 6, 2026, Remaining boundaries, Removed code, Session activation, Validation

### Community 288 - "Fly scale and cost decision — 2026-09-06"
Cohesion: 0.25
Nodes (8): Anticipated cost — transparent scenarios, Architecture, Cost improvements already implemented, Deliberate limits, Fly scale and cost decision — 2026-09-06, Recommendation, Recovery and skill integration, Sizing and growth

### Community 289 - "Jev in Mission Control"
Cohesion: 0.22
Nodes (8): Agent and CI use, Jev in Mission Control, Model management, Operator workflow, Privacy and audit behavior, Runtime configuration, Setup assistant boundary, policyId()

### Community 290 - "OpenClaw Gateway Hardening"
Cohesion: 0.11
Nodes (18): 1. Network Security, 2. Authentication, 3. Hardened Gateway Configuration, 4. File Permissions, 5. Tool Security, 6. Monitoring, 7. Known CVEs, Agent Auth: Least-Privilege Key Guidance (+10 more)

### Community 291 - "4. Агент №2 — Implementor (OpenAI gpt-4o-mini)"
Cohesion: 0.25
Nodes (8): 4.1. Step 1 — Template, 4.2. Step 2 — Configure, 4.3. Soul, 4.4. ⚠ Важно — задать `dispatchModel = openai/gpt-4o-mini`, 4. Агент №2 — Implementor (OpenAI gpt-4o-mini), Вариант A: через UI (Config tab), Вариант B: через API (если UI падает), Что должно произойти

### Community 292 - "tasks-bulk-put-agent-access.test.ts"
Cohesion: 0.33
Nodes (4): prepareMock, requireRoleMock, transactionMock, validateBodyMock

### Community 293 - "Consolidated Fly implementation audit — 2026-09-05"
Cohesion: 0.29
Nodes (6): Consolidated Fly implementation audit — 2026-09-05, Current verdict, Improvements/deferred gaps, Remaining release gates (ordered), Repairs integrated, Verification in this continuation

### Community 294 - "Local Mission Control consolidation — 2026-09-05"
Cohesion: 0.29
Nodes (6): Local Mission Control consolidation — 2026-09-05, Preservation and integration, Remaining host/runtime findings, Result, Runtime completion, Verification

### Community 295 - "Onboarding + Walkthrough hardening plan"
Cohesion: 0.29
Nodes (6): Deliverables, Goals, Onboarding + Walkthrough hardening plan, Phase 1: audit and test map, Phase 2: implementation hardening, Phase 3: verification

### Community 296 - "Mission Control 2.2.0"
Cohesion: 0.29
Nodes (6): Compatibility, Highlights, Mission Control 2.2.0, Security posture, Upgrade notes, Verification

### Community 297 - "Mission Control 2.3.0"
Cohesion: 0.29
Nodes (6): Compatibility, Highlights, Mission Control 2.3.0, Security posture, Upgrade notes, Verification

### Community 298 - "3. Агент №1 — Architect (Claude Opus)"
Cohesion: 0.29
Nodes (7): 3.1. Открыть форму создания, 3.2. Step 1 — Template (выбор шаблона), 3.3. Step 2 — Configure (точные названия полей и опций), 3.4. Step 3 — Review, 3.5. После создания — задать Soul, 3.6. dispatchModel НЕ нужен для Anthropic, 3. Агент №1 — Architect (Claude Opus)

### Community 299 - "Release Process"
Cohesion: 0.29
Nodes (6): Branch Strategy, Compatibility and Deprecation, Release Checklist, Release Process, Required Gates Before Release, Tagging and Publishing

### Community 300 - "Mission Control Installer Skill"
Cohesion: 0.29
Nodes (6): Environment Configuration, Mission Control Installer Skill, Post-Install, Prerequisites, Usage, What This Skill Does

### Community 301 - "[1.3.0] - 2026-03-02"
Cohesion: 0.40
Nodes (5): [1.3.0] - 2026-03-02, Added, Changed, Contributors, Fixed

### Community 302 - "Fly worker topology decision — September 6, 2026"
Cohesion: 0.25
Nodes (7): Boundaries, Controller split brain (found during validation, now resolved), Fly worker topology decision — September 6, 2026, Measured evidence, Post-redeploy validation (2026-09-06 20:09), Remaining cutover step (not applied), Why not per-project apps

### Community 303 - "[2.2.0] - 2026-07-17"
Cohesion: 0.33
Nodes (6): [2.2.0] - 2026-07-17, Added, Changed, Fixed, Security, Upgrade notes

### Community 304 - "Desktop consolidation"
Cohesion: 0.33
Nodes (5): Desktop consolidation, Development, History and local data, Runtime ownership, Validation

### Community 305 - "OpenClaw Config Compatibility Note"
Cohesion: 0.33
Nodes (5): Fix, Incident, OpenClaw Config Compatibility Note, Recovery, Root Cause

### Community 306 - "8. Запустить конвейер"
Cohesion: 0.33
Nodes (6): 8.1. Architect декомпозирует, 8.2. Раскладка подзадач (вручную, ~3 мин), 8.3. Dev пишет код, 8.4. Linter (опционально, показывает работу local LLM), 8.5. Aegis ревью, 8. Запустить конвейер

### Community 307 - "pull_request_template.md"
Cohesion: 0.33
Nodes (5): Contribution Checklist, Evidence, Notes, Risk Level, Summary

### Community 308 - "check-workflow-action-pins.py"
Cohesion: 0.33
Nodes (4): pathlib, re, Fail when a workflow executes a remote action through a mutable ref., sys

### Community 310 - "better-sqlite3"
Cohesion: 0.14
Nodes (9): better-sqlite3, runMigrations(), AuditEvent, createTestDb(), FakeDb, Project, Workspace, TOKEN_AGENT_KEY_SQL (+1 more)

### Community 311 - "claude-tasks.ts"
Cohesion: 0.33
Nodes (8): cachedResult, ClaudeCodeScanResult, ClaudeCodeTask, ClaudeCodeTeam, safeParse(), scanClaudeCodeTasks(), scanTasks(), scanTeams()

### Community 312 - "repository"
Cohesion: 0.67
Nodes (3): repository, type, url

### Community 313 - "Support"
Cohesion: 0.33
Nodes (6): Bugs, Feature proposals, Maintainer scope, Questions and setup help, Security reports, Support

### Community 314 - "TODO later: Overview modular board"
Cohesion: 0.33
Nodes (5): Constraints, Decisions (locked), Done, Resume, TODO later: Overview modular board

### Community 315 - "Mission Control Wiki"
Cohesion: 0.33
Nodes (6): Core Concepts, Mission Control Wiki, Notes For Maintainers, Quick Start (5 Minutes), Recommended Wiki Structure, Start Here

### Community 316 - "Mission Control desktop"
Cohesion: 0.40
Nodes (4): Development, Local configuration and authentication, Mission Control desktop, Packaging and installation

### Community 318 - "activity-timeline-widget.tsx"
Cohesion: 0.53
Nodes (5): LogLike, ActivityTimelineWidget(), getSourceLabel(), getStatusBadge(), timeAgo()

### Community 319 - "0. Подготовка"
Cohesion: 0.40
Nodes (5): 0.1. `.env` в корне проекта, 0.2. LMStudio — поднять на хосте, 0.3. Поднять MC, 0.4. Войти, 0. Подготовка

### Community 320 - "5. Агент №3 — Linter (Local LMStudio)"
Cohesion: 0.40
Nodes (5): 5.1. Step 1 — Template, 5.2. Step 2 — Configure, 5.3. Soul, 5.4. dispatchModel — указать LMStudio модель, 5. Агент №3 — Linter (Local LMStudio)

### Community 321 - "Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Where should a Jev decision dashboard live across the repository fleet, and how should it integrate with Mission Control?, Source Nodes

### Community 322 - "Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Implement complete repo-agnostic Jev by TypeSafe frontend and backend in Mission Control with advanced tests and secure rollout, Source Nodes

### Community 323 - "runtime-types.ts"
Cohesion: 0.60
Nodes (3): AGENT_RUNTIME_TYPES, AgentRuntimeType, isAgentRuntimeType()

### Community 324 - "MobileBottomSheet"
Cohesion: 1.00
Nodes (3): MobileBottomSheet(), handleClose(), handleKeyDown()

### Community 325 - "2. Project (для тикет-префикса и группировки задач)"
Cohesion: 0.50
Nodes (4): 2.1. Создать проект — top-bar форма, 2.2. После создания — раскрыть и доконфигурить (опционально), 2.3. Закрой модал, 2. Project (для тикет-префикса и группировки задач)

### Community 326 - "6. Агент №4 — Aegis (Claude Sonnet, reviewer)"
Cohesion: 0.50
Nodes (4): 6.1. Создание (если нет), 6.2. Soul (формат строгий — MC парсит ответ), 6.3. dispatchModel — оставь Anthropic (нужен дефолт), 6. Агент №4 — Aegis (Claude Sonnet, reviewer)

### Community 327 - "event-bus.ts"
Cohesion: 0.16
Nodes (10): ref_events, dynamic, GET(), runtime, GET(), eventBelongsToWorkspace(), globalBus, ServerEvent (+2 more)

## Knowledge Gaps
- **2119 isolated node(s):** `name`, `version`, `private`, `description`, `type` (+2114 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 2563 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **36 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Work-memory lessons

**Preferred sources** — corroborated by past sessions; start here.
- `runs.ts` (2× useful, score=1.996804729)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `vitest` connect `vitest` to `mac-cleanup/index.ts`, `lib/config.ts`, `auth.ts`, `chat-icons.tsx`, `assistant/route.ts`, `jev-setup-session-repository.ts`, `useMissionControl`, `login/page.tsx`, `store/index.ts`, `group-sessions.ts`, `header-bar.tsx`, `api/tasks/route.ts`, `eventBus`, `api/status/route.ts`, `jev-context.ts`, `memory-utils.ts`, `getGitHubToken`, `chat-session-identity.ts`, `security-scan.ts`, `chat-session-metrics.ts`, `task-dispatch.ts`, `engine-logo.tsx`, `chat-composer.tsx`, `task-board-panel.tsx`, `cron-management-panel.tsx`, `apiFetch`, `claude-fleet-rename.ts`, `webhooks.ts`, `next`, `package.json`, `agent-avatar.tsx`, `chat-usage-tracker.ts`, `claude-sessions.ts`, `api/memory/route.ts`, `gateway-config-panel.tsx`, `ref_fs`, `injection-guard.ts`, `fly-topology.test.ts`, `dashboard-cli-fleets.ts`, `exec-approval-panel.tsx`, `chat-display.ts`, `handoff/route.ts`, `requireRole`, `useSmartPoll`, `mcp-audit.ts`, `session-terminal-widget.tsx`, `doctor/route.ts`, `claude-fleet-plans.ts`, `gnap-sync.ts`, `jev-panel.tsx`, `session-list-local.ts`, `workload/route.ts`, `github-sync-engine.ts`, `layout.tsx`, `gateways/connect/route.ts`, `onboarding/route.ts`, `dashboard-board.ts`, `models.ts`, `skills/route.ts`, `graph/route.ts`, `fetchWithRetry`, `evals/route.ts`, `proxy.ts`, `agent-runtimes.ts`, `runtime-history.ts`, `local-session-list.ts`, `runCommand`, `recurring-tasks.ts`, `api-contract-parity.ts`, `cli-inventory.ts`, `device-identity.ts`, `chat-session-pane.tsx`, `framework-templates.ts`, `react`, `pty-websocket.ts`, `jev-cloud.test.ts`, `artifact/route.ts`, `fly-reconciler.ts`, `claude-transcript.ts`, `session-transcript-types.ts`, `claude-code-sessions.ts`, `ref_node_module`, `codex-transcript.ts`, `session-archive.ts`, `rail-width.ts`, `ref_node_crypto`, `session-continue-run.ts`, `websocket-utils.ts`, `agent-filesystem-isolation.test.ts`, `provider-subscriptions.ts`, `agent-sync.ts`, `coordinator-routing.ts`, `adapter-compliance.test.ts`, `rate-limit.ts`, `message-bubble.tsx`, `chat-fly-jobs.ts`, `openclaw-doctor-banner.tsx`, `alert-evaluate.ts`, `kimi-sessions.ts`, `dashboard-cli-fleets.test.ts`, `fleet-agents.ts`, `handoff-brief.ts`, `hook-profiles.ts`, `api/health/route.ts`, `gateway-runtime.ts`, `channels/route.ts`, `dashboard-board-persist.ts`, `readLimiter`, `session-terminal-cell.tsx`, `memory/health/route.ts`, `health-utils.test.ts`, `fleet-projects.ts`, `fly-stale-controller.ts`, `openclaw-gateway.ts`, `task-costs.ts`, `github.ts`, `task-routing.ts`, `chat-permission-mode-route.test.ts`, `tasks-route-noop-update.test.ts`, `typesafe-probe.ts`, `github-activity.ts`, `jev-client.ts`, `releases/update/route.ts`, `aggregate/route.ts`, `migrations.ts`, `os-users/route.ts`, `minimax.ts`, `jev-validation.ts`, `locale-parity.test.ts`, `token-utils.test.ts`, `gateways/control/route.ts`, `chat-folder-order-route.test.ts`, `memory-search.ts`, `repo-health-tasks.ts`, `token-runtime-isolation.test.ts`, `task-dispatch-sandbox.test.ts`, `dashboard-page-frame.tsx`, `github-pulls-api.ts`, `workspace-sql-regressions.test.ts`, `task-branch-route-agent-access.test.ts`, `session-plan.ts`, `release-update-route-security.test.ts`, `tasks-bulk-put-agent-access.test.ts`, `better-sqlite3`, `vitest.config.ts`, `runtime-types.ts`, `event-bus.ts`?**
  _High betweenness centrality (0.183) - this node is a cross-community bridge._
- **Why does `next` connect `next` to `vitest`, `lib/config.ts`, `auth.ts`, `assistant/route.ts`, `jev-setup-session-repository.ts`, `useMissionControl`, `denyUnscopedResourceForStrictWorkspace`, `hermes/route.ts`, `agent-squad-panel-phase3.tsx`, `login/page.tsx`, `store/index.ts`, `api/tasks/route.ts`, `office-panel.tsx`, `integrations/route.ts`, `api/status/route.ts`, `nav-rail.tsx`, `memory-utils.ts`, `workspaces.ts`, `security-scan.ts`, `engine-logo.tsx`, `keys/route.ts`, `task-board-panel.tsx`, `apiFetch`, `package.json`, `agent-avatar.tsx`, `api/memory/route.ts`, `ref_fs`, `github/route.ts`, `handoff/route.ts`, `requireRole`, `useSmartPoll`, `mcp-audit.ts`, `doctor/route.ts`, `gnap-sync.ts`, `workload/route.ts`, `layout.tsx`, `gateways/connect/route.ts`, `onboarding/route.ts`, `fly/status/route.ts`, `skills/route.ts`, `graph/route.ts`, `fetchWithRetry`, `evals/route.ts`, `super-admin.ts`, `proxy.ts`, `agent-runtimes.ts`, `runCommand`, `recurring-tasks.ts`, `tokens/route.ts`, `gateways/health/route.ts`, `cli-inventory.ts`, `chat-session-pane.tsx`, `framework-templates.ts`, `flight-deck/route.ts`, `react`, `runs.ts`, `artifact/route.ts`, `session-transcript-route.ts`, `session-continue-run.ts`, `agent-filesystem-isolation.test.ts`, `agent-sync.ts`, `[id]/diagnostics/route.ts`, `rate-limit.ts`, `message-bubble.tsx`, `fleetMemoryRoots`, `api/health/route.ts`, `channels/route.ts`, `readLimiter`, `memory/health/route.ts`, `pty-manager.ts`, `permission-mode/route.ts`, `chat-permission-mode-route.test.ts`, `chat/messages/route.ts`, `tasks-route-noop-update.test.ts`, `optimize/route.ts`, `releases/update/route.ts`, `aggregate/route.ts`, `migrations.ts`, `index/route.ts`, `os-users/route.ts`, `attribution/route.ts`, `locale-parity.test.ts`, `gateways/control/route.ts`, `chat-folder-order-route.test.ts`, `memory-search.ts`, `token-runtime-isolation.test.ts`, `github-pulls-api.ts`, `task-branch-route-agent-access.test.ts`, `release-update-route-security.test.ts`, `tasks-bulk-put-agent-access.test.ts`, `event-bus.ts`?**
  _High betweenness centrality (0.180) - this node is a cross-community bridge._
- **Why does `react` connect `react` to `chat-icons.tsx`, `dashboard-page-frame.tsx`, `jev-setup-session-repository.ts`, `security-audit-panel.tsx`, `useMissionControl`, `rail-width.ts`, `webhook-panel.tsx`, `agent-squad-panel-phase3.tsx`, `login/page.tsx`, `store/index.ts`, `group-sessions.ts`, `header-bar.tsx`, `office-panel.tsx`, `nav-rail.tsx`, `pipeline-tab.tsx`, `message-bubble.tsx`, `chat-fly-jobs.ts`, `openclaw-doctor-banner.tsx`, `security-scan-card.tsx`, `engine-logo.tsx`, `cost-tracker-panel.tsx`, `chat-composer.tsx`, `task-board-panel.tsx`, `cron-management-panel.tsx`, `apiFetch`, `widget-grid.tsx`, `package.json`, `chat-usage-tracker.ts`, `terminal-view.tsx`, `session-terminal-cell.tsx`, `websocket.ts`, `gateway-config-panel.tsx`, `dashboard-cli-fleets.ts`, `exec-approval-panel.tsx`, `channels-panel.tsx`, `chat-display.ts`, `agent-comms-panel.tsx`, `useSmartPoll`, `session-terminal-widget.tsx`, `jev-panel.tsx`, `skills-panel.tsx`, `layout.tsx`, `chat-session-pane.tsx`, `memory-browser-panel.tsx`, `plugins.ts`, `conversation-list.tsx`, `nodes-panel.tsx`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **What connects `name`, `version`, `private` to the rest of the system?**
  _2119 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `vitest` be split into smaller, more focused modules?**
  _Cohesion score 0.03469812630117974 - nodes in this community are weakly interconnected._
- **Should `widget-primitives.tsx` be split into smaller, more focused modules?**
  _Cohesion score 0.11088709677419355 - nodes in this community are weakly interconnected._
- **Should `mac-cleanup/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.0545996477442081 - nodes in this community are weakly interconnected._