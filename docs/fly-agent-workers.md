...
Command success, bounded failure, cancellation, replay and Machine cleanup passed on 2026-09-06. Shared capacity is 25, within compute budgets. See [commissioning evidence](fly-commissioning-2026-09-06.md) for live results, final images, image scan results and session limitations.
...
```sh
cd /Users/tylerdevries/Dev/mission-control
...
MC_FLY_CORE_IMAGE=registry.fly.io/mission-control-workers-tyler@sha256:VERIFIED_CORE_DIGEST
MC_FLY_BROWSER_IMAGE=registry.fly.io/mission-control-workers-tyler@sha256:VERIFIED_BROWSER_DIGEST
...
docker buildx build --platform linux/amd64 --provenance=false --push -f workers/browser/Dockerfile -t registry.fly.io/mission-control-workers-tyler:polled-browser-v1 .
docker buildx imagetools inspect registry.fly.io/mission-control-workers-tyler:polled-core-v1
docker buildx imagetools inspect registry.fly.io/mission-control-workers-tyler:polled-browser-v1
```

Use returned digests in Doppler. Do not use `fly deploy` on this repository's legacy `fly.toml`: it describes a second control plane, not the canonical local design. Preserve it for historical rollback only. No Vercel deployment is needed for this persistent local SQLite scheduler.
...
All capable clients use `node /Users/tylerdevries/Dev/mission-control/scripts/mc-mcp-server.cjs`, with the existing protected Mission Control profile or `MC_URL`/`MC_API_KEY` environment. For a Claude Code installation missing this server:

```sh
claude mcp add --transport stdio --scope user mission-control -- node /Users/tylerdevries/Dev/mission-control/scripts/mc-mcp-server.cjs
```
...
Use Mission Control for eligible clean heavy verification; dirty local source stays local. Browser/e2e and live-image commissioning remain separate release gates. See `docs/fly-worker-audit.md` for observed evidence and remaining gaps.
...
node /Users/tylerdevries/Dev/mission-control/scripts/mc-fly.cjs submit /absolute/path/leaf.json
node /Users/tylerdevries/Dev/mission-control/scripts/mc-fly.cjs cancel /absolute/path/cancellation.json
```
...
`mission-control-workers-tyler` is the disposable compute pool. It has zero Machines while idle and creates up to the shared limit of 25 when accepted jobs need them, subject to budgets and provider capacity. The scheduler starts up to three per reconciliation pass, pacing provider requests. Machines disappear after results and cleanup.

`mission-control-control-tyler` is a retained cloud controller with its own database/volume. Its worker admission is disabled. The canonical controller currently runs locally under launchd; all four client registrations target it. The cloud app is not an automatic failover replica, and local offload reconciliation requires the Mac controller to remain available. Never activate both independent databases as schedulers for the same pool.
