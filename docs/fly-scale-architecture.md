...
Read current regional rates at [Fly pricing](https://fly.io/docs/about/pricing/). Set all selected-class prices explicitly. Scoped Doppler configuration and independent Fly inventory were verified. Admission uses explicit regional rates with $0.25/job, $3/day and $40/month compute budgets. Reported spend is a control-plane estimate, not a reconciled provider invoice.
...
Repository access uses a secret registry with one read-only deploy key per private repository, enrolled by `scripts/mc-fly-enroll.py`. Keys are repository-scoped and read-only; no personal or OAuth token reaches a worker.
